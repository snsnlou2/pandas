...
MUSEC_PER_DAY = (1000000.0 * SEC_PER_DAY)
_mpl_units = {}
...
def _to_ordinalf(tm):
    tot_sec = ((((tm.hour * 3600) + (tm.minute * 60)) + tm.second) + float((tm.microsecond / 1000000.0)))
    return tot_sec
...
def _from_ordinal(x, tz=None):
    ...
    if (microsecond > 999990):
        dt += timedelta(microseconds=(1000000.0 - microsecond))
    return dt
...
def _daily_finder(vmin, vmax, freq):
    ...
    vmin_orig = vmin
    (vmin, vmax) = (Period(ordinal=int(vmin), freq=freq), Period(ordinal=int(vmax), freq=freq))
...
class TimeSeries_TimedeltaFormatter(Formatter):
    ...
    def __call__(self, x, pos=0):
        ...
        n_decimals = int(np.ceil(np.log10(((100 * 1000000000.0) / abs((vmax - vmin))))))
        if (n_decimals > 9):
            n_decimals = 9
