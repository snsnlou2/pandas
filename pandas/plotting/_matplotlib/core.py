...
from pandas.core.dtypes.common import is_extension_array_dtype, is_float, is_float_dtype, is_hashable, is_integer, is_integer_dtype, is_iterator, is_list_like, is_number, is_numeric_dtype
from pandas.core.dtypes.generic import ABCDataFrame, ABCIndex, ABCMultiIndex, ABCPeriodIndex, ABCSeries
...
from pandas.plotting._matplotlib.tools import create_subplots, flatten_axes, format_date_labels, get_all_lines, get_xlim, handle_shared_axes, table
if TYPE_CHECKING:
...
class MPLPlot():
    ...
    def __init__(self, data, kind=None, by=None, subplots=False, sharex=None, sharey=False, use_index=True, figsize=None, grid=None, legend=True, rot=None, ax=None, fig=None, title=None, xlim=None, ylim=None, xticks=None, yticks=None, xlabel=None, ylabel=None, sort_columns=False, fontsize=None, secondary_y=False, colormap=None, table=False, layout=None, include_bool=False, **kwds):
        ...
        self.ax = ax
        self.fig = fig
        self.axes = np.array([], dtype=object)
    ...
    def _setup_subplots(self):
        if self.subplots:
            (fig, axes) = create_subplots(naxes=self.nseries, sharex=self.sharex, sharey=self.sharey, figsize=self.figsize, ax=self.ax, layout=self.layout, layout_type=self._layout_type)
...
number of columns = {self.nseries}''')
    ...
    def _get_colors(self, num_colors=None, color_kwds='color'):
        ...
        return get_standard_colors(num_colors=num_colors, colormap=self.colormap, color=self.kwds.get(color_kwds))
        ...
        if isinstance(err, ABCDataFrame):
            ...
            if (isinstance(self.data, ABCSeries) and (err_shape[0] == 2)):
                ...
                if (err_shape[2] != len(self.data)):
                    raise ValueError(f'Asymmetrical error bars should be provided with the shape (2, {len(self.data)})')
...
class AreaPlot(LinePlot):
    ...
    @classmethod
    def _plot(cls, ax, x, y, style=None, column_num=None, stacking_id=None, is_errorbar=False, **kwds):
        if (column_num == 0):
            cls._initialize_stacker(ax, stacking_id, len(y))
        y_values = cls._get_stacked_values(ax, stacking_id, y, kwds['label'])
        line_kwds = kwds.copy()
    ...
    def __init__(self, data, **kwargs):
        ...
        if isinstance(self.data.index, ABCMultiIndex):
            if ((kwargs['ax'] is not None) and kwargs['ax'].has_data()):
                warnings.warn(('Redrawing a bar plot with a MultiIndex is not supported ' + 'and may lead to inconsistent label positions.'), UserWarning)
