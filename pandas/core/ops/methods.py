...
from pandas.core.ops.roperator import radd, rdivmod, rfloordiv, rmod, rmul, rpow, rsub, rtruediv
...
def _create_methods(cls, arith_method, comp_method):
    ...
    new_methods = {}
    new_methods.update({'add': arith_method(operator.add), 'radd': arith_method(radd), 'sub': arith_method(operator.sub), 'mul': arith_method(operator.mul), 'truediv': arith_method(operator.truediv), 'floordiv': arith_method(operator.floordiv), 'mod': arith_method(operator.mod), 'pow': arith_method(operator.pow), 'rmul': arith_method(rmul), 'rsub': arith_method(rsub), 'rtruediv': arith_method(rtruediv), 'rfloordiv': arith_method(rfloordiv), 'rpow': arith_method(rpow), 'rmod': arith_method(rmod)})
    new_methods['div'] = new_methods['truediv']
    new_methods['rdiv'] = new_methods['rtruediv']
    if have_divmod:
        new_methods['divmod'] = arith_method(divmod)
        new_methods['rdivmod'] = arith_method(rdivmod)
    new_methods.update({'eq': comp_method(operator.eq), 'ne': comp_method(operator.ne), 'lt': comp_method(operator.lt), 'gt': comp_method(operator.gt), 'le': comp_method(operator.le), 'ge': comp_method(operator.ge)})
