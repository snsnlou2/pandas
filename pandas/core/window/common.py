...
def flex_binary_moment(arg1, arg2, f, pairwise=False):
    if (not (isinstance(arg1, (np.ndarray, ABCSeries, ABCDataFrame)) and isinstance(arg2, (np.ndarray, ABCSeries, ABCDataFrame)))):
        raise TypeError('arguments to moment function must be of type np.ndarray/Series/DataFrame')
    if (isinstance(arg1, (np.ndarray, ABCSeries)) and isinstance(arg2, (np.ndarray, ABCSeries))):
        ...
        if isinstance(arg2, ABCDataFrame):
            if (pairwise is False):
                ...
                if len(result_index):
                    result = concat([concat([results[i][j] for (j, c) in enumerate(arg2.columns)], ignore_index=True) for (i, c) in enumerate(arg1.columns)], ignore_index=True, axis=1)
                    result.columns = arg1.columns
                    if (arg2.columns.nlevels > 1):
                        arg2.columns = cast(MultiIndex, arg2.columns)
                        result.index = MultiIndex.from_product((arg2.columns.levels + [result_index]))
            ...
            results = {i: f(*prep_binary(arg1.iloc[:, i], arg2)) for (i, col) in enumerate(arg1.columns)}
            return dataframe_from_int_dict(results, arg1)
    else:
