...
import collections
from typing import Dict, Generic, Hashable, Iterator, List, Optional, Sequence, Tuple, Type
import numpy as np
...
from pandas.core.dtypes.cast import maybe_cast_result, maybe_cast_result_dtype, maybe_downcast_to_dtype
from pandas.core.dtypes.common import ensure_float, ensure_float64, ensure_int64, ensure_int_or_float, ensure_platform_int, is_bool_dtype, is_categorical_dtype, is_complex_dtype, is_datetime64_any_dtype, is_datetime64tz_dtype, is_extension_array_dtype, is_float_dtype, is_integer_dtype, is_numeric_dtype, is_period_dtype, is_sparse, is_timedelta64_dtype, needs_i8_conversion
...
from pandas.core.sorting import compress_group_index, decons_obs_group_ids, get_flattened_list, get_group_index, get_group_index_sorter, get_indexer_dict

class BaseGrouper():
    '\n    This is an internal Grouper class, which actually holds\n    the generated groups\n\n    Parameters\n    ----------\n    axis : Index\n    groupings : Sequence[Grouping]\n        all the grouping instances to handle in this grouper\n        for example for grouper list to groupby, need to pass the list\n    sort : bool, default True\n        whether this grouper will give sorted result or not\n    group_keys : bool, default True\n    mutated : bool, default False\n    indexer : intp array, optional\n        the indexer created by Grouper\n        some groupers (TimeGrouper) will sort its axis and its\n        group_info is also sorted, so need the indexer to reorder\n\n    '

    def __init__(self, axis, groupings, sort=True, group_keys=True, mutated=False, indexer=None, dropna=True):
    ...
    def indices(self):
        ...
        if ((len(self.groupings) == 1) and isinstance(self.result_index, ABCCategoricalIndex)):
    ...
    def get_group_levels(self):
        ...
        return name_list
    _cython_functions = {'aggregate': {'add': 'group_add', 'prod': 'group_prod', 'min': 'group_min', 'max': 'group_max', 'mean': 'group_mean', 'median': 'group_median', 'var': 'group_var', 'first': 'group_nth', 'last': 'group_last', 'ohlc': 'group_ohlc'}, 'transform': {'cumprod': 'group_cumprod', 'cumsum': 'group_cumsum', 'cummin': 'group_cummin', 'cummax': 'group_cummax', 'rank': 'group_rank'}}
    ...
    @final
    def _get_cython_func_and_vals(self, kind, how, values, is_numeric):
    ...
    @final
    def _disallow_invalid_ops(self, values, how):
    ...
    @final
    def _ea_wrap_cython_operation(self, kind, values, how, axis, min_count=(- 1), **kwargs):
        '\n        If we have an ExtensionArray, unwrap, call _cython_operation, and\n        re-wrap if appropriate.\n        '
        orig_values = values
        if (is_datetime64tz_dtype(values.dtype) or is_period_dtype(values.dtype)):
            values = values.view('M8[ns]')
            res_values = self._cython_operation(kind, values, how, axis, min_count, **kwargs)
            if (how in ['rank']):
                return res_values
    ...
    def _cython_operation(self, kind, values, how, axis, min_count=(- 1), **kwargs):
        ...
        if is_extension_array_dtype(values.dtype):
            return self._ea_wrap_cython_operation(kind, values, how, axis, min_count, **kwargs)
        ...
        if (kind == 'aggregate'):
            ...
            result = maybe_fill(np.empty_like(values, dtype=out_dtype), fill_value=np.nan)
    ...
    @final
    def _aggregate(self, result, counts, values, comp_ids, agg_func, min_count=(- 1)):
    ...
    @final
    def _transform(self, result, values, comp_ids, transform_func, is_datetimelike, **kwargs):
        (comp_ids, _, ngroups) = self.group_info
...
class BinGrouper(BaseGrouper):
    ...
    def group_info(self):
        ...
        return (comp_ids.astype('int64', copy=False), obs_group_ids.astype('int64', copy=False), ngroups)
...
def _is_indexed_like(obj, axes, axis):
...
def get_splitter(data, labels, ngroups, axis=0):
