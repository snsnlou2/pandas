...
REDUCTIONS = ('sum', 'prod')
_unary_math_ops = ('sin', 'cos', 'exp', 'log', 'expm1', 'log1p', 'sqrt', 'sinh', 'cosh', 'tanh', 'arcsin', 'arccos', 'arctan', 'arccosh', 'arcsinh', 'arctanh', 'abs', 'log10', 'floor', 'ceil')
...
class UndefinedVariableError(NameError):
    ...
    def __init__(self, name, is_local=None):
...
class Div(BinOp):
    ...
    def __init__(self, lhs, rhs):
        ...
        if ((not isnumeric(lhs.return_type)) or (not isnumeric(rhs.return_type))):
            raise TypeError(f"unsupported operand type(s) for {self.op}: '{lhs.return_type}' and '{rhs.return_type}'")
...
class FuncNode():
    ...
    def __init__(self, name):
        ...
        if ((name not in MATHOPS) or (NUMEXPR_INSTALLED and (NUMEXPR_VERSION < LooseVersion('2.6.9')) and (name in ('floor', 'ceil')))):
