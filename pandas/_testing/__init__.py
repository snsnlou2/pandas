...
import re
import string
from typing import Any, Callable, ContextManager, List, Optional, Sequence, Tuple, Type, Union, cast
import warnings
import zipfile
import numpy as np
...
from pandas._testing.contexts import decompress_file, ensure_clean, ensure_clean_dir, ensure_safe_environment_variables, set_timezone, use_numexpr, with_csv_dialect
...
ALL_NUMPY_DTYPES = (((((((ALL_REAL_DTYPES + COMPLEX_DTYPES) + STRING_DTYPES) + DATETIME64_DTYPES) + TIMEDELTA64_DTYPES) + BOOL_DTYPES) + OBJECT_DTYPES) + BYTES_DTYPES)
...
_names = ['Alice', 'Bob', 'Charlie', 'Dan', 'Edith', 'Frank', 'George', 'Hannah', 'Ingrid', 'Jerry', 'Kevin', 'Laura', 'Michael', 'Norbert', 'Oliver', 'Patricia', 'Quinn', 'Ray', 'Sarah', 'Tim', 'Ursula', 'Victor', 'Wendy', 'Xavier', 'Yvonne', 'Zelda']
...
def _make_timeseries(start='2000-01-01', end='2000-12-31', freq='1D', seed=None):
    ...
    columns = {'name': state.choice(_names, size=n), 'id': state.poisson(1000, size=n), 'x': ((state.rand(n) * 2) - 1), 'y': ((state.rand(n) * 2) - 1)}
...
def index_subclass_makers_generator():
    make_index_funcs = [makeDateIndex, makePeriodIndex, makeTimedeltaIndex, makeRangeIndex, makeIntervalIndex, makeCategoricalIndex, makeMultiIndex]
...
def makeCustomIndex(nentries, nlevels, prefix='#', names=False, ndupe_l=None, idx_type=None):
    ...
    if (isinstance(names, str) and (nlevels == 1)):
        names = [names]
    idx_func = {'i': makeIntIndex, 'f': makeFloatIndex, 's': makeStringIndex, 'u': makeUnicodeIndex, 'dt': makeDateIndex, 'td': makeTimedeltaIndex, 'p': makePeriodIndex}.get(idx_type)
    if idx_func:
        ...
        raise ValueError(f"{repr(idx_type)} is not a legal value for `idx_type`, use  'i'/'f'/'s'/'u'/'dt'/'p'/'td'.")
...
def makeCustomDataframe(nrows, ncols, c_idx_names=True, r_idx_names=True, c_idx_nlevels=1, r_idx_nlevels=1, data_gen_f=None, c_ndupe_l=None, r_ndupe_l=None, dtype=None, c_idx_type=None, r_idx_type=None):
    ...
    assert ((r_idx_type is None) or ((r_idx_type in ('i', 'f', 's', 'u', 'dt', 'p', 'td')) and (r_idx_nlevels == 1)))
    assert ((c_idx_type is None) or ((c_idx_type in ('i', 'f', 's', 'u', 'dt', 'p', 'td')) and (c_idx_nlevels == 1)))
...
@optional_args
def network(t, url='https://www.google.com', raise_on_error=_RAISE_NETWORK_ERROR_DEFAULT, check_before_test=False, error_classes=None, skip_errnos=_network_errno_vals, _skip_on_messages=_network_error_messages):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if any(((m.lower() in e_str.lower()) for m in _skip_on_messages)):
                skip(f'Skipping test because exception message is known and error {err}')
...
@contextmanager
def assert_produces_warning(expected_warning=Warning, filter_level='always', check_stacklevel=True, raise_on_extra_warnings=True, match=None):
...
def _assert_caught_expected_warning(*, caught_warnings, expected_warning, match, check_stacklevel):
    ...
    if (match and (not matched_message)):
        raise AssertionError(f'Did not see warning {repr(expected_warning.__name__)} matching {match}')
...
def _is_unexpected_warning(actual_warning, expected_warning):
...
def _assert_raised_with_correct_stacklevel(actual_warning):
    ...
    msg = f'Warning not set with correct stacklevel. File where warning is raised: {actual_warning.filename} != {caller.filename}. Warning message: {actual_warning.message}'
