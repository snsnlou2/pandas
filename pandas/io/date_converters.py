...
def parse_all_fields(year_col, month_col, day_col, hour_col, minute_col, second_col):
    ...
    return parsing.try_parse_datetime_components(year_col, month_col, day_col, hour_col, minute_col, second_col)
...
def _check_columns(cols):
    ...
    for (i, n) in enumerate(map(len, tail)):
        if (n != N):
            raise AssertionError(f'All columns must have the same length: {N}; column {i} has length {n}')
    return N
