...
class ODSWriter(ExcelWriter):
    ...
    def __init__(self, path, engine=None, mode='w', storage_options=None, **engine_kwargs):
        from odf.opendocument import OpenDocumentSpreadsheet
        engine_kwargs['engine'] = engine
        if (mode == 'a'):
            raise ValueError('Append mode is not supported with odf!')
        super().__init__(path, mode=mode, storage_options=storage_options, **engine_kwargs)
    ...
    def _make_table_cell(self, cell):
        ...
        if isinstance(val, datetime.datetime):
            ...
            return (pvalue, TableCell(valuetype=class_to_cell_type[type(val)], value=value, attributes=attributes))
