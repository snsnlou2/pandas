...
def _convert_datetimes(sas_datetimes, unit):
    ...
    try:
        ...
        if (unit == 's'):
            return sas_datetimes.apply((lambda sas_float: (datetime(1960, 1, 1) + timedelta(seconds=sas_float))))
        elif (unit == 'd'):
            return sas_datetimes.apply((lambda sas_float: (datetime(1960, 1, 1) + timedelta(days=sas_float))))
        else:
...
class SAS7BDATReader(ReaderBase, abc.Iterator):
    ...
    def __init__(self, path_or_buf, index=None, convert_dates=True, blank_missing=True, chunksize=None, encoding=None, convert_text=True, convert_header_text=True):
        self.index = index
    ...
    def _process_subheader_pointers(self, offset, subheader_pointer_index):
        ...
        x = _SubheaderPointer(subheader_offset, subheader_length, subheader_compression, subheader_type)
        return x
...
''')
    ...
    def _process_format_subheader(self, offset, length):
        ...
        label_idx = self._read_int(text_subheader_label, const.column_label_text_subheader_index_length)
