...
def validate_window_func(name, args, kwargs):
    ...
    msg = f'numpy operations are not valid with window objects. Use .{name}() directly instead '
    if (len(args) > 0):
...
def validate_rolling_func(name, args, kwargs):
    numpy_args = ('axis', 'dtype', 'out')
    msg = f'numpy operations are not valid with window objects. Use .rolling(...).{name}() instead '
    if (len(args) > 0):
        raise UnsupportedFunctionCall(msg)
    for arg in numpy_args:
        if (arg in kwargs):
...
def validate_expanding_func(name, args, kwargs):
    numpy_args = ('axis', 'dtype', 'out')
    msg = f'numpy operations are not valid with window objects. Use .expanding(...).{name}() instead '
    if (len(args) > 0):
        raise UnsupportedFunctionCall(msg)
    for arg in numpy_args:
        if (arg in kwargs):
...
def validate_minmax_axis(axis, ndim=1):
