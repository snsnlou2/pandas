...
def test_asfreq_bug():
    ...
    expected = DataFrame(data=[1, np.nan, np.nan, 3], index=timedelta_range('0 day', periods=4, freq='1T'))
...
def test_resample_single_period_timedelta():
    ...
    expected = Series([1, 5, 4], index=pd.timedelta_range('1 day', freq='2s', periods=3))
...
def test_resample_categorical_data_with_timedeltaindex():
    ...
    expected = DataFrame({'Group_obj': ['A', 'A'], 'Group': ['A', 'A']}, index=pd.TimedeltaIndex([0, 10], unit='s', freq='10s'))
...
def test_resample_with_timedelta_yields_no_empty_groups():
    df = DataFrame(np.random.normal(size=(10000, 4)), index=pd.timedelta_range(start='0s', periods=10000, freq='3906250n'))
    result = df.loc['1s':, :].resample('3s').apply((lambda x: len(x)))
    expected = DataFrame((([([768.0] * 4)] * 12) + [([528.0] * 4)]), index=pd.timedelta_range(start='1s', periods=13, freq='3s'))
    tm.assert_frame_equal(result, expected)

def test_resample_quantile_timedelta():
    df = DataFrame({'value': pd.to_timedelta(np.arange(4), unit='s')}, index=pd.date_range('20200101', periods=4, tz='UTC'))
    result = df.resample('2D').quantile(0.99)
    expected = DataFrame({'value': [pd.Timedelta('0 days 00:00:00.990000'), pd.Timedelta('0 days 00:00:02.990000')]}, index=pd.date_range('20200101', periods=2, tz='UTC', freq='2D'))
    tm.assert_frame_equal(result, expected)
