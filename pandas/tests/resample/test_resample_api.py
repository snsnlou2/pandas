...
def test_groupby_resample_api():
    df = DataFrame({'date': pd.date_range(start='2016-01-01', periods=4, freq='W'), 'group': [1, 1, 2, 2], 'val': [5, 6, 7, 8]}).set_index('date')
    i = (pd.date_range('2016-01-03', periods=8).tolist() + pd.date_range('2016-01-17', periods=8).tolist())
...
def test_groupby_resample_on_api():
    df = DataFrame({'key': (['A', 'B'] * 5), 'dates': pd.date_range('2016-01-01', periods=10), 'values': np.random.randn(10)})
...
def test_downsample_but_actually_upsampling():
    ...
    expected = Series([0, 20, 40, 60, 80], index=pd.date_range('2012-01-01 00:00:00', freq='20s', periods=5))
    tm.assert_series_equal(result, expected)
...
def test_fillna():
    ...
    tm.assert_series_equal(result, expected)
    msg = 'Invalid fill method\\. Expecting pad \\(ffill\\), backfill \\(bfill\\) or nearest\\. Got 0'
...
def test_selection_api_validation():
    ...
    msg = "Only valid with DatetimeIndex, TimedeltaIndex or PeriodIndex, but got an instance of 'Int64Index'"
...
def test_agg_with_datetime_index_list_agg_func(col_name):
    df = DataFrame(list(range(200)), index=pd.date_range(start='2017-01-01', freq='15min', periods=200, tz='Europe/Berlin'), columns=[col_name])
    result = df.resample('1d').aggregate(['mean'])
    expected = DataFrame([47.5, 143.5, 195.5], index=pd.date_range(start='2017-01-01', freq='D', periods=3, tz='Europe/Berlin'), columns=pd.MultiIndex(levels=[[col_name], ['mean']], codes=[[0], [0]]))
    tm.assert_frame_equal(result, expected)
