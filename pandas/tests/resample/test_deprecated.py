...
all_ts = pytest.mark.parametrize('_index_factory,_series_name,_index_start,_index_end', [DATE_RANGE, PERIOD_RANGE, TIMEDELTA_RANGE])
...
@pytest.mark.parametrize('loffset', [timedelta(minutes=1), '1min', Minute(1), np.timedelta64(1, 'm')])
...
def test_resample_loffset_upsample():
    ...
    s = Series(np.random.randn(14), index=rng)
    with tm.assert_produces_warning(FutureWarning):
        result = s.resample('5min', closed='right', label='right', loffset=timedelta(minutes=1)).ffill()
    idx = date_range('1/1/2000', periods=4, freq='5min')
...
def test_resample_float_base():
    dt = pd.to_datetime(['2018-11-26 16:17:43.51', '2018-11-26 16:17:44.51', '2018-11-26 16:17:45.51'])
