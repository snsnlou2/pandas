...
@pytest.mark.parametrize('closed, expected', [('right', (lambda s: Series([s[0], s[1:6].mean(), s[6:11].mean(), s[11:].mean()], index=date_range('1/1/2000', periods=4, freq='5min', name='index')))), ('left', (lambda s: Series([s[:5].mean(), s[5:10].mean(), s[10:].mean()], index=date_range('1/1/2000 00:05', periods=3, freq='5min', name='index'))))])
...
def test_resample_integerarray():
    ts = Series(range(9), index=pd.date_range('1/1/2000', periods=9, freq='T'), dtype='Int64')
    result = ts.resample('3T').sum()
    expected = Series([3, 12, 21], index=pd.date_range('1/1/2000', periods=3, freq='3T'), dtype='Int64')
    tm.assert_series_equal(result, expected)
    result = ts.resample('3T').mean()
    expected = Series([1, 4, 7], index=pd.date_range('1/1/2000', periods=3, freq='3T'), dtype='Float64')
    tm.assert_series_equal(result, expected)
...
def test_resample_how_ohlc(series):
    ...
    expected = DataFrame(s.groupby(grouplist).agg(_ohlc).values.tolist(), index=date_range('1/1/2000', periods=4, freq='5min', name='index'), columns=['open', 'high', 'low', 'close'])
...
def test_resample_basic_from_daily():
    dti = date_range(start=datetime(2005, 1, 1), end=datetime(2005, 1, 10), freq='D', name='index')
...
def test_resample_ohlc_dataframe():
    ...
    exp = pd.concat([df['VOLUME'].resample('H').ohlc(), df['PRICE'].resample('H').ohlc()], axis=1, keys=['VOLUME', 'PRICE'])
...
def test_resample_origin_with_day_freq_on_dst():
    ...
    def _create_series(values, timestamps, freq='D'):
        return Series(values, index=DatetimeIndex([Timestamp(t, tz=tz) for t in timestamps], freq=freq, ambiguous=True))
...
def test_period_with_agg():
    s2 = Series(np.random.randint(0, 5, 50), index=pd.period_range('2012-01-01', freq='H', periods=50), dtype='float64')
...
def test_resample_dtype_preservation():
    df = DataFrame({'date': pd.date_range(start='2016-01-01', periods=4, freq='W'), 'group': [1, 1, 2, 2], 'val': Series([5, 6, 7, 8], dtype='int32')}).set_index('date')
...
def test_resample_timegrouper():
    ...
    for dates in [dates1, dates2, dates3]:
        ...
        exp_idx = DatetimeIndex(['2014-07-31', '2014-08-31', '2014-09-30', '2014-10-31', '2014-11-30'], freq='M', name='A')
...
def test_groupby_with_dst_time_change():
    index = DatetimeIndex([1478064900001000000, 1480037118776792000], tz='UTC').tz_convert('America/Chicago')
    df = DataFrame([1, 2], index=index)
    result = df.groupby(Grouper(freq='1d')).last()
    expected_index_values = pd.date_range('2016-11-02', '2016-11-24', freq='d', tz='America/Chicago')
...
def test_resample_dst_anchor():
    ...
    tm.assert_frame_equal(df.resample(rule='MS').sum(), DataFrame([5], index=DatetimeIndex([datetime(2012, 11, 1)], tz='US/Eastern', freq='MS')))
    ...
    tm.assert_frame_equal(df.resample('W-MON').agg(how)[['a', 'b', 'c']], DataFrame({'a': [0, 48, 384, 720, 1056, 1394], 'b': [47, 383, 719, 1055, 1393, 1586], 'c': [48, 336, 336, 336, 338, 193]}, index=date_range('9/30/2013', '11/4/2013', freq='W-MON', tz='Europe/Paris')), 'W-MON Frequency')
    tm.assert_frame_equal(df.resample('2W-MON').agg(how)[['a', 'b', 'c']], DataFrame({'a': [0, 48, 720, 1394], 'b': [47, 719, 1393, 1586], 'c': [48, 672, 674, 193]}, index=date_range('9/30/2013', '11/11/2013', freq='2W-MON', tz='Europe/Paris')), '2W-MON Frequency')
    tm.assert_frame_equal(df.resample('MS').agg(how)[['a', 'b', 'c']], DataFrame({'a': [0, 48, 1538], 'b': [47, 1537, 1586], 'c': [48, 1490, 49]}, index=date_range('9/1/2013', '11/1/2013', freq='MS', tz='Europe/Paris')), 'MS Frequency')
    tm.assert_frame_equal(df.resample('2MS').agg(how)[['a', 'b', 'c']], DataFrame({'a': [0, 1538], 'b': [1537, 1586], 'c': [1538, 49]}, index=date_range('9/1/2013', '11/1/2013', freq='2MS', tz='Europe/Paris')), '2MS Frequency')
    df_daily = df['10/26/2013':'10/29/2013']
    tm.assert_frame_equal(df_daily.resample('D').agg({'a': 'min', 'b': 'max', 'c': 'count'})[['a', 'b', 'c']], DataFrame({'a': [1248, 1296, 1346, 1394], 'b': [1295, 1345, 1393, 1441], 'c': [48, 50, 48, 48]}, index=date_range('10/26/2013', '10/29/2013', freq='D', tz='Europe/Paris')), 'D Frequency')
...
def test_downsample_across_dst():
    ...
    expected = Series((([5.0, np.nan] * 3) + [5.0]), index=date_range(tz.localize(dt), periods=7, freq='H'))
    tm.assert_series_equal(result, expected)

def test_downsample_across_dst_weekly():
    df = DataFrame(index=DatetimeIndex(['2017-03-25', '2017-03-26', '2017-03-27', '2017-03-28', '2017-03-29'], tz='Europe/Amsterdam'), data=[11, 12, 13, 14, 15])
    result = df.resample('1W').sum()
    expected = DataFrame([23, 42], index=DatetimeIndex(['2017-03-26', '2017-04-02'], tz='Europe/Amsterdam', freq='W'))
    tm.assert_frame_equal(result, expected)
    ...
    expected = Series(index=pd.date_range('2013-04-07', freq='W', periods=5, tz='Europe/London'), dtype=np.float64)
    tm.assert_series_equal(result, expected)
...
def test_resample_with_nat():
    index = DatetimeIndex([pd.NaT, '1970-01-01 00:00:00', pd.NaT, '1970-01-01 00:00:01', '1970-01-01 00:00:02'])
    frame = DataFrame([2, 3, 5, 7, 11], index=index)
    index_1s = DatetimeIndex(['1970-01-01 00:00:00', '1970-01-01 00:00:01', '1970-01-01 00:00:02'])
...
def test_resample_datetime_values():
    ...
    exp = Series([datetime(2016, 1, 15), pd.NaT, datetime(2016, 1, 19)], index=date_range('2016-01-15', periods=3, freq='2D'), name='timestamp')
...
def test_resample_apply_product():
    ...
    expected = DataFrame(np.array([[0, 24], [60, 210], [336, 720], [990, 1716]], dtype=np.int64), index=DatetimeIndex(['2012-03-31', '2012-06-30', '2012-09-30', '2012-12-31'], freq='Q-DEC'), columns=['A', 'B'])
    tm.assert_frame_equal(result, expected)
...
def test_resample_calendar_day_with_dst(first, last, freq_in, freq_out, exp_last):
    ts = Series(1.0, pd.date_range(first, last, freq=freq_in, tz='Europe/Amsterdam'))
    result = ts.resample(freq_out).pad()
    expected = Series(1.0, pd.date_range(first, exp_last, freq=freq_out, tz='Europe/Amsterdam'))
    tm.assert_series_equal(result, expected)
...
def test_resample_aggregate_functions_min_count(func):
    ...
    expected = Series([np.nan], index=DatetimeIndex(['2020-03-31'], dtype='datetime64[ns]', freq='Q-DEC'))
    tm.assert_series_equal(result, expected)
