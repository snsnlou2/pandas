...
test_frame = DataFrame({'A': ((([1] * 20) + ([2] * 12)) + ([3] * 8)), 'B': np.arange(40)}, index=date_range('1/1/2000', freq='s', periods=40))
...
def test_groupby_resample_on_api_with_getitem():
    df = DataFrame({'id': list('aabbb'), 'date': pd.date_range('1-1-2016', periods=5), 'data': 1})
...
def test_apply_columns_multilevel():
    ...
    expected = DataFrame(np.array(([0] * 4)).reshape(2, 2), index=date_range(start='2017-01-01', freq='1H', periods=2), columns=pd.MultiIndex.from_tuples([('A', 'a', '', 'one'), ('B', 'b', 'i', 'two')]))
    tm.assert_frame_equal(result, expected)
...
def test_resample_groupby_with_label():
    ...
    mi = [np.array([0, 0, 1, 2]), pd.to_datetime(np.array(['1999-12-26', '2000-01-02', '2000-01-02', '2000-01-02']))]
...
def test_median_duplicate_columns():
    df = DataFrame(np.random.randn(20, 3), columns=list('aaa'), index=pd.date_range('2012-01-01', periods=20, freq='s'))
    df2 = df.copy()
    df2.columns = ['a', 'b', 'c']
...
def test_apply_to_one_column_of_df():
    df = DataFrame({'col': range(10), 'col1': range(10, 20)}, index=pd.date_range('2012-01-01', periods=10, freq='20min'))
    result = df.resample('H').apply((lambda group: group.col.sum()))
    expected = Series([3, 12, 21, 9], index=pd.date_range('2012-01-01', periods=4, freq='H'))
    tm.assert_series_equal(result, expected)
    result = df.resample('H').apply((lambda group: group['col'].sum()))
    tm.assert_series_equal(result, expected)
