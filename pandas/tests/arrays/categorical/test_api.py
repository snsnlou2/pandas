...
class TestCategoricalAPIWithFactor(TestCategorical):
    ...
    def test_describe(self):
        ...
        exp_index = CategoricalIndex(['a', 'b', 'c'], name='categories', ordered=self.factor.ordered)
        expected = DataFrame({'counts': [3, 2, 3], 'freqs': [(3 / 8.0), (2 / 8.0), (3 / 8.0)]}, index=exp_index)
        tm.assert_frame_equal(desc, expected)
        cat = self.factor.copy()
        cat.set_categories(['a', 'b', 'c', 'd'], inplace=True)
        desc = cat.describe()
        exp_index = CategoricalIndex(list('abcd'), ordered=self.factor.ordered, name='categories')
