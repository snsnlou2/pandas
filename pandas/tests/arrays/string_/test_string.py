...
@pytest.fixture(params=['string', pytest.param('arrow_string', marks=skip_if_no_pyarrow)])
def dtype(request):
...
@pytest.fixture(params=[pd.arrays.StringArray, pytest.param(ArrowStringArray, marks=skip_if_no_pyarrow)])
def cls(request):
    return request.param

def test_repr(dtype, request):
    if (dtype == 'arrow_string'):
        reason = "AssertionError: assert '      A\n0     a\n1  None\n2     b' == '      A\n0     a\n1  <NA>\n2     b'"
...
@pytest.mark.parametrize('input, method', [(['a', 'b', 'c'], operator.methodcaller('capitalize')), (['a', 'b', 'c'], operator.methodcaller('capitalize')), (['a b', 'a bc. de'], operator.methodcaller('capitalize'))])
...
def test_astype_roundtrip(dtype, request):
    if (dtype == 'arrow_string'):
        ...
        mark = pytest.mark.xfail(reason='GH#36153 casting from StringArray to dt64 fails', raises=ValueError)
        request.node.add_marker(mark)
...
def test_add(dtype, request):
    if (dtype == 'arrow_string'):
        reason = "TypeError: unsupported operand type(s) for +: 'ArrowStringArray' and 'ArrowStringArray'"
        mark = pytest.mark.xfail(reason=reason)
        request.node.add_marker(mark)
...
def test_add_sequence(dtype, request):
    if (dtype == 'arrow_string'):
        reason = "TypeError: unsupported operand type(s) for +: 'ArrowStringArray' and 'list'"
        mark = pytest.mark.xfail(reason=reason)
        request.node.add_marker(mark)
...
def test_mul(dtype, request):
    if (dtype == 'arrow_string'):
        reason = "TypeError: unsupported operand type(s) for *: 'ArrowStringArray' and 'int'"
        mark = pytest.mark.xfail(reason=reason)
        request.node.add_marker(mark)
...
def test_min_max_numpy(method, box, dtype, request):
    if (dtype == 'arrow_string'):
        if (box is pd.array):
            reason = "TypeError: '<=' not supported between instances of 'str' and 'NoneType'"
        else:
            reason = "AttributeError: 'ArrowStringArray' object has no attribute 'max'"
        mark = pytest.mark.xfail(reason=reason)
        request.node.add_marker(mark)
