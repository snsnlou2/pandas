...
def test_pow_scalar(dtype):
    ...
    tm.assert_extension_array_equal(result, expected)
    result = (a ** np.nan)
    expected = FloatingArray(np.array([np.nan, np.nan, 1, np.nan, np.nan], dtype=dtype.numpy_dtype), mask=a._mask)
    tm.assert_extension_array_equal(result, expected)
    a = a[1:]
    result = (0 ** a)
...
def test_error_invalid_values(data, all_arithmetic_operators):
    ...
    s = pd.Series(data)
    ops = getattr(s, op)
    msg = '(:?can only perform ops with numeric values)|(:?FloatingArray cannot perform the operation mod)'
