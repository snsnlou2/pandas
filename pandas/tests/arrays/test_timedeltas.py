...
class TestTimedeltaArray():
    ...
    @pytest.mark.parametrize('other', [1, np.int64(1), 1.0, np.datetime64('NaT'), pd.Timestamp.now(), 'invalid', (((np.arange(10, dtype='i8') * 24) * 3600) * (10 ** 9)), (((np.arange(10) * 24) * 3600) * (10 ** 9)).view('datetime64[ns]'), pd.Timestamp.now().to_period('D')])
...
class TestReductions():
    ...
    @pytest.mark.parametrize('add', [Timedelta(0), pd.Timestamp.now(), pd.Timestamp.now('UTC'), pd.Timestamp.now('Asia/Tokyo')])
