...
def test_error_invalid_values(data, all_arithmetic_operators):
    ...
    s = pd.Series(data)
    ops = getattr(s, op)
    msg = '(:?can only perform ops with numeric values)|(:?IntegerArray cannot perform the operation mod)'
    ...
    if (op != '__rpow__'):
        msg = 'can only perform ops with numeric values|cannot perform .* with this index type: DatetimeArray|Addition/subtraction of integers and integer-arrays with DatetimeArray is no longer supported. *'
        with pytest.raises(TypeError, match=msg):
...
def test_arith_coerce_scalar(data, all_arithmetic_operators):
    ...
    if ((all_arithmetic_operators == '__rfloordiv__') and _np_version_under1p20):
        mask = ((expected == np.inf) | (expected == (- np.inf))).fillna(False).to_numpy(bool)
...
@pytest.mark.parametrize('source, target', [([1, 2, 3], [(- 1), (- 2), (- 3)]), ([1, 2, None], [(- 1), (- 2), None]), ([(- 1), 0, 1], [1, 0, (- 1)])])
