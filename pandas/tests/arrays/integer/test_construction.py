...
@pytest.mark.parametrize('values', [['foo', 'bar'], ['1', '2'], 'foo', 1, 1.0, pd.date_range('20130101', periods=2), np.array(['foo']), [[1, 2], [3, 4]], [np.nan, {'a': 1}]])
def test_to_integer_array_error(values):
    msg = '(:?.* cannot be converted to an IntegerDtype)|(:?values must be a 1D list-like)|(Cannot pass scalar)'
...
@pytest.mark.parametrize('bool_values, int_values, target_dtype, expected_dtype', [([False, True], [0, 1], Int64Dtype(), Int64Dtype()), ([False, True], [0, 1], 'Int64', Int64Dtype()), ([False, True, np.nan], [0, 1, np.nan], Int64Dtype(), Int64Dtype())])
