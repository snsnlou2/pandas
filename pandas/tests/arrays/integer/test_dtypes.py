...
def test_preserve_dtypes(op):
    ...
    expected = pd.DataFrame({'B': np.array([1.0, 3.0]), 'C': pd.array([1, 3], dtype='Int64')}, index=pd.Index(['a', 'b'], name='A'))
...
def test_astype(all_data):
    ...
    tm.assert_series_equal(result, expected)
    s = pd.Series(mixed)
    msg = 'cannot convert to .*-dtype NumPy array with missing values.*'
    with pytest.raises(ValueError, match=msg):
