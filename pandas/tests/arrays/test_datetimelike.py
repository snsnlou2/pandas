...
class SharedTests():
    ...
    def test_searchsorted_castable_strings(self, arr1d, box, request):
        ...
        with pytest.raises(TypeError, match=re.escape(f"value should be a '{arr1d._scalar_type.__name__}', 'NaT', or array of those. Got 'StringArray' instead.")):
...
class TestPeriodArray(SharedTests):
    ...
    def test_array_interface(self, arr1d):
        ...
        msg = "float\\(\\) argument must be a string or a number, not 'Period'"
        with pytest.raises(TypeError, match=msg):
...
@pytest.mark.parametrize('array,casting_nats', [(TimedeltaIndex(['1 Day', '3 Hours', 'NaT'])._data, (pd.NaT, np.timedelta64('NaT', 'ns'))), (pd.date_range('2000-01-01', periods=3, freq='D')._data, (pd.NaT, np.datetime64('NaT', 'ns'))), (pd.period_range('2000-01-01', periods=3, freq='D')._data, (pd.NaT,))], ids=(lambda x: type(x).__name__))
...
@pytest.mark.parametrize('array', [pd.date_range('2000', periods=4).array, pd.timedelta_range('2000', periods=4).array])
...
@pytest.mark.parametrize('as_index', [True, False])
@pytest.mark.parametrize('values', [pd.to_datetime(['2020-01-01', '2020-02-01']), TimedeltaIndex([1, 2], unit='D'), PeriodIndex(['2020-01-01', '2020-02-01'], freq='D')])
