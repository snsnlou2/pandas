...
class TestSparseArray():
    ...
    @pytest.mark.parametrize('data,shape,dtype', [([0, 0, 0, 0, 0], (5,), None), ([], (0,), None), ([0], (1,), None), (['A', 'A', np.nan, 'B'], (4,), object)])
...
class TestSparseArrayAnalytics():
    ...
    @pytest.mark.parametrize('arr', [np.array([0, 1, np.nan, 1]), np.array([0, 1, 1]), np.array([True, True, False])])
...
@pytest.mark.parametrize('arr, loc', [([None, 1, 2], 0), ([0, None, 2], 1), ([0, 1, None], 2), ([0, 1, 1, None, None], 3), ([1, 1, 1, 2], (- 1)), ([], (- 1))])
def test_first_fill_value_loc(arr, loc):
    result = SparseArray(arr)._first_fill_value_loc()
    assert (result == loc)

@pytest.mark.parametrize('arr', [[1, 2, np.nan, np.nan], [1, np.nan, 2, np.nan], [1, 2, np.nan]])
