...
@pytest.mark.parametrize('dtype, fill_value', [('int', 0), ('float', np.nan), ('bool', False), ('object', np.nan), ('datetime64[ns]', pd.NaT), ('timedelta64[ns]', pd.NaT)])
...
@pytest.mark.parametrize('dtype, fill_value', [('int', None), ('float', None), ('bool', None), ('object', None), ('datetime64[ns]', None), ('timedelta64[ns]', None), ('int', np.nan), ('float', 0)])
...
@pytest.mark.parametrize('a, b', [(SparseDtype('float64'), SparseDtype('float32')), (SparseDtype('float64'), SparseDtype('float64', 0)), (SparseDtype('float64'), SparseDtype('datetime64[ns]', np.nan)), (SparseDtype(int, pd.NaT), SparseDtype(float, pd.NaT)), (SparseDtype('float64'), np.dtype('float64'))])
...
@pytest.mark.parametrize('dtype, expected', [(SparseDtype(int), True), (SparseDtype(float), True), (SparseDtype(bool), True), (SparseDtype(object), False), (SparseDtype(str), False)])
...
@pytest.mark.parametrize('a, b, expected', [(SparseDtype(float, 0.0), SparseDtype(np.dtype('float'), 0.0), True), (SparseDtype(int, 0), SparseDtype(int, 0), True), (SparseDtype(float, float('nan')), SparseDtype(float, np.nan), True), (SparseDtype(float, 0), SparseDtype(float, np.nan), False), (SparseDtype(int, 0.0), SparseDtype(float, 0.0), False)])
...
@pytest.mark.parametrize('original, dtype, expected_error_msg', [(SparseDtype(float, np.nan), int, re.escape('Cannot convert non-finite values (NA or inf) to integer')), (SparseDtype(str, 'abc'), int, re.escape("invalid literal for int() with base 10: 'abc'"))])
