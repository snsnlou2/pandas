...
import numpy as np
import pandas
from pandas import Categorical, DataFrame, Index, MultiIndex, NaT, Period, RangeIndex, Series, Timestamp, bdate_range, date_range, period_range, timedelta_range
from pandas.tseries.offsets import FY5253, BusinessDay, BusinessHour, CustomBusinessDay, DateOffset, Day, Easter, Hour, LastWeekOfMonth, Minute, MonthBegin, MonthEnd, QuarterBegin, QuarterEnd, SemiMonthBegin, SemiMonthEnd, Week, WeekOfMonth, YearBegin, YearEnd
try:
...
def create_data():
    ...
    index = {'int': Index(np.arange(10)), 'date': date_range('20130101', periods=10), 'period': period_range('2013-01-01', freq='M', periods=10), 'float': Index(np.arange(10, dtype=np.float64)), 'uint': Index(np.arange(10, dtype=np.uint64)), 'timedelta': timedelta_range('00:00:00', freq='30T', periods=10)}
