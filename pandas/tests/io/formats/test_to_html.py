...
def biggie_df_fixture(request):
    ...
    if (request.param == 'mixed'):
        df = DataFrame({'A': np.random.randn(200), 'B': tm.makeStringIndex(200)}, index=np.arange(200))
        df.loc[:20, 'A'] = np.nan
        df.loc[:20, 'B'] = np.nan
        return df
...
def test_to_html_regression_GH6098():
    df = DataFrame({'clé1': ['a', 'a', 'b', 'b', 'a'], 'clé2': ['1er', '2ème', '1er', '2ème', '1er'], 'données1': np.random.randn(5), 'données2': np.random.randn(5)})
...
@pytest.mark.parametrize('option,result,expected', [(None, (lambda df: df.to_html()), '1'), (None, (lambda df: df.to_html(border=0)), '0'), (0, (lambda df: df.to_html()), '0'), (0, (lambda df: df._repr_html_()), '0')])
