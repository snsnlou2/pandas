...
class TestStyler():
    ...
    def setup_method(self, method):
        ...
        self.dataframes = [self.df, DataFrame({'f': [1.0, 2.0], 'o': ['a', 'b'], 'c': pd.Categorical(['a', 'b'])})]
    ...
    def test_where_with_one_style(self):
        ...
        expected = {(r, c): [style1] for (r, row) in enumerate(self.df.index) for (c, col) in enumerate(self.df.columns) if f(self.df.loc[(row, col)])}
        assert (result == expected)
    ...
    def test_empty(self):
        ...
        expected = [{'props': [('color', ' red')], 'selectors': ['row0_col0']}, {'props': [('', '')], 'selectors': ['row1_col0']}]
        assert (result == expected)
    ...
    def test_bar_align_left_0points(self):
        ...
        result = df.style.bar()._compute().ctx
        expected = {(0, 0): ['width: 10em', ' height: 80%'], (0, 1): ['width: 10em', ' height: 80%'], (0, 2): ['width: 10em', ' height: 80%'], (1, 0): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg,#d65f5f 50.0%, transparent 50.0%)'], (1, 1): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg,#d65f5f 50.0%, transparent 50.0%)'], (1, 2): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg,#d65f5f 50.0%, transparent 50.0%)'], (2, 0): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg,#d65f5f 100.0%, transparent 100.0%)'], (2, 1): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg,#d65f5f 100.0%, transparent 100.0%)'], (2, 2): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg,#d65f5f 100.0%, transparent 100.0%)']}
        assert (result == expected)
        result = df.style.bar(axis=1)._compute().ctx
        expected = {(0, 0): ['width: 10em', ' height: 80%'], (0, 1): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg,#d65f5f 50.0%, transparent 50.0%)'], (0, 2): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg,#d65f5f 100.0%, transparent 100.0%)'], (1, 0): ['width: 10em', ' height: 80%'], (1, 1): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg,#d65f5f 50.0%, transparent 50.0%)'], (1, 2): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg,#d65f5f 100.0%, transparent 100.0%)'], (2, 0): ['width: 10em', ' height: 80%'], (2, 1): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg,#d65f5f 50.0%, transparent 50.0%)'], (2, 2): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg,#d65f5f 100.0%, transparent 100.0%)']}
        assert (result == expected)
    ...
    def test_bar_align_mid_axis_none(self):
        ...
        expected = {(0, 0): ['width: 10em', ' height: 80%'], (1, 0): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg, transparent 33.3%, #d65f5f 33.3%, #d65f5f 50.0%, transparent 50.0%)'], (0, 1): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg,#d65f5f 33.3%, transparent 33.3%)'], (1, 1): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg, transparent 33.3%, #d65f5f 33.3%, #d65f5f 100.0%, transparent 100.0%)']}
        assert (result == expected)
    ...
    def test_bar_align_mid_nans(self):
        ...
        result = df.style.bar(align='mid', axis=None)._compute().ctx
        expected = {(0, 0): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg, transparent 25.0%, #d65f5f 25.0%, #d65f5f 50.0%, transparent 50.0%)'], (0, 1): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg,#d65f5f 25.0%, transparent 25.0%)'], (1, 1): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg, transparent 25.0%, #d65f5f 25.0%, #d65f5f 100.0%, transparent 100.0%)']}
        assert (result == expected)

    def test_bar_align_zero_nans(self):
        df = DataFrame({'A': [1, None], 'B': [(- 1), 2]})
        result = df.style.bar(align='zero', axis=None)._compute().ctx
        expected = {(0, 0): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg, transparent 50.0%, #d65f5f 50.0%, #d65f5f 75.0%, transparent 75.0%)'], (0, 1): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg, transparent 25.0%, #d65f5f 25.0%, #d65f5f 50.0%, transparent 50.0%)'], (1, 1): ['width: 10em', ' height: 80%', 'background: linear-gradient(90deg, transparent 50.0%, #d65f5f 50.0%, #d65f5f 100.0%, transparent 100.0%)']}
        assert (result == expected)
    ...
    def test_format_non_numeric_na(self):
        df = DataFrame({'object': [None, np.nan, 'foo'], 'datetime': [None, pd.NaT, pd.Timestamp('20120101')]})
        ctx = df.style.set_na_rep('NA')._translate()
        assert (ctx['body'][0][1]['display_value'] == 'NA')
        assert (ctx['body'][0][2]['display_value'] == 'NA')
    ...
    def test_highlight_null_subset(self):
        ...
        result = df.style.highlight_null(null_color='red', subset=['A']).highlight_null(null_color='green', subset=['B'])._compute().ctx
    ...
    def test_highlight_max(self):
        ...
        for max_ in [True, False]:
            ...
            expected = {(0, 1): ['background-color: yellow'], (1, 1): ['background-color: yellow']}
            assert (result == expected)
        df['C'] = ['a', 'b']
    ...
    def test_display_set_precision(self):
        df = DataFrame(data=[[1.0, 2.009], [3.2121, 4.566]], columns=['a', 'b'])
        s = Styler(df)
    ...
    def test_mi_sparse(self):
        df = DataFrame({'A': [1, 2]}, index=pd.MultiIndex.from_arrays([['a', 'a'], [0, 1]]))
        result = df.style._translate()
    ...
    def test_mi_sparse_index_names(self):
        df = DataFrame({'A': [1, 2]}, index=pd.MultiIndex.from_arrays([['a', 'a'], [0, 1]], names=['idx_level_0', 'idx_level_1']))
        result = df.style._translate()
        head = result['head'][1]
    ...
    def test_mi_sparse_column_names(self):
        df = DataFrame(np.arange(16).reshape(4, 4), index=pd.MultiIndex.from_arrays([['a', 'a', 'b', 'a'], [0, 1, 1, 2]], names=['idx_level_0', 'idx_level_1']), columns=pd.MultiIndex.from_arrays([['C1', 'C1', 'C2', 'C2'], [1, 0, 1, 0]], names=['col_0', 'col_1']))
        result = df.style._translate()
        head = result['head'][1]
...
def test_block_names():
    expected = {'before_style', 'style', 'table_styles', 'before_cellstyle', 'cellstyle', 'before_table', 'table', 'caption', 'thead', 'tbody', 'after_table', 'before_head_rows', 'head_tr', 'after_head_rows', 'before_rows', 'tr', 'after_rows'}
    result = set(Styler.template.blocks)
    assert (result == expected)

def test_from_custom_template(tmpdir):
    p = tmpdir.mkdir('templates').join('myhtml.tpl')
    p.write(textwrap.dedent('        {% extends "html.tpl" %}\n        {% block table %}\n        <h1>{{ table_title|default("My Table") }}</h1>\n        {{ super() }}\n        {% endblock table %}'))
