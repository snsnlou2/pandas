...
import pandas as pd
from pandas import DataFrame, Index, MultiIndex, NaT, Series, Timestamp, date_range, get_option, option_context, read_csv, reset_option, set_option
...
class TestDataFrameFormatting():
    ...
    def test_show_null_counts(self):
        ...
        with option_context('display.max_info_rows', 5, 'display.max_info_columns', 5):
            ...
            check(True, False)
            check(False, False)
        with tm.assert_produces_warning(FutureWarning, match='null_counts is deprecated.+'):
            buf = StringIO()
    ...
    def test_repr_truncation(self):
        ...
        with option_context('display.max_colwidth', max_len):
            df = DataFrame({'A': np.random.randn(10), 'B': [tm.rands(np.random.randint((max_len - 1), (max_len + 1))) for i in range(10)]})
            r = repr(df)
            r = r[(r.find('\n') + 1):]
    ...
    def test_repr_truncates_terminal_size(self, monkeypatch):
        ...
        index = range(5)
        columns = pd.MultiIndex.from_tuples([('This is a long title with > 37 chars.', 'cat'), ('This is a loooooonger title with > 43 chars.', 'dog')])
    ...
    def test_datetimelike_frame(self):
        df = DataFrame({'date': ([Timestamp('20130101').tz_localize('UTC')] + ([pd.NaT] * 5))})
    ...
    def test_to_string(self):
        biggie = DataFrame({'A': np.random.randn(200), 'B': tm.makeStringIndex(200)}, index=np.arange(200))
    ...
    def test_max_rows_fitted(self, length, min_rows, max_rows, expected):
        ...
        formatter = fmt.DataFrameFormatter(DataFrame(np.random.rand(length, 3)), max_rows=max_rows, min_rows=min_rows)
