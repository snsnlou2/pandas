...
class TestUltraJSONTests():
    ...
    def test_invalid_double_precision(self, invalid_val):
        ...
        msg = "Invalid value '.*' for option 'double_precision', max is '15'|an integer is required \\(got type "
...
class TestNumpyJSONTests():
    ...
    @pytest.mark.parametrize('bad_input,exc_type,err_msg,kwargs', [([{}, []], ValueError, 'nesting not supported for object or variable length dtypes', {}), ([42, None], TypeError, "int() argument must be a string, a bytes-like object or a number, not 'NoneType'", {}), ([['a'], 42], ValueError, 'Cannot decode multidimensional arrays with variable length elements to numpy', {}), ([42, {}, 'a'], TypeError, "int() argument must be a string, a bytes-like object or a number, not 'dict'", {}), ([42, ['a'], 42], ValueError, "invalid literal for int() with base 10: 'a'", {}), (['a', 'b', [], 'c'], ValueError, 'nesting not supported for object or variable length dtypes', {}), ([{'a': 'b'}], ValueError, 'Cannot decode multidimensional arrays with variable length elements to numpy', {'labelled': True}), ({'a': {'b': {'c': 42}}}, ValueError, 'labels only supported up to 2 dimensions', {'labelled': True}), ([{'a': 42, 'b': 23}, {'c': 17}], ValueError, 'cannot reshape array of size 3 into shape (2,1)', {'labelled': True})])
...
class TestPandasJSONTests():
    ...
    def test_dataframe_numpy_labelled(self, orient):
        ...
        df = DataFrame([[1, 2, 3], [4, 5, 6]], index=['a', 'b'], columns=['x', 'y', 'z'], dtype=int)
        kwargs = ({} if (orient is None) else {'orient': orient})
    ...
    def test_series_nested(self, orient):
        ...
        kwargs = ({} if (orient is None) else {'orient': orient})
        exp = {'s1': ujson.decode(ujson.encode(s, **kwargs)), 's2': ujson.decode(ujson.encode(s, **kwargs))}
        assert (ujson.decode(ujson.encode(nested, **kwargs)) == exp)
    ...
    def test_index(self):
        ...
        tm.assert_index_equal(i, output)
        output = Index(ujson.decode(ujson.encode(i, orient='values'), numpy=True), name='index')
        tm.assert_index_equal(i, output)
        output = Index(ujson.decode(ujson.encode(i, orient='records')), name='index')
        tm.assert_index_equal(i, output)
        output = Index(ujson.decode(ujson.encode(i, orient='records'), numpy=True), name='index')
        tm.assert_index_equal(i, output)
        output = Index(ujson.decode(ujson.encode(i, orient='index')), name='index')
        tm.assert_index_equal(i, output)
        output = Index(ujson.decode(ujson.encode(i, orient='index'), numpy=True), name='index')
        tm.assert_index_equal(i, output)
