...
from pandas import DataFrame, MultiIndex, Series, Timestamp, date_range, read_csv, to_datetime
...
@pytest.mark.parametrize('flavor', [pytest.param('bs4', marks=td.skip_if_no('bs4')), pytest.param('lxml', marks=td.skip_if_no('lxml'))], scope='class')
class TestReadHtml():
    ...
    def test_to_html_compat(self):
        df = tm.makeCustomDataframe(4, 3, data_gen_f=(lambda *args: np.random.rand()), c_idx_names=False, r_idx_names=False).applymap('{:.3f}'.format).astype(float)
        out = df.to_html()
    ...
    def test_banklist_url(self):
        url = 'https://www.fdic.gov/bank/individual/failed/banklist.html'
        df1 = self.read_html(url, match='First Federal Bank of Florida', attrs={'id': 'table'})
    ...
    def _bank_data(self, *args, **kwargs):
        return self.read_html(self.banklist_data, *args, match='Metcalf', attrs={'id': 'table'}, **kwargs)
    ...
    def test_banklist_header(self, datapath):
        ...
        def try_remove_ws(x):
            try:
                ...
                return x
        df = self.read_html(self.banklist_data, match='Metcalf', attrs={'id': 'table'})[0]
    ...
    def test_gold_canyon(self):
        ...
        df = self.read_html(self.banklist_data, match='Gold Canyon', attrs={'id': 'table'})[0]
