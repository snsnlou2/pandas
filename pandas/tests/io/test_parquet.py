...
@pytest.fixture(params=[pytest.param('fastparquet', marks=pytest.mark.skipif((not _HAVE_FASTPARQUET), reason='fastparquet is not installed')), pytest.param('pyarrow', marks=pytest.mark.skipif((not _HAVE_PYARROW), reason='pyarrow is not installed'))])
def engine(request):
...
def test_get_engine_auto_error_message():
    ...
    have_pa_bad_version = (False if (not _HAVE_PYARROW) else (LooseVersion(pyarrow.__version__) < LooseVersion(pa_min_ver)))
    have_fp_bad_version = (False if (not _HAVE_FASTPARQUET) else (LooseVersion(fastparquet.__version__) < LooseVersion(fp_min_ver)))
...
def test_cross_engine_fp_pa(df_cross_compat, pa, fp):
    if ((LooseVersion(pyarrow.__version__) < '0.15') and (LooseVersion(pyarrow.__version__) >= '0.13')):
        pytest.xfail('Reading fastparquet with pyarrow in 0.14 fails: https://issues.apache.org/jira/browse/ARROW-6492')
    df = df_cross_compat
    with tm.ensure_clean() as path:
...
class Base():
    ...
    def test_parquet_read_from_url(self, df_compat, engine):
        ...
        url = 'https://raw.githubusercontent.com/pandas-dev/pandas/master/pandas/tests/io/data/parquet/simple.parquet'
...
class TestBasic(Base):
    ...
    def test_write_column_multiindex(self, engine):
        ...
        msg = '\\s*parquet must have string column names for all values in\\s*each level of the MultiIndex'
        self.check_error_on_write(df, engine, ValueError, msg)
...
class TestParquetPyArrow(Base):
    ...
    @pytest.mark.xfail((is_platform_windows() and PY38), reason='localhost connection rejected', strict=False)
    def test_s3_roundtrip_explicit_fs(self, df_compat, s3_resource, pa, s3so):
        ...
        kw = {'filesystem': s3}
        check_round_trip(df_compat, pa, path='pandas-test/pyarrow.parquet', read_kwargs=kw, write_kwargs=kw)

    def test_s3_roundtrip(self, df_compat, s3_resource, pa, s3so):
        if (LooseVersion(pyarrow.__version__) <= LooseVersion('0.17.0')):
            pytest.skip()
        s3so = {'storage_options': s3so}
        check_round_trip(df_compat, pa, path='s3://pandas-test/pyarrow.parquet', read_kwargs=s3so, write_kwargs=s3so)

    @td.skip_if_no('s3fs')
    @pytest.mark.parametrize('partition_col', [['A'], []])
    def test_s3_roundtrip_for_dir(self, df_compat, s3_resource, pa, partition_col, s3so):
        expected_df = df_compat.copy()
        pa10 = ((LooseVersion(pyarrow.__version__) >= LooseVersion('1.0.0')) and (LooseVersion(pyarrow.__version__) < LooseVersion('2.0.0')))
        if partition_col:
            if pa10:
        ...
        check_round_trip(df_compat, pa, expected=expected_df, path='s3://pandas-test/parquet_dir', read_kwargs={'storage_options': s3so}, write_kwargs={'partition_cols': partition_col, 'compression': None, 'storage_options': s3so}, check_like=True, repeat=1)
    ...
    def test_additional_extension_arrays(self, pa):
        ...
        if (LooseVersion(pyarrow.__version__) >= LooseVersion('0.16.0')):
            ...
            expected = df.assign(a=df.a.astype('int64'), b=df.b.astype('int64'), c=df.c.astype('object'))
        check_round_trip(df, pa, expected=expected)
    ...
    def test_use_nullable_dtypes(self, pa):
        import pyarrow.parquet as pq
        table = pyarrow.table({'a': pyarrow.array([1, 2, 3, None], 'int64'), 'b': pyarrow.array([1, 2, 3, None], 'uint8'), 'c': pyarrow.array(['a', 'b', 'c', None]), 'd': pyarrow.array([True, False, True, None])})
        with tm.ensure_clean() as path:
            pq.write_table(table, path)
            result1 = read_parquet(path)
            result2 = read_parquet(path, use_nullable_dtypes=True)
        assert (result1['a'].dtype == np.dtype('float64'))
        expected = pd.DataFrame({'a': pd.array([1, 2, 3, None], dtype='Int64'), 'b': pd.array([1, 2, 3, None], dtype='UInt8'), 'c': pd.array(['a', 'b', 'c', None], dtype='string'), 'd': pd.array([True, False, True, None], dtype='boolean')})
...
class TestParquetFastParquet(Base):
    ...
    def test_s3_roundtrip(self, df_compat, s3_resource, fp, s3so):
        check_round_trip(df_compat, fp, path='s3://pandas-test/fastparquet.parquet', read_kwargs={'storage_options': s3so}, write_kwargs={'compression': None, 'storage_options': s3so})
