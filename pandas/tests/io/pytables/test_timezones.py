...
from pandas.tests.io.pytables.common import _maybe_remove, ensure_clean_path, ensure_clean_store
...
def test_append_with_timezones_dateutil(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        df = DataFrame({'A': [(Timestamp('20130102 2:00:00', tz=gettz('US/Eastern')) + (timedelta(hours=1) * i)) for i in range(5)]})
        ...
        _maybe_remove(store, 'df_tz')
        df = DataFrame({'A': Timestamp('20130102', tz=gettz('US/Eastern')), 'B': Timestamp('20130603', tz=gettz('US/Eastern'))}, index=range(5))
        store.append('df_tz', df)
        result = store['df_tz']
        _compare_with_tz(result, df)
        tm.assert_frame_equal(result, df)
        df = DataFrame({'A': Timestamp('20130102', tz=gettz('US/Eastern')), 'B': Timestamp('20130102', tz=gettz('EET'))}, index=range(5))
        ...
        _compare_with_tz(result, df)
        tm.assert_frame_equal(result, df)
        df = DataFrame({'A': Timestamp('20130102', tz=gettz('US/Eastern')), 'B': Timestamp('20130102', tz=gettz('CET'))}, index=range(5))
...
def test_append_with_timezones_pytz(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        _maybe_remove(store, 'df_tz')
        df = DataFrame({'A': [(Timestamp('20130102 2:00:00', tz='US/Eastern') + (timedelta(hours=1) * i)) for i in range(5)]})
        store.append('df_tz', df, data_columns=['A'])
        result = store['df_tz']
        _compare_with_tz(result, df)
        tm.assert_frame_equal(result, df)
        ...
        _maybe_remove(store, 'df_tz')
        df = DataFrame({'A': Timestamp('20130102', tz='US/Eastern'), 'B': Timestamp('20130603', tz='US/Eastern')}, index=range(5))
        store.append('df_tz', df)
        result = store['df_tz']
        _compare_with_tz(result, df)
        tm.assert_frame_equal(result, df)
        df = DataFrame({'A': Timestamp('20130102', tz='US/Eastern'), 'B': Timestamp('20130102', tz='EET')}, index=range(5))
        msg = 'invalid info for \\[values_block_1\\] for \\[tz\\], existing_value \\[US/Eastern\\] conflicts with new value \\[EET\\]'
        with pytest.raises(ValueError, match=msg):
            store.append('df_tz', df)
        _maybe_remove(store, 'df_tz')
        store.append('df_tz', df, data_columns=['A', 'B'])
        ...
        _compare_with_tz(result, df)
        tm.assert_frame_equal(result, df)
        df = DataFrame({'A': Timestamp('20130102', tz='US/Eastern'), 'B': Timestamp('20130102', tz='CET')}, index=range(5))
        msg = 'invalid info for \\[B\\] for \\[tz\\], existing_value \\[EET\\] conflicts with new value \\[CET\\]'
        with pytest.raises(ValueError, match=msg):
            store.append('df_tz', df)
    with ensure_clean_store(setup_path) as store:
...
def test_legacy_datetimetz_object(datapath, setup_path):
    expected = DataFrame({'A': Timestamp('20130102', tz='US/Eastern'), 'B': Timestamp('20130603', tz='CET')}, index=range(5))
