...
@pytest.mark.parametrize('pickle_writer', [pytest.param(python_pickler, id='python'), pytest.param(pd.to_pickle, id='pandas_proto_default'), pytest.param(functools.partial(pd.to_pickle, protocol=pickle.HIGHEST_PROTOCOL), id='pandas_proto_highest'), pytest.param(functools.partial(pd.to_pickle, protocol=4), id='pandas_proto_4'), pytest.param(functools.partial(pd.to_pickle, protocol=5), id='pandas_proto_5', marks=pytest.mark.skipif((not PY38), reason='protocol 5 not supported'))])
...
def test_legacy_sparse_warning(datapath):
    ...
    with tm.assert_produces_warning(FutureWarning):
        ...
        pd.read_pickle(datapath('io', 'data', 'pickle', 'sparseseries-0.20.3.pickle.gz'), compression='gzip')
    with tm.assert_produces_warning(FutureWarning):
        simplefilter('ignore', DeprecationWarning)
        pd.read_pickle(datapath('io', 'data', 'pickle', 'sparseframe-0.20.3.pickle.gz'), compression='gzip')
...
class TestCompression():
    _compression_to_extension = {None: '.none', 'gzip': '.gz', 'bz2': '.bz2', 'zip': '.zip', 'xz': '.xz'}
...
@pytest.mark.parametrize(['pickle_file', 'excols'], [('test_py27.pkl', pd.Index(['a', 'b', 'c'])), ('test_mi_py27.pkl', pd.MultiIndex.from_arrays([['a', 'b', 'c'], ['A', 'B', 'C']]))])
...
def test_pickle_dataframe_with_multilevel_index(multiindex_year_month_day_dataframe_random_data, multiindex_dataframe_random_data):
