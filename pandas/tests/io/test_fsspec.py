...
import numpy as np
import pytest
from pandas import DataFrame, date_range, read_csv, read_excel, read_feather, read_json, read_parquet, read_pickle, read_stata
...
def test_from_s3_csv(s3_resource, tips_file, s3so):
    ...
    tm.assert_equal(read_csv('s3://pandas-test/tips.csv.gz', storage_options=s3so), read_csv(tips_file))
    tm.assert_equal(read_csv('s3://pandas-test/tips.csv.bz2', storage_options=s3so), read_csv(tips_file))

@pytest.mark.parametrize('protocol', ['s3', 's3a', 's3n'])
@td.skip_if_no('s3fs')
def test_s3_protocols(s3_resource, tips_file, protocol, s3so):
    tm.assert_equal(read_csv(('%s://pandas-test/tips.csv' % protocol), storage_options=s3so), read_csv(tips_file))
