...
from pandas.io.stata import CategoricalConversionWarning, InvalidColumnName, PossiblePrecisionLoss, StataMissingValue, StataReader, StataWriterUTF8, read_stata
...
class TestStata():
    ...
    def test_big_dates(self):
        ...
        with tm.ensure_clean() as path:
            ...
            written_and_read_again = self.read_dta(path)
            tm.assert_frame_equal(written_and_read_again.set_index('index'), expected, check_datetimelike_compat=True)
    ...
    @staticmethod
    def _convert_categorical(from_frame):
    ...
    def test_utf8_writer(self, version):
        ...
        data = DataFrame([[1.0, 1, 'ᴬ', 'ᴀ relatively long ŝtring'], [2.0, 2, 'ᴮ', ''], [3.0, 3, 'ᴰ', None]], columns=['a', 'β', 'ĉ', 'strls'])
        data['ᴐᴬᵀ'] = cat
