...
SQL_STRINGS = {'create_iris': {'sqlite': 'CREATE TABLE iris (\n                "SepalLength" REAL,\n                "SepalWidth" REAL,\n                "PetalLength" REAL,\n                "PetalWidth" REAL,\n                "Name" TEXT\n            )', 'mysql': 'CREATE TABLE iris (\n                `SepalLength` DOUBLE,\n                `SepalWidth` DOUBLE,\n                `PetalLength` DOUBLE,\n                `PetalWidth` DOUBLE,\n                `Name` VARCHAR(200)\n            )', 'postgresql': 'CREATE TABLE iris (\n                "SepalLength" DOUBLE PRECISION,\n                "SepalWidth" DOUBLE PRECISION,\n                "PetalLength" DOUBLE PRECISION,\n                "PetalWidth" DOUBLE PRECISION,\n                "Name" VARCHAR(200)\n            )'}, 'insert_iris': {'sqlite': 'INSERT INTO iris VALUES(?, ?, ?, ?, ?)', 'mysql': 'INSERT INTO iris VALUES(%s, %s, %s, %s, "%s");', 'postgresql': 'INSERT INTO iris VALUES(%s, %s, %s, %s, %s);'}, 'create_test_types': {'sqlite': 'CREATE TABLE types_test_data (\n                    "TextCol" TEXT,\n                    "DateCol" TEXT,\n                    "IntDateCol" INTEGER,\n                    "IntDateOnlyCol" INTEGER,\n                    "FloatCol" REAL,\n                    "IntCol" INTEGER,\n                    "BoolCol" INTEGER,\n                    "IntColWithNull" INTEGER,\n                    "BoolColWithNull" INTEGER\n                )', 'mysql': 'CREATE TABLE types_test_data (\n                    `TextCol` TEXT,\n                    `DateCol` DATETIME,\n                    `IntDateCol` INTEGER,\n                    `IntDateOnlyCol` INTEGER,\n                    `FloatCol` DOUBLE,\n                    `IntCol` INTEGER,\n                    `BoolCol` BOOLEAN,\n                    `IntColWithNull` INTEGER,\n                    `BoolColWithNull` BOOLEAN\n                )', 'postgresql': 'CREATE TABLE types_test_data (\n                    "TextCol" TEXT,\n                    "DateCol" TIMESTAMP,\n                    "DateColWithTz" TIMESTAMP WITH TIME ZONE,\n                    "IntDateCol" INTEGER,\n                    "IntDateOnlyCol" INTEGER,\n                    "FloatCol" DOUBLE PRECISION,\n                    "IntCol" INTEGER,\n                    "BoolCol" BOOLEAN,\n                    "IntColWithNull" INTEGER,\n                    "BoolColWithNull" BOOLEAN\n                )'}, 'insert_test_types': {'sqlite': {'query': '\n                INSERT INTO types_test_data\n                VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)\n                ', 'fields': ('TextCol', 'DateCol', 'IntDateCol', 'IntDateOnlyCol', 'FloatCol', 'IntCol', 'BoolCol', 'IntColWithNull', 'BoolColWithNull')}, 'mysql': {'query': '\n                INSERT INTO types_test_data\n                VALUES("%s", %s, %s, %s, %s, %s, %s, %s, %s)\n                ', 'fields': ('TextCol', 'DateCol', 'IntDateCol', 'IntDateOnlyCol', 'FloatCol', 'IntCol', 'BoolCol', 'IntColWithNull', 'BoolColWithNull')}, 'postgresql': {'query': '\n                INSERT INTO types_test_data\n                VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)\n                ', 'fields': ('TextCol', 'DateCol', 'DateColWithTz', 'IntDateCol', 'IntDateOnlyCol', 'FloatCol', 'IntCol', 'BoolCol', 'IntColWithNull', 'BoolColWithNull')}}, 'read_parameters': {'sqlite': 'SELECT * FROM iris WHERE Name=? AND SepalLength=?', 'mysql': 'SELECT * FROM iris WHERE `Name`="%s" AND `SepalLength`=%s', 'postgresql': 'SELECT * FROM iris WHERE "Name"=%s AND "SepalLength"=%s'}, 'read_named_parameters': {'sqlite': '\n                SELECT * FROM iris WHERE Name=:name AND SepalLength=:length\n                ', 'mysql': '\n                SELECT * FROM iris WHERE\n                `Name`="%(name)s" AND `SepalLength`=%(length)s\n                ', 'postgresql': '\n                SELECT * FROM iris WHERE\n                "Name"=%(name)s AND "SepalLength"=%(length)s\n                '}, 'read_no_parameters_with_percent': {'sqlite': "SELECT * FROM iris WHERE Name LIKE '%'", 'mysql': "SELECT * FROM iris WHERE `Name` LIKE '%'", 'postgresql': 'SELECT * FROM iris WHERE "Name" LIKE \'%\''}, 'create_view': {'sqlite': '\n                CREATE VIEW iris_view AS\n                SELECT * FROM iris\n                '}}
...
class PandasSQLTest():
    ...
    def _load_test2_data(self):
        df = DataFrame({'A': [4, 1, 3, 6], 'B': ['asd', 'gsq', 'ylt', 'jkl'], 'C': [1.1, 3.1, 6.9, 5.3], 'D': [False, True, True, False], 'E': ['1990-11-22', '1991-10-26', '1993-11-26', '1995-12-12']})
...
class _TestSQLApi(PandasSQLTest):
    ...
    @pytest.mark.parametrize('read_sql, text, mode', [(sql.read_sql, 'SELECT * FROM types_test_data', ('sqlalchemy', 'fallback')), (sql.read_sql, 'types_test_data', 'sqlalchemy'), (sql.read_sql_query, 'SELECT * FROM types_test_data', ('sqlalchemy', 'fallback')), (sql.read_sql_table, 'types_test_data', 'sqlalchemy')])
    def test_custom_dateparsing_error(self, read_sql, text, mode, error):
        if (self.mode in mode):
            expected = self.types_test[self.flavor].astype({'DateCol': 'datetime64[ns]'})
            result = read_sql(text, con=self.conn, parse_dates={'DateCol': {'errors': error}})
    ...
    def test_get_schema_with_schema(self):
        create_sql = sql.get_schema(self.test_frame1, 'test', con=self.conn, schema='pypi')
...
class _TestSQLAlchemy(SQLAlchemyMixIn, PandasSQLTest):
    ...
    def test_date_parsing(self):
        ...
        assert issubclass(df.DateCol.dtype.type, np.datetime64)
        df = sql.read_sql_table('types_test_data', self.conn, parse_dates={'DateCol': '%Y-%m-%d %H:%M:%S'})
        assert issubclass(df.DateCol.dtype.type, np.datetime64)
        df = sql.read_sql_table('types_test_data', self.conn, parse_dates={'DateCol': {'format': '%Y-%m-%d %H:%M:%S'}})
        assert issubclass(df.DateCol.dtype.type, np.datetime64)
        df = sql.read_sql_table('types_test_data', self.conn, parse_dates=['IntDateCol'])
        assert issubclass(df.IntDateCol.dtype.type, np.datetime64)
        df = sql.read_sql_table('types_test_data', self.conn, parse_dates={'IntDateCol': 's'})
        assert issubclass(df.IntDateCol.dtype.type, np.datetime64)
        df = sql.read_sql_table('types_test_data', self.conn, parse_dates={'IntDateCol': {'unit': 's'}})
        assert issubclass(df.IntDateCol.dtype.type, np.datetime64)
...
class _TestMySQLAlchemy():
    ...
    @classmethod
    def connect(cls):
        return sqlalchemy.create_engine(f'mysql+{cls.driver}://root@localhost:{cls.port}/pandas', connect_args=cls.connect_args)
...
class _TestPostgreSQLAlchemy():
    ...
    @classmethod
    def connect(cls):
        return sqlalchemy.create_engine(f'postgresql+{cls.driver}://postgres:postgres@localhost:{cls.port}/pandas')
    ...
    def test_schema_support(self):
        ...
        df.to_sql('test_schema_public_explicit', self.conn, index=False, schema='public')
        ...
        df.to_sql('test_schema_other', self.conn, schema='other', index=False, if_exists='replace')
        df.to_sql('test_schema_other', self.conn, schema='other', index=False, if_exists='append')
    ...
    def test_copy_from_callable_insertion_method(self):
        ...
        expected.to_sql('test_copy_insert', self.conn, index=False, method=psql_insert_copy)
...
@pytest.mark.single
@pytest.mark.db
@pytest.mark.skip(reason='gh-13611: there is no support for MySQL if SQLAlchemy is not installed')
class TestXMySQL(MySQLMixIn):
    ...
    def setup_class(cls):
        ...
        try:
            ...
            raise RuntimeError("Create a group of connection parameters under the heading [pandas] in your system's mysql default file, typically located at ~/.my.cnf or /etc/.my.cnf.") from err
        except pymysql.Error as err:
            raise RuntimeError("Cannot connect to database. Create a group of connection parameters under the heading [pandas] in your system's mysql default file, typically located at ~/.my.cnf or /etc/.my.cnf.") from err
