...
@pytest.mark.parametrize('engine', [pytest.param('xlwt', marks=pytest.mark.xfail(reason='xlwt does not support openpyxl-compatible style dicts')), 'xlsxwriter', 'openpyxl'])
def test_styler_to_excel(engine):

    def style(df):
        return DataFrame([['font-weight: bold', '', ''], ['', 'color: blue', ''], ['', '', 'text-decoration: underline'], ['border-style: solid', '', ''], ['', 'font-style: italic', ''], ['', '', 'text-align: right'], ['background-color: red', '', ''], ['number-format: 0%', '', ''], ['', '', ''], ['', '', ''], ['', '', '']], index=df.index, columns=df.columns)

    def assert_equal_style(cell1, cell2, engine):
        if (engine in ['xlsxwriter', 'openpyxl']):
            pytest.xfail(reason=f'GH25351: failing on some attribute comparisons in {engine}')
    ...
    with tm.ensure_clean(('.xlsx' if (engine != 'xlwt') else '.xls')) as path:
        with ExcelWriter(path, engine=engine) as writer:
            ...
            ExcelFormatter(styled, style_converter=custom_converter).write(writer, sheet_name='custom')
        ...
        for (col1, col2) in zip(wb['frame'].columns, wb['styled'].columns):
            ...
            for (cell1, cell2) in zip(col1, col2):
                ...
                if (ref == 'B2'):
                    ...
                    assert (cell2.border.top.style == cell2.border.right.style == cell2.border.bottom.style == cell2.border.left.style == 'medium')
                elif (ref == 'C6'):
