...
engine_params = [pytest.param('xlrd', marks=[td.skip_if_no('xlrd')]), pytest.param('openpyxl', marks=[td.skip_if_no('openpyxl'), pytest.mark.filterwarnings('ignore:.*html argument')]), pytest.param(None, marks=[td.skip_if_no('xlrd')]), pytest.param('pyxlsb', marks=td.skip_if_no('pyxlsb')), pytest.param('odf', marks=td.skip_if_no('odf'))]

def _is_valid_engine_ext_pair(engine, read_ext):
    ...
    if ((read_ext == '.xlsb') and (engine != 'pyxlsb')):
        return False
    if ((engine == 'xlrd') and (xlrd_version is not None) and (xlrd_version >= '2') and (read_ext != '.xls')):
        return False
    return True
...
@pytest.fixture(autouse=True, params=[_transfer_marks(eng, ext) for eng in engine_params for ext in read_ext_params if _is_valid_engine_ext_pair(eng, ext)])
...
class TestReaders():
    ...
    def test_reader_special_dtypes(self, read_ext):
        if (pd.read_excel.keywords['engine'] == 'pyxlsb'):
            pytest.xfail('Sheets containing datetimes not supported by pyxlsb')
        expected = DataFrame.from_dict({'IntCol': [1, 2, (- 3), 4, 0], 'FloatCol': [1.25, 2.25, 1.83, 1.92, 5e-10], 'BoolCol': [True, False, True, True, False], 'StrCol': [1, 2, 3, 4, 5], 'Str2Col': ['a', 3, 'c', 'd', 'e'], 'DateCol': [datetime(2013, 10, 30), datetime(2013, 10, 31), datetime(1905, 1, 1), datetime(2013, 12, 14), datetime(2015, 3, 14)]})
        basename = 'test_types'
        ...
        actual = pd.read_excel((basename + read_ext), sheet_name='Sheet1', convert_float=False)
        ...
        actual = pd.read_excel((basename + read_ext), sheet_name='Sheet1', convert_float=False, converters={'StrCol': str})
    ...
    def test_reader_dtype(self, read_ext):
        ...
        expected = DataFrame({'a': [1, 2, 3, 4], 'b': [2.5, 3.5, 4.5, 5.5], 'c': [1, 2, 3, 4], 'd': [1.0, 2.0, np.nan, 4.0]}).reindex(columns=['a', 'b', 'c', 'd'])
        tm.assert_frame_equal(actual, expected)
    ...
    def test_read_excel_multiindex_blank_after_name(self, read_ext, sheet_name, idx_lvl2):
        if (pd.read_excel.keywords['engine'] == 'pyxlsb'):
    ...
    def test_excel_old_index_format(self, read_ext):
        ...
        data = np.array([[None, None, None, None, None], ['R0C0', 'R0C1', 'R0C2', 'R0C3', 'R0C4'], ['R1C0', 'R1C1', 'R1C2', 'R1C3', 'R1C4'], ['R2C0', 'R2C1', 'R2C2', 'R2C3', 'R2C4'], ['R3C0', 'R3C1', 'R3C2', 'R3C3', 'R3C4'], ['R4C0', 'R4C1', 'R4C2', 'R4C3', 'R4C4']])
    ...
    def test_read_excel_skiprows(self, read_ext):
        ...
        tm.assert_frame_equal(actual, expected)
        actual = pd.read_excel(('testskiprows' + read_ext), sheet_name='skiprows_list', skiprows=np.array([0, 2]))
        tm.assert_frame_equal(actual, expected)
        actual = pd.read_excel(('testskiprows' + read_ext), sheet_name='skiprows_list', skiprows=(lambda x: (x in [0, 2])))
        tm.assert_frame_equal(actual, expected)
        actual = pd.read_excel(('testskiprows' + read_ext), sheet_name='skiprows_list', skiprows=3, names=['a', 'b', 'c', 'd'])
        expected = DataFrame([[2, 3.5, pd.Timestamp('2015-01-02'), False], [3, 4.5, pd.Timestamp('2015-01-03'), False], [4, 5.5, pd.Timestamp('2015-01-04'), True]], columns=['a', 'b', 'c', 'd'])
        tm.assert_frame_equal(actual, expected)
...
class TestExcelFileRead():
    ...
    @pytest.mark.skipif(((xlrd_version is not None) and (xlrd_version >= '2')), reason='xlrd no longer supports xlsx')
    ...
    def test_read_datetime_multiindex(self, engine, read_ext):
        ...
        expected_column_index = MultiIndex.from_tuples([(pd.to_datetime('02/29/2020'), pd.to_datetime('03/01/2020'))], names=[pd.to_datetime('02/29/2020').to_pydatetime(), pd.to_datetime('03/01/2020').to_pydatetime()])
