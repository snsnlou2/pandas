...
from pandas.io.excel import ExcelFile, ExcelWriter, _OpenpyxlWriter, _XlsxWriter, _XlwtWriter, register_writer
...
class TestRoundTrip():
    ...
    def test_multiindex_interval_datetimes(self, ext):
        midx = pd.MultiIndex.from_arrays([range(4), pd.interval_range(start=pd.Timestamp('2020-01-01'), periods=4, freq='6M')])
        df = DataFrame(range(4), index=midx)
        with tm.ensure_clean(ext) as pth:
            df.to_excel(pth)
            result = pd.read_excel(pth, index_col=[0, 1])
        expected = DataFrame(range(4), pd.MultiIndex.from_arrays([range(4), ['(2020-01-31, 2020-07-31]', '(2020-07-31, 2021-01-31]', '(2021-01-31, 2021-07-31]', '(2021-07-31, 2022-01-31]']]))
        tm.assert_frame_equal(result, expected)
...
class TestExcelWriter():
    ...
    def test_int_types(self, np_type, path):
        ...
        recons = pd.read_excel(path, sheet_name='test1', convert_float=False, index_col=0)
    ...
    def test_duplicated_columns(self, path):
        ...
        tm.assert_frame_equal(result, expected)
        result = pd.read_excel(path, sheet_name='test1', index_col=0, mangle_dupe_cols=True)
        tm.assert_frame_equal(result, expected)
