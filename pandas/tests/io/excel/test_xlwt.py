...
def test_to_excel_styleconverter(ext):
    hstyle = {'font': {'bold': True}, 'borders': {'top': 'thin', 'right': 'thin', 'bottom': 'thin', 'left': 'thin'}, 'alignment': {'horizontal': 'center', 'vertical': 'top'}}
...
def test_to_excel_xlwt_warning(ext):
    ...
    with tm.ensure_clean(ext) as path:
        with tm.assert_produces_warning(FutureWarning, match='As the xlwt package is no longer maintained'):
            df.to_excel(path)

def test_option_xls_writer_deprecated(ext):
    with tm.assert_produces_warning(FutureWarning, match='As the xlwt package is no longer maintained', check_stacklevel=False):
