...
def test_excel_file_warning_with_xlsx_file(datapath):
    ...
    has_openpyxl = (import_optional_dependency('openpyxl', raise_on_missing=False, on_version='ignore') is not None)
    if (not has_openpyxl):
        with tm.assert_produces_warning(FutureWarning, raise_on_extra_warnings=False, match='The xlrd engine is no longer maintained'):
...
def test_read_excel_warning_with_xlsx_file(datapath):
    ...
    if (not has_openpyxl):
        if (xlrd_version >= '2'):
            with pytest.raises(ValueError, match='Your version of xlrd is '):
                pd.read_excel(path, 'Sheet1', engine=None)
        else:
            with tm.assert_produces_warning(FutureWarning, raise_on_extra_warnings=False, match='The xlrd engine is no longer maintained'):
                pd.read_excel(path, 'Sheet1', engine=None)
