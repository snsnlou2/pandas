...
def s3_base(worker_id):
    ...
    with tm.ensure_safe_environment_variables():
        ...
        proc = subprocess.Popen(shlex.split(f'moto_server s3 -p {endpoint_port}'), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        timeout = 5
        while (timeout > 0):
            try:
...
def s3_resource(s3_base, tips_file, jsonl_file, feather_file):
    ...
    import boto3
    import s3fs
    test_s3_files = [('tips#1.csv', tips_file), ('tips.csv', tips_file), ('tips.csv.gz', (tips_file + '.gz')), ('tips.csv.bz2', (tips_file + '.bz2')), ('items.jsonl', jsonl_file), ('simple_dataset.feather', feather_file)]
