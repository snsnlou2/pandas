...
@pytest.mark.parametrize('responder, read_method, port, parquet_engine', [(CSVUserAgentResponder, pd.read_csv, 34259, None), (JSONUserAgentResponder, pd.read_json, 34260, None), (ParquetPyArrowUserAgentResponder, pd.read_parquet, 34268, 'pyarrow'), (ParquetFastParquetUserAgentResponder, pd.read_parquet, 34273, 'fastparquet'), (PickleUserAgentResponder, pd.read_pickle, 34271, None), (StataUserAgentResponder, pd.read_stata, 34272, None), (GzippedCSVUserAgentResponder, pd.read_csv, 34261, None), (GzippedJSONUserAgentResponder, pd.read_json, 34262, None)])
...
@pytest.mark.parametrize('responder, read_method, port, parquet_engine', [(CSVUserAgentResponder, pd.read_csv, 34263, None), (JSONUserAgentResponder, pd.read_json, 34264, None), (ParquetPyArrowUserAgentResponder, pd.read_parquet, 34270, 'pyarrow'), (ParquetFastParquetUserAgentResponder, pd.read_parquet, 34275, 'fastparquet'), (PickleUserAgentResponder, pd.read_pickle, 34273, None), (StataUserAgentResponder, pd.read_stata, 34274, None), (GzippedCSVUserAgentResponder, pd.read_csv, 34265, None), (GzippedJSONUserAgentResponder, pd.read_json, 34266, None)])
def test_server_and_custom_headers(responder, read_method, port, parquet_engine):
    ...
    server_thread.start()
    if (parquet_engine is None):
        df_http = read_method(f'http://localhost:{port}', storage_options={'User-Agent': custom_user_agent})
    else:
        df_http = read_method(f'http://localhost:{port}', storage_options={'User-Agent': custom_user_agent}, engine=parquet_engine)
    server.shutdown()
    server.server_close()
    server_thread.join()
    tm.assert_frame_equal(df_true, df_http)

@pytest.mark.parametrize('responder, read_method, port', [(AllHeaderCSVResponder, pd.read_csv, 34267)])
def test_server_and_all_custom_headers(responder, read_method, port):
    custom_user_agent = 'Super Cool One'
    custom_auth_token = 'Super Secret One'
    storage_options = {'User-Agent': custom_user_agent, 'Auth': custom_auth_token}
    server = http.server.HTTPServer(('localhost', port), responder)
    server_thread = threading.Thread(target=server.serve_forever)
    server_thread.start()
    df_http = read_method(f'http://localhost:{port}', storage_options=storage_options)
    server.shutdown()
    server.server_close()
    server_thread.join()
