...
@pytest.mark.network
@pytest.mark.parametrize('compress_type, extension', [('gzip', '.gz'), ('bz2', '.bz2'), ('zip', '.zip'), ('xz', '.xz')])
...
def check_compressed_urls(salaries_table, compression, extension, mode, engine):
    base_url = 'https://github.com/pandas-dev/pandas/raw/master/pandas/tests/io/parser/data/salaries.csv'
...
def test_url_encoding_csv():
    ...
    path = ('https://raw.githubusercontent.com/pandas-dev/pandas/master/' + 'pandas/tests/io/parser/data/unicode_series.csv')
...
class TestS3():
    ...
    def test_parse_public_s3_bucket(self, tips_df, s3so):
        for (ext, comp) in [('', None), ('.gz', 'gzip'), ('.bz2', 'bz2')]:
            df = read_csv(('s3://pandas-test/tips.csv' + ext), compression=comp, storage_options=s3so)
    ...
    def test_parse_public_s3_bucket_nrows(self, tips_df, s3so):
        for (ext, comp) in [('', None), ('.gz', 'gzip'), ('.bz2', 'bz2')]:
            df = read_csv(('s3://pandas-test/tips.csv' + ext), nrows=10, compression=comp, storage_options=s3so)
            assert isinstance(df, DataFrame)
            assert (not df.empty)
            tm.assert_frame_equal(tips_df.iloc[:10], df)
    ...
    def test_parse_public_s3_bucket_chunked(self, tips_df, s3so):
        chunksize = 5
        for (ext, comp) in [('', None), ('.gz', 'gzip'), ('.bz2', 'bz2')]:
            with read_csv(('s3://pandas-test/tips.csv' + ext), chunksize=chunksize, compression=comp, storage_options=s3so) as df_reader:
    ...
    def test_parse_public_s3_bucket_chunked_python(self, tips_df, s3so):
        chunksize = 5
        for (ext, comp) in [('', None), ('.gz', 'gzip'), ('.bz2', 'bz2')]:
            with read_csv(('s3://pandas-test/tips.csv' + ext), chunksize=chunksize, compression=comp, engine='python', storage_options=s3so) as df_reader:
                assert (df_reader.chunksize == chunksize)
                for i_chunk in [0, 1, 2]:
                    df = df_reader.get_chunk()
                    assert isinstance(df, DataFrame)
    ...
    def test_parse_public_s3_bucket_python(self, tips_df, s3so):
        for (ext, comp) in [('', None), ('.gz', 'gzip'), ('.bz2', 'bz2')]:
            df = read_csv(('s3://pandas-test/tips.csv' + ext), engine='python', compression=comp, storage_options=s3so)
            assert isinstance(df, DataFrame)
            assert (not df.empty)
            tm.assert_frame_equal(df, tips_df)
    ...
    def test_infer_s3_compression(self, tips_df, s3so):
        for ext in ['', '.gz', '.bz2']:
            df = read_csv(('s3://pandas-test/tips.csv' + ext), engine='python', compression='infer', storage_options=s3so)
            assert isinstance(df, DataFrame)
            assert (not df.empty)
            tm.assert_frame_equal(df, tips_df)
    ...
    def test_parse_public_s3_bucket_nrows_python(self, tips_df, s3so):
        for (ext, comp) in [('', None), ('.gz', 'gzip'), ('.bz2', 'bz2')]:
            df = read_csv(('s3://pandas-test/tips.csv' + ext), engine='python', nrows=10, compression=comp, storage_options=s3so)
            assert isinstance(df, DataFrame)
            assert (not df.empty)
            tm.assert_frame_equal(tips_df.iloc[:10], df)
