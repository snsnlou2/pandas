...
def test_multiple_date_col_custom(all_parsers, keep_date_col):
    ...
    result = parser.read_csv(StringIO(data), header=None, date_parser=date_parser, prefix='X', parse_dates={'actual': [1, 2], 'nominal': [1, 3]}, keep_date_col=keep_date_col)
...
def test_multiple_date_col(all_parsers, keep_date_col):
    data = 'KORD,19990127, 19:00:00, 18:56:00, 0.8100, 2.8100, 7.2000, 0.0000, 280.0000\nKORD,19990127, 20:00:00, 19:56:00, 0.0100, 2.2100, 7.2000, 0.0000, 260.0000\nKORD,19990127, 21:00:00, 20:56:00, -0.5900, 2.2100, 5.7000, 0.0000, 280.0000\nKORD,19990127, 21:00:00, 21:18:00, -0.9900, 2.0100, 3.6000, 0.0000, 270.0000\nKORD,19990127, 22:00:00, 21:56:00, -0.5900, 1.7100, 5.1000, 0.0000, 290.0000\nKORD,19990127, 23:00:00, 22:56:00, -0.5900, 1.7100, 4.6000, 0.0000, 280.0000\n'
    parser = all_parsers
    result = parser.read_csv(StringIO(data), header=None, prefix='X', parse_dates=[[1, 2], [1, 3]], keep_date_col=keep_date_col)
...
def test_date_col_as_index_col(all_parsers):
    data = 'KORD,19990127 19:00:00, 18:56:00, 0.8100, 2.8100, 7.2000, 0.0000, 280.0000\nKORD,19990127 20:00:00, 19:56:00, 0.0100, 2.2100, 7.2000, 0.0000, 260.0000\nKORD,19990127 21:00:00, 20:56:00, -0.5900, 2.2100, 5.7000, 0.0000, 280.0000\nKORD,19990127 21:00:00, 21:18:00, -0.9900, 2.0100, 3.6000, 0.0000, 270.0000\nKORD,19990127 22:00:00, 21:56:00, -0.5900, 1.7100, 5.1000, 0.0000, 290.0000\n'
    parser = all_parsers
    result = parser.read_csv(StringIO(data), header=None, prefix='X', parse_dates=[1], index_col=1)
...
@pytest.mark.parametrize('date_parser, warning', ([conv.parse_date_time, FutureWarning], [pd.to_datetime, None]))
def test_multiple_date_cols_int_cast(all_parsers, date_parser, warning):
    ...
    with tm.assert_produces_warning(warning, check_stacklevel=False):
        result = parser.read_csv(StringIO(data), header=None, date_parser=date_parser, parse_dates=parse_dates, prefix='X')
    expected = DataFrame([[datetime(1999, 1, 27, 19, 0), datetime(1999, 1, 27, 18, 56), 'KORD', 0.81], [datetime(1999, 1, 27, 20, 0), datetime(1999, 1, 27, 19, 56), 'KORD', 0.01], [datetime(1999, 1, 27, 21, 0), datetime(1999, 1, 27, 20, 56), 'KORD', (- 0.59)], [datetime(1999, 1, 27, 21, 0), datetime(1999, 1, 27, 21, 18), 'KORD', (- 0.99)], [datetime(1999, 1, 27, 22, 0), datetime(1999, 1, 27, 21, 56), 'KORD', (- 0.59)], [datetime(1999, 1, 27, 23, 0), datetime(1999, 1, 27, 22, 56), 'KORD', (- 0.59)]], columns=['actual', 'nominal', 'X0', 'X4'])
    result = result[expected.columns]
    tm.assert_frame_equal(result, expected)
...
def test_multiple_date_col_timestamp_parse(all_parsers):
    ...
    result = parser.read_csv(StringIO(data), parse_dates=[[0, 1]], header=None, date_parser=Timestamp)
...
def test_date_parser_int_bug(all_parsers):
    ...
    result = parser.read_csv(StringIO(data), index_col=0, parse_dates=[0], date_parser=(lambda x: datetime.utcfromtimestamp(int(x))))
...
def test_csv_custom_parser(all_parsers):
    data = 'A,B,C\n20090101,a,1,2\n20090102,b,3,4\n20090103,c,4,5\n'
    parser = all_parsers
    result = parser.read_csv(StringIO(data), date_parser=(lambda x: datetime.strptime(x, '%Y%m%d')))
...
def test_parse_dates_custom_euro_format(all_parsers, kwargs):
    ...
    if ('dayfirst' in kwargs):
        df = parser.read_csv(StringIO(data), names=['time', 'Q', 'NTU'], date_parser=(lambda d: du_parse(d, **kwargs)), header=0, index_col=0, parse_dates=True, na_values=['NA'])
        ...
        with pytest.raises(TypeError, match=msg), tm.assert_produces_warning(FutureWarning):
...
def test_bad_date_parse(all_parsers, cache_dates, value):
    ...
    s = StringIO((f'''{value},
''' * 50000))
    parser.read_csv(s, header=None, names=['foo', 'bar'], parse_dates=['foo'], infer_datetime_format=False, cache_dates=cache_dates)
...
def test_parse_date_time_multi_level_column_name(all_parsers, date_parser, warning):
    ...
    parser = all_parsers
    with tm.assert_produces_warning(warning, check_stacklevel=False):
        result = parser.read_csv(StringIO(data), header=[0, 1], parse_dates={'date_time': [0, 1]}, date_parser=date_parser)
...
@pytest.mark.parametrize('date_parser, warning', ([conv.parse_date_fields, FutureWarning], [pd.to_datetime, None]))
def test_parse_date_fields(all_parsers, date_parser, warning):
    parser = all_parsers
    data = 'year,month,day,a\n2001,01,10,10.\n2001,02,1,11.'
    with tm.assert_produces_warning(warning, check_stacklevel=False):
        result = parser.read_csv(StringIO(data), header=0, parse_dates={'ymd': [0, 1, 2]}, date_parser=date_parser)
    expected = DataFrame([[datetime(2001, 1, 10), 10.0], [datetime(2001, 2, 1), 11.0]], columns=['ymd', 'a'])
    tm.assert_frame_equal(result, expected)

@pytest.mark.parametrize('date_parser, warning', ([conv.parse_all_fields, FutureWarning], [(lambda x: pd.to_datetime(x, format='%Y %m %d %H %M %S')), None]))
def test_parse_date_all_fields(all_parsers, date_parser, warning):
    parser = all_parsers
    data = 'year,month,day,hour,minute,second,a,b\n2001,01,05,10,00,0,0.0,10.\n2001,01,5,10,0,00,1.,11.\n'
    with tm.assert_produces_warning(warning, check_stacklevel=False):
        result = parser.read_csv(StringIO(data), header=0, date_parser=date_parser, parse_dates={'ymdHMS': [0, 1, 2, 3, 4, 5]})
    expected = DataFrame([[datetime(2001, 1, 5, 10, 0, 0), 0.0, 10.0], [datetime(2001, 1, 5, 10, 0, 0), 1.0, 11.0]], columns=['ymdHMS', 'a', 'b'])
    tm.assert_frame_equal(result, expected)

@pytest.mark.parametrize('date_parser, warning', ([conv.parse_all_fields, FutureWarning], [(lambda x: pd.to_datetime(x, format='%Y %m %d %H %M %S.%f')), None]))
...
def test_generic(all_parsers):
    ...
    with tm.assert_produces_warning(FutureWarning, check_stacklevel=False):
        result = parser.read_csv(StringIO(data), header=0, parse_dates={'ym': [0, 1]}, date_parser=(lambda y, m: date(year=int(y), month=int(m), day=1)))
...
def test_date_parser_resolution_if_not_ns(all_parsers):
    ...
    result = parser.read_csv(StringIO(data), date_parser=date_parser, parse_dates={'datetime': ['date', 'time']}, index_col=['datetime', 'prn'])
    datetimes = np_array_datetime64_compat((['2013-11-03T19:00:00Z'] * 3), dtype='datetime64[s]')
    expected = DataFrame(data={'rxstatus': (['00E80000'] * 3)}, index=MultiIndex.from_tuples([(datetimes[0], 126), (datetimes[1], 23), (datetimes[2], 13)], names=['datetime', 'prn']))
    tm.assert_frame_equal(result, expected)
...
def test_parse_timezone(all_parsers):
    ...
    dti = DatetimeIndex(list(pd.date_range(start='2018-01-04 09:01:00', end='2018-01-04 09:05:00', freq='1min', tz=pytz.FixedOffset(540))), freq=None)
...
@pytest.mark.parametrize('date_string,dayfirst,expected', [('13/02/2019', False, datetime(2019, 2, 13)), ('13/02/2019', True, datetime(2019, 2, 13)), ('02/13/2019', False, datetime(2019, 2, 13)), ('02/13/2019', True, datetime(2019, 2, 13)), ('04/02/2019', True, datetime(2019, 2, 4))])
...
def test_hypothesis_delimited_date(date_format, dayfirst, delimiter, test_datetime):
    if ((date_format == '%m %Y') and (delimiter == '.')):
        pytest.skip('parse_datetime_string cannot reliably tell whether         e.g. %m.%Y is a float or a date, thus we skip it')
    (result, expected) = (None, None)
    (except_in_dateutil, except_out_dateutil) = (None, None)
    date_string = test_datetime.strftime(date_format.replace(' ', delimiter))
    (except_out_dateutil, result) = _helper_hypothesis_delimited_date(parse_datetime_string, date_string, dayfirst=dayfirst)
    (except_in_dateutil, expected) = _helper_hypothesis_delimited_date(du_parse, date_string, default=_DEFAULT_DATETIME, dayfirst=dayfirst, yearfirst=False)
