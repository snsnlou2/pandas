...
def test_empty_pass_dtype(all_parsers):
    ...
    expected = DataFrame({'one': np.empty(0, dtype='u1'), 'two': np.empty(0, dtype=object)}, index=Index([], dtype=object))
    tm.assert_frame_equal(result, expected)
...
def test_empty_with_mangled_column_pass_dtype_by_names(all_parsers):
    ...
    expected = DataFrame({'one': np.empty(0, dtype='u1'), 'one.1': np.empty(0, dtype='f')}, index=Index([], dtype=object))
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize('dtype,expected', [(np.float64, DataFrame(columns=['a', 'b'], dtype=np.float64)), ('category', DataFrame({'a': Categorical([]), 'b': Categorical([])}, index=[])), ({'a': 'category', 'b': 'category'}, DataFrame({'a': Categorical([]), 'b': Categorical([])}, index=[])), ('datetime64[ns]', DataFrame(columns=['a', 'b'], dtype='datetime64[ns]')), ('timedelta64[ns]', DataFrame({'a': Series([], dtype='timedelta64[ns]'), 'b': Series([], dtype='timedelta64[ns]')}, index=[])), ({'a': np.int64, 'b': np.int32}, DataFrame({'a': Series([], dtype=np.int64), 'b': Series([], dtype=np.int32)}, index=[])), ({0: np.int64, 1: np.int32}, DataFrame({'a': Series([], dtype=np.int64), 'b': Series([], dtype=np.int32)}, index=[])), ({'a': np.int64, 1: np.int32}, DataFrame({'a': Series([], dtype=np.int64), 'b': Series([], dtype=np.int32)}, index=[]))])
