...
@pytest.mark.parametrize('dtype', ['category', CategoricalDtype(), {'a': 'category', 'b': 'category', 'c': CategoricalDtype()}])
def test_categorical_dtype(all_parsers, dtype):
    parser = all_parsers
    data = 'a,b,c\n1,a,3.4\n1,a,3.4\n2,b,4.5'
    expected = DataFrame({'a': Categorical(['1', '1', '2']), 'b': Categorical(['a', 'a', 'b']), 'c': Categorical(['3.4', '3.4', '4.5'])})
...
def test_categorical_category_dtype_unsorted(all_parsers):
    ...
    expected = DataFrame({'a': [1, 1, 1, 2], 'b': Categorical(['a', 'b', 'b', 'c'], categories=['c', 'b', 'a'])})
