...
@pytest.mark.parametrize('args', [(1, pd.Series([1])), (1, pd.DataFrame({'A': [1]})), (pd.Series([1]), 1), (pd.DataFrame({'A': [1]}), 1), (pd.Series([1]), pd.Series([1])), (pd.DataFrame({'A': [1]}), pd.DataFrame({'A': [1]})), (pd.Series([1]), pd.DataFrame({'A': [1]})), (pd.DataFrame({'A': [1]}), pd.Series([1]))])
...
@pytest.mark.parametrize('method', [operator.methodcaller('sum'), (lambda x: x.agg('sum'))])
...
@pytest.mark.parametrize('obj', [pd.Series([0, 0]), pd.DataFrame({'A': [0, 1], 'B': [1, 2]})])
@pytest.mark.parametrize('method', [(lambda x: x.agg(['sum', 'count'])), (lambda x: x.transform((lambda y: y))), (lambda x: x.apply((lambda y: y)))])
@not_implemented_mark
