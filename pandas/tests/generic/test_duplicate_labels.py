...
class TestPreserves():
    ...
    @pytest.mark.parametrize('frame', [False, pytest.param(True, marks=not_implemented)])
    ...
    @pytest.mark.parametrize('objs, kwargs', [([pd.Series(1, index=['a', 'b']).set_flags(allows_duplicate_labels=False), pd.Series(2, index=['c', 'd']).set_flags(allows_duplicate_labels=False)], {}), ([pd.Series(1, index=['a', 'b']).set_flags(allows_duplicate_labels=False), pd.Series(2, index=['a', 'b']).set_flags(allows_duplicate_labels=False)], {'ignore_index': True}), ([pd.Series(1, index=['a', 'b']).set_flags(allows_duplicate_labels=False), pd.Series(2, index=['a', 'b']).set_flags(allows_duplicate_labels=False)], {'axis': 1}), ([pd.DataFrame({'A': [1, 2]}, index=['a', 'b']).set_flags(allows_duplicate_labels=False), pd.DataFrame({'A': [1, 2]}, index=['c', 'd']).set_flags(allows_duplicate_labels=False)], {}), ([pd.DataFrame({'A': [1, 2]}, index=['a', 'b']).set_flags(allows_duplicate_labels=False), pd.DataFrame({'A': [1, 2]}, index=['a', 'b']).set_flags(allows_duplicate_labels=False)], {'ignore_index': True}), ([pd.DataFrame({'A': [1, 2]}, index=['a', 'b']).set_flags(allows_duplicate_labels=False), pd.DataFrame({'B': [1, 2]}, index=['a', 'b']).set_flags(allows_duplicate_labels=False)], {'axis': 1}), ([pd.DataFrame({'A': [1, 2]}, index=['a', 'b']).set_flags(allows_duplicate_labels=False), pd.Series([1, 2], index=['a', 'b'], name='B').set_flags(allows_duplicate_labels=False)], {'axis': 1})])
    def test_concat(self, objs, kwargs):
        result = pd.concat(objs, **kwargs)
        assert (result.flags.allows_duplicate_labels is False)

    @pytest.mark.parametrize('left, right, kwargs, expected', [pytest.param(pd.DataFrame({'A': [0, 1]}, index=['a', 'b']).set_flags(allows_duplicate_labels=False), pd.DataFrame({'B': [0, 1]}, index=['a', 'd']).set_flags(allows_duplicate_labels=False), {'left_index': True, 'right_index': True}, False, marks=not_implemented), pytest.param(pd.DataFrame({'A': [0, 1]}, index=['a', 'b']).set_flags(allows_duplicate_labels=False), pd.DataFrame({'B': [0, 1]}, index=['a', 'd']), {'left_index': True, 'right_index': True}, False, marks=not_implemented), (pd.DataFrame({'A': [0, 1]}, index=['a', 'b']), pd.DataFrame({'B': [0, 1]}, index=['a', 'd']), {'left_index': True, 'right_index': True}, True)])
...
class TestRaises():
    ...
    @pytest.mark.parametrize('func', [operator.methodcaller('append', pd.Series(0, index=['a', 'b']))])
    ...
    @pytest.mark.parametrize('objs, kwargs', [([pd.Series(1, index=[0, 1], name='a').set_flags(allows_duplicate_labels=False), pd.Series(2, index=[0, 1], name='a').set_flags(allows_duplicate_labels=False)], {'axis': 1})])
...
@pytest.mark.parametrize('method, frame_only', [(operator.methodcaller('set_index', 'A', inplace=True), True), (operator.methodcaller('set_axis', ['A', 'B'], inplace=True), False), (operator.methodcaller('reset_index', inplace=True), True), (operator.methodcaller('rename', (lambda x: x), inplace=True), False)])
