...
import pytest
import pandas as pd
from pandas import Float64Index, Index, Int64Index, RangeIndex, Series, Timedelta, TimedeltaIndex, UInt64Index, array
...
def adjust_negative_zero(zero, expected):
    ...
    if np.signbit(np.array(zero)).any():
        ...
        expected *= (- 1)
    return expected
ser_or_index = [Series, Index]
lefts = [RangeIndex(10, 40, 10)]
lefts.extend([cls([10, 20, 30], dtype=dtype) for dtype in ['i1', 'i2', 'i4', 'i8', 'u1', 'u2', 'u4', 'u8', 'f2', 'f4', 'f8'] for cls in ser_or_index])
...
class TestNumericArraylikeArithmeticWithDatetimeLike():
    ...
    @pytest.mark.parametrize('other', [Timedelta(hours=31), Timedelta(hours=31).to_pytimedelta(), Timedelta(hours=31).to_timedelta64(), Timedelta(hours=31).to_timedelta64().astype('m8[h]'), np.timedelta64('NaT'), np.timedelta64('NaT', 'D'), pd.offsets.Minute(3), pd.offsets.Second(0)])
    def test_add_sub_timedeltalike_invalid(self, numeric_idx, other, box_with_array):
        box = box_with_array
        left = tm.box_expected(numeric_idx, box)
        msg = 'unsupported operand type|Addition/subtraction of integers and integer-arrays|Instead of adding/subtracting|cannot use operands with types dtype|Concatenation operation is not implemented for NumPy arrays'
        with pytest.raises(TypeError, match=msg):
            (left + other)
        with pytest.raises(TypeError, match=msg):
            (other + left)
    ...
    def test_add_sub_datetimelike_invalid(self, numeric_idx, other, box_with_array):
        box = box_with_array
        left = tm.box_expected(numeric_idx, box)
        msg = '|'.join(['unsupported operand type', 'Cannot (add|subtract) NaT (to|from) ndarray', 'Addition/subtraction of integers and integer-arrays', 'Concatenation operation is not implemented for NumPy arrays', 'operand type\\(s\\) all returned NotImplemented from __array_ufunc__'])
        with pytest.raises(TypeError, match=msg):
            (left + other)
        with pytest.raises(TypeError, match=msg):
            (other + left)
...
class TestAdditionSubtraction():
    ...
    def test_datetime64_with_index(self):
        ...
        tm.assert_series_equal(result, expected)
        ser = Series(pd.date_range('20130101', periods=5), index=pd.date_range('20130101', periods=5))
        expected = (ser - ser.index.to_series())
        result = (ser - ser.index)
        tm.assert_series_equal(result, expected)
...
class TestUFuncCompat():
    ...
    @pytest.mark.parametrize('holder', [Int64Index, UInt64Index, Float64Index, RangeIndex, Series])
...
class TestNumericArithmeticUnsorted():
    ...
    def test_binops(self):
        ...
        scalars = [(- 1), 1, 2]
        idxs = [RangeIndex(0, 10, 1), RangeIndex(0, 20, 2), RangeIndex((- 10), 10, 2), RangeIndex(5, (- 5), (- 1))]
    ...
    def test_numeric_compat2(self):
        ...
        cases_exact = [(RangeIndex(0, 1000, 2), 2, RangeIndex(0, 500, 1)), (RangeIndex((- 99), (- 201), (- 3)), (- 3), RangeIndex(33, 67, 1)), (RangeIndex(0, 1000, 1), 2, (RangeIndex(0, 1000, 1)._int64index // 2)), (RangeIndex(0, 100, 1), 2.0, (RangeIndex(0, 100, 1)._int64index // 2.0)), (RangeIndex(0), 50, RangeIndex(0)), (RangeIndex(2, 4, 2), 3, RangeIndex(0, 1, 1)), (RangeIndex((- 5), (- 10), (- 6)), 4, RangeIndex((- 2), (- 1), 1)), (RangeIndex((- 100), (- 200), 3), 2, RangeIndex(0))]
