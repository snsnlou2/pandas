...
class TestPeriodArrayLikeComparisons():
    ...
    @pytest.mark.parametrize('scalar', ['foo', Timestamp.now(), Timedelta(days=4), 9, 9.5])
...
class TestPeriodIndexComparisons():
    ...
    def test_eq_integer_disallowed(self, other):
        ...
        result = (idx == other)
        tm.assert_numpy_array_equal(result, expected)
        msg = '|'.join(["not supported between instances of 'Period' and 'int'", 'Invalid comparison between dtype=period\\[D\\] and '])
...
class TestPeriodSeriesComparisons():
    ...
    def test_cmp_series_period_series_mixed_freq(self):
        base = Series([Period('2011', freq='A'), Period('2011-02', freq='M'), Period('2013', freq='A'), Period('2011-04', freq='M')])
        ser = Series([Period('2012', freq='A'), Period('2011-01', freq='M'), Period('2013', freq='A'), Period('2011-05', freq='M')])
...
class TestPeriodIndexArithmetic():
    ...
    def test_parr_add_sub_float_raises(self, op, other, box_with_array):
        ...
        pi = dti.to_period('D')
        pi = tm.box_expected(pi, box_with_array)
        msg = 'unsupported operand type\\(s\\) for [+-]: .* and .*|Concatenation operation is not implemented for NumPy arrays'
        with pytest.raises(TypeError, match=msg):
            op(pi, other)

    @pytest.mark.parametrize('other', [Timestamp.now(), Timestamp.now().to_pydatetime(), Timestamp.now().to_datetime64(), pd.date_range('2016-01-01', periods=3, freq='H'), pd.date_range('2016-01-01', periods=3, tz='Europe/Brussels'), pd.date_range('2016-01-01', periods=3, freq='S')._data, pd.date_range('2016-01-01', periods=3, tz='Asia/Tokyo')._data])
    def test_parr_add_sub_invalid(self, other, box_with_array):
        rng = pd.period_range('1/1/2000', freq='D', periods=3)
        rng = tm.box_expected(rng, box_with_array)
        msg = '(:?cannot add PeriodArray and .*)|(:?cannot subtract .* from (:?a\\s)?.*)|(:?unsupported operand type\\(s\\) for \\+: .* and .*)'
        with pytest.raises(TypeError, match=msg):
            (rng + other)
        with pytest.raises(TypeError, match=msg):
            (other + rng)
    ...
    def test_pi_add_sub_td64_array_non_tick_raises(self):
        ...
        with pytest.raises(TypeError, match=msg):
            (rng - tdarr)
        msg = 'cannot subtract PeriodArray from timedelta64\\[ns\\]'
        with pytest.raises(TypeError, match=msg):
            (tdarr - rng)
    ...
    def test_pi_add_timedeltalike_minute_gt1(self, three_days):
        ...
        result = (rng - other)
        tm.assert_index_equal(result, expected)
        msg = "(:?bad operand type for unary -: 'PeriodArray')|(:?cannot subtract PeriodArray from timedelta64\\[[hD]\\])"
        with pytest.raises(TypeError, match=msg):
            (other - rng)
    ...
    def test_parr_add_sub_object_array(self):
        ...
        with tm.assert_produces_warning(PerformanceWarning):
            result = (parr + other)
        expected = PeriodIndex(['2001-01-01', '2001-01-03', '2001-01-05'], freq='D').array
        tm.assert_equal(result, expected)
        with tm.assert_produces_warning(PerformanceWarning):
            result = (parr - other)
...
class TestPeriodSeriesArithmetic():
    ...
    def test_ops_series_timedelta(self):
        ...
        expected = Series([Period('2015-01-02', freq='D'), Period('2015-01-03', freq='D')], name='xxx')
...
class TestPeriodIndexSeriesMethods():
    ...
    def test_parr_ops_errors(self, ng, func, box_with_array):
        ...
        msg = 'unsupported operand type\\(s\\)|can only concatenate|must be str|object to str implicitly'
        with pytest.raises(TypeError, match=msg):
            func(obj, ng)
