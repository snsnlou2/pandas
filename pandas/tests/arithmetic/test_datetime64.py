...
class TestDatetime64ArrayLikeComparisons():
    ...
    def test_compare_zerodim(self, tz_naive_fixture, box_with_array):
        tz = tz_naive_fixture
        box = box_with_array
        xbox = (box_with_array if (box_with_array not in [pd.Index, pd.array]) else np.ndarray)
...
class TestDatetimeIndexComparisons():
    ...
    def test_dti_cmp_nat_behaves_like_float_cmp_nan(self):
        ...
        didx1 = DatetimeIndex(['2014-01-01', pd.NaT, '2014-03-01', pd.NaT, '2014-05-01', '2014-07-01'])
        didx2 = DatetimeIndex(['2014-02-01', '2014-03-01', pd.NaT, pd.NaT, '2014-06-01', '2014-07-01'])
        darr = np.array([np_datetime64_compat('2014-02-01 00:00Z'), np_datetime64_compat('2014-03-01 00:00Z'), np_datetime64_compat('nat'), np.datetime64('nat'), np_datetime64_compat('2014-06-01 00:00Z'), np_datetime64_compat('2014-07-01 00:00Z')])
    ...
    def test_nat_comparison_tzawareness(self, op):
        dti = DatetimeIndex(['2014-01-01', pd.NaT, '2014-03-01', pd.NaT, '2014-05-01', '2014-07-01'])
...
class TestDatetime64Arithmetic():
    ...
    @pytest.mark.parametrize('ts', [Timestamp('2013-01-01'), Timestamp('2013-01-01').to_pydatetime(), Timestamp('2013-01-01').to_datetime64()])
    ...
    def test_dt64arr_addsub_time_objects_raises(self, box_with_array, tz_naive_fixture):
        ...
        with warnings.catch_warnings(record=True):
            ...
            msg = "unsupported operand type\\(s\\) for -: '(Timestamp|DatetimeArray)' and 'datetime.time'"
            with pytest.raises(TypeError, match=msg):
                (obj1 - obj2)
            msg = '|'.join(['cannot subtract DatetimeArray from ndarray', "ufunc (subtract|'subtract') cannot use operands with types dtype\\('O'\\) and dtype\\('<M8\\[ns\\]'\\)"])
            with pytest.raises(TypeError, match=msg):
                (obj2 - obj1)
            msg = "unsupported operand type\\(s\\) for \\+: '(Timestamp|DatetimeArray)' and 'datetime.time'"
            with pytest.raises(TypeError, match=msg):
                (obj1 + obj2)
            msg = '|'.join(["unsupported operand type\\(s\\) for \\+: '(Timestamp|DatetimeArray)' and 'datetime.time'", "ufunc (add|'add') cannot use operands with types dtype\\('O'\\) and dtype\\('<M8\\[ns\\]'\\)"])
            with pytest.raises(TypeError, match=msg):
                (obj2 + obj1)
...
class TestDatetime64DateOffsetArithmetic():
    ...
    def test_dt64arr_add_sub_DateOffset(self, box_with_array):
        ...
        exp = tm.box_expected(exp, box_with_array)
        tm.assert_equal(result, exp)
        s = DatetimeIndex([Timestamp('2000-01-15 00:15:00', tz='US/Central'), Timestamp('2000-02-15', tz='US/Central')], name='a')
        s = tm.box_expected(s, box_with_array)
        result = (s + pd.offsets.Day())
        result2 = (pd.offsets.Day() + s)
        exp = DatetimeIndex([Timestamp('2000-01-16 00:15:00', tz='US/Central'), Timestamp('2000-02-16', tz='US/Central')], name='a')
        exp = tm.box_expected(exp, box_with_array)
        tm.assert_equal(result, exp)
        tm.assert_equal(result2, exp)
        s = DatetimeIndex([Timestamp('2000-01-15 00:15:00', tz='US/Central'), Timestamp('2000-02-15', tz='US/Central')], name='a')
        ...
        exp = DatetimeIndex([Timestamp('2000-01-31 00:15:00', tz='US/Central'), Timestamp('2000-02-29', tz='US/Central')], name='a')
        exp = tm.box_expected(exp, box_with_array)
        tm.assert_equal(result, exp)
        tm.assert_equal(result2, exp)
...
def test_shift_months(years, months):
    dti = DatetimeIndex([Timestamp('2000-01-05 00:15:00'), Timestamp('2000-01-31 00:23:00'), Timestamp('2000-01-01'), Timestamp('2000-02-29'), Timestamp('2000-12-31')])
