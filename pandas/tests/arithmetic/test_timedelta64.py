...
import pandas as pd
from pandas import DataFrame, DatetimeIndex, NaT, Series, Timedelta, TimedeltaIndex, Timestamp, offsets, timedelta_range
...
class TestTimedelta64ArrayLikeComparisons():
    ...
    @pytest.mark.parametrize('invalid', [345600000000000, 'a', Timestamp.now(), Timestamp.now('UTC'), Timestamp.now().to_datetime64(), Timestamp.now().to_pydatetime(), Timestamp.now().date()])
...
class TestTimedelta64ArrayComparisons():
    ...
    def test_comparisons_nat(self):
        tdidx1 = TimedeltaIndex(['1 day', pd.NaT, '1 day 00:00:01', pd.NaT, '1 day 00:00:01', '5 day 00:00:03'])
        tdidx2 = TimedeltaIndex(['2 day', '2 day', pd.NaT, pd.NaT, '1 day 00:00:02', '5 days 00:00:03'])
...
class TestTimedeltaArraylikeAddSubOps():
    ...
    def test_td64arr_add_sub_object_array(self, box_with_array):
        ...
        tdarr = tm.box_expected(tdi, box)
        other = np.array([Timedelta(days=1), pd.offsets.Day(2), Timestamp('2000-01-04')])
        with tm.assert_produces_warning(PerformanceWarning):
            result = (tdarr + other)
        expected = pd.Index([Timedelta(days=2), Timedelta(days=4), Timestamp('2000-01-07')])
        expected = tm.box_expected(expected, xbox)
        tm.assert_equal(result, expected)
...
class TestTimedeltaArraylikeMulDivOps():
    ...
    def test_td64arr_mul_too_short_raises(self, box_with_array):
        idx = TimedeltaIndex(np.arange(5, dtype='int64'))
        idx = tm.box_expected(idx, box_with_array)
        msg = 'cannot use operands with types dtype|Cannot multiply with unequal lengths|Unable to coerce to Series'
        with pytest.raises(TypeError, match=msg):
            (idx * idx[:3])
        with pytest.raises(ValueError, match=msg):
            (idx * np.array([1, 2]))
    ...
    def test_td64arr_mul_int_series(self, box_with_array, names, request):
        ...
        tdi = tm.box_expected(tdi, box)
        xbox = (Series if ((box is pd.Index) or (box is tm.to_array) or (box is pd.array)) else box)
        expected = tm.box_expected(expected, xbox)
        result = (ser * tdi)
        tm.assert_equal(result, expected)
