...
from pandas import DataFrame, DatetimeIndex, MultiIndex, Series, Timedelta, Timestamp, date_range, period_range, to_datetime, to_timedelta
...
def test_invalid_constructor(frame_or_series, w):
    c = frame_or_series(range(5)).rolling
    msg = 'window must be an integer|passed window foo is not compatible with a datetimelike index'
    with pytest.raises(ValueError, match=msg):
        c(window=w)
...
def test_closed_fixed(closed, arithmetic_win_operators):
    ...
    expected = getattr(df_time.rolling('2D', closed=closed), func_name)().reset_index(drop=True)
    tm.assert_frame_equal(result, expected)
...
def test_closed_fixed_binary_col():
    ...
    result = rolling.mean()
    expected = DataFrame([np.nan, 0, 0.5, (2 / 3), 0.5, 0.4, 0.5, 0.428571], columns=['binary_col'], index=date_range(start='2020-01-01', freq='min', periods=len(data)))
    tm.assert_frame_equal(result, expected)
...
def test_rolling_window_as_string():
    ...
    expData = ((((((((((((((((((((((((((((((((((((([np.nan] * 2) + ([88.0] * 16)) + ([97.0] * 9)) + [98.0]) + ([99.0] * 21)) + ([95.0] * 16)) + ([93.0] * 5)) + ([89.0] * 5)) + ([96.0] * 21)) + ([94.0] * 14)) + ([90.0] * 13)) + ([88.0] * 2)) + ([90.0] * 9)) + ([96.0] * 21)) + ([95.0] * 6)) + [91.0]) + ([87.0] * 6)) + ([92.0] * 21)) + ([83.0] * 2)) + ([86.0] * 10)) + ([87.0] * 5)) + ([98.0] * 21)) + ([97.0] * 14)) + ([93.0] * 7)) + ([87.0] * 4)) + ([86.0] * 4)) + ([95.0] * 21)) + ([85.0] * 14)) + ([83.0] * 2)) + ([76.0] * 5)) + ([81.0] * 2)) + ([98.0] * 21)) + ([95.0] * 14)) + ([91.0] * 7)) + [86.0]) + ([93.0] * 3)) + ([95.0] * 20))
    expected = Series(expData, index=days.rename('DateCol')._with_freq(None), name='metric')
    tm.assert_series_equal(result, expected)
...
def test_iter_rolling_on_dataframe(expected, window):
    df = DataFrame({'A': [1, 2, 3, 4, 5], 'B': [4, 5, 6, 7, 8], 'C': date_range(start='2016-01-01', periods=5, freq='D')})
    expected = [DataFrame(values, index=index) for (values, index) in expected]
...
def test_rolling_numerical_accuracy_kahan_mean(add):
    df = DataFrame({'A': [(3002399751580331.0 + add), (- 0.0), (- 0.0)]}, index=[Timestamp('19700101 09:00:00'), Timestamp('19700101 09:00:03'), Timestamp('19700101 09:00:06')])
...
@pytest.mark.parametrize(('index', 'window'), [(period_range(start='2020-01-01 08:00', end='2020-01-01 08:08', freq='T'), '2T'), (period_range(start='2020-01-01 08:00', end='2020-01-01 12:00', freq='30T'), '1h')])
