...
@pytest.fixture(params=['sum', 'mean', 'median', 'max', 'min', 'var', 'std', 'kurt', 'skew', 'count'])
...
@pytest.fixture(params=[pytest.param('numba', marks=td.skip_if_no('numba', '0.46.0')), 'cython'])
def engine(request):
    'engine keyword argument for rolling.apply'
    return request.param

@pytest.fixture(params=[pytest.param(('numba', True), marks=td.skip_if_no('numba', '0.46.0')), ('cython', True), ('cython', False)])
...
def _create_consistency_data():
    ...
    def create_series():
        return [Series(dtype=object), Series([np.nan]), Series([np.nan, np.nan]), Series([3.0]), Series([np.nan, 3.0]), Series([3.0, np.nan]), Series([1.0, 3.0]), Series([2.0, 2.0]), Series([3.0, 1.0]), Series([5.0, 5.0, 5.0, 5.0, np.nan, np.nan, np.nan, 5.0, 5.0, np.nan, np.nan]), Series([np.nan, 5.0, 5.0, 5.0, np.nan, np.nan, np.nan, 5.0, 5.0, np.nan, np.nan]), Series([np.nan, np.nan, 5.0, 5.0, np.nan, np.nan, np.nan, 5.0, 5.0, np.nan, np.nan]), Series([np.nan, 3.0, np.nan, 3.0, 4.0, 5.0, 6.0, np.nan, np.nan, 7.0, 12.0, 13.0, 14.0, 15.0]), Series([np.nan, 5.0, np.nan, 2.0, 4.0, 0.0, 9.0, np.nan, np.nan, 3.0, 12.0, 13.0, 14.0, 15.0]), Series([2.0, 3.0, np.nan, 3.0, 4.0, 5.0, 6.0, np.nan, np.nan, 7.0, 12.0, 13.0, 14.0, 15.0]), Series([2.0, 5.0, np.nan, 2.0, 4.0, 0.0, 9.0, np.nan, np.nan, 3.0, 12.0, 13.0, 14.0, 15.0]), Series(range(10)), Series(range(20, 0, (- 2)))]

    def create_dataframes():
        return ([DataFrame(), DataFrame(columns=['a']), DataFrame(columns=['a', 'a']), DataFrame(columns=['a', 'b']), DataFrame(np.arange(10).reshape((5, 2))), DataFrame(np.arange(25).reshape((5, 5))), DataFrame(np.arange(25).reshape((5, 5)), columns=['a', 'b', 99, 'd', 'd'])] + [DataFrame(s) for s in create_series()])
...
def frame():
    ...
    return DataFrame(np.random.randn(100, 10), index=bdate_range(datetime(2009, 1, 1), periods=100), columns=np.arange(10))
...
@pytest.fixture(params=['object', 'category', 'int8', 'int16', 'int32', 'int64', 'uint8', 'uint16', 'uint32', 'uint64', 'float16', 'float32', 'float64', 'm8[ns]', 'M8[ns]', pytest.param('datetime64[ns, UTC]', marks=pytest.mark.skip('direct creation of extension dtype datetime64[ns, UTC] is not supported ATM'))])
def dtypes(request):
