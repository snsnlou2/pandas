...
class TestRolling():
    ...
    @pytest.mark.parametrize('f', ['sum', 'mean', 'min', 'max', pytest.param('count', marks=pytest.mark.filterwarnings('ignore:min_periods:FutureWarning')), 'kurt', 'skew'])
    ...
    def test_groupby_rolling_center_on(self):
        ...
        expected = Series([1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 7.0, 7.5, 7.5, 7.5], name='value', index=MultiIndex.from_tuples((('group_1', Timestamp('2020-01-01')), ('group_1', Timestamp('2020-01-02')), ('group_1', Timestamp('2020-01-03')), ('group_1', Timestamp('2020-01-04')), ('group_1', Timestamp('2020-01-05')), ('group_1', Timestamp('2020-01-06')), ('group_2', Timestamp('2020-01-07')), ('group_2', Timestamp('2020-01-08')), ('group_2', Timestamp('2020-01-09')), ('group_2', Timestamp('2020-01-10'))), names=['gb', 'Date']))
        tm.assert_series_equal(result, expected)
    ...
    def test_groupby_rolling_subset_with_closed(self):
        df = DataFrame({'column1': range(6), 'column2': range(6), 'group': (3 * ['A', 'B']), 'date': ([Timestamp('2019-01-01')] * 6)})
        result = df.groupby('group').rolling('1D', on='date', closed='left')['column1'].sum()
        expected = Series([np.nan, 0.0, 2.0, np.nan, 1.0, 4.0], index=MultiIndex.from_tuples((([('A', Timestamp('2019-01-01'))] * 3) + ([('B', Timestamp('2019-01-01'))] * 3)), names=['group', 'date']), name='column1')
        tm.assert_series_equal(result, expected)
    ...
    def test_groupby_rolling_count_closed_on(self):
        df = DataFrame({'column1': range(6), 'column2': range(6), 'group': (3 * ['A', 'B']), 'date': date_range(end='20190101', periods=6)})
        result = df.groupby('group').rolling('3d', on='date', closed='left')['column1'].count()
        expected = Series([np.nan, 1.0, 1.0, np.nan, 1.0, 1.0], name='column1', index=MultiIndex.from_tuples([('A', Timestamp('2018-12-27')), ('A', Timestamp('2018-12-29')), ('A', Timestamp('2018-12-31')), ('B', Timestamp('2018-12-28')), ('B', Timestamp('2018-12-30')), ('B', Timestamp('2019-01-01'))], names=['group', 'date']))
        tm.assert_series_equal(result, expected)
