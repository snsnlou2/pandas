...
def test_min_periods(series, minp, roll_func, kwargs):
    result = getattr(series.rolling((len(series) + 1), min_periods=minp), roll_func)(**kwargs)
    expected = getattr(series.rolling(len(series), min_periods=minp), roll_func)(**kwargs)
...
def test_center(roll_func, kwargs, minp):
    ...
    expected = getattr(concat([obj, Series(([np.NaN] * 9))]).rolling(20, min_periods=minp), roll_func)(**kwargs)[9:].reset_index(drop=True)
