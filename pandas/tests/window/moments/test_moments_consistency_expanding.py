...
def test_expanding_func(func, static_comp, frame_or_series):
    ...
    if (frame_or_series is Series):
        ...
        tm.assert_series_equal(result.iloc[10], static_comp(data[:11]), check_names=False)
...
def test_expanding_consistency_mean(consistency_data, min_periods):
    ...
    expected = (x.expanding(min_periods=min_periods).sum() / x.expanding(min_periods=min_periods).count())
...
def test_expanding_consistency_var_debiasing_factors(consistency_data, min_periods):
    ...
    var_debiasing_factors_x = (x.expanding().count() / (x.expanding().count() - 1.0).replace(0.0, np.nan))
