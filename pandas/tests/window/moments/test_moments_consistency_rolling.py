...
def test_rolling_apply_consistency_sum_nans(consistency_data, window, min_periods, center, f):
    (x, is_constant, no_nans) = consistency_data
    if ((f is np.nansum) and (min_periods == 0)):
        pass
    else:
        rolling_f_result = x.rolling(window=window, min_periods=min_periods, center=center).sum()
...
def test_rolling_apply_consistency_sum_no_nans(consistency_data, window, min_periods, center, f):
    (x, is_constant, no_nans) = consistency_data
    if no_nans:
        if ((f is np.nansum) and (min_periods == 0)):
            pass
...
@pytest.mark.parametrize('f', [(lambda x: x.rolling(window=10, min_periods=5).cov(x, pairwise=False)), (lambda x: x.rolling(window=10, min_periods=5).corr(x, pairwise=False)), (lambda x: x.rolling(window=10, min_periods=5).max()), (lambda x: x.rolling(window=10, min_periods=5).min()), (lambda x: x.rolling(window=10, min_periods=5).sum()), (lambda x: x.rolling(window=10, min_periods=5).mean()), (lambda x: x.rolling(window=10, min_periods=5).std()), (lambda x: x.rolling(window=10, min_periods=5).var()), (lambda x: x.rolling(window=10, min_periods=5).skew()), (lambda x: x.rolling(window=10, min_periods=5).kurt()), (lambda x: x.rolling(window=10, min_periods=5).quantile(quantile=0.5)), (lambda x: x.rolling(window=10, min_periods=5).median()), (lambda x: x.rolling(window=10, min_periods=5).apply(sum, raw=False)), (lambda x: x.rolling(window=10, min_periods=5).apply(sum, raw=True)), pytest.param((lambda x: x.rolling(win_type='boxcar', window=10, min_periods=5).mean()), marks=td.skip_if_no_scipy)])
...
def test_rolling_max_gh6297():
    ...
    expected = Series([1.0, 2.0, 6.0, 4.0, 5.0], index=DatetimeIndex([datetime(1975, 1, i, 0) for i in range(1, 6)], freq='D'))
...
def test_rolling_max_resample():
    ...
    series = series.map((lambda x: float(x)))
    series = series.sort_index()
    expected = Series([0.0, 1.0, 2.0, 3.0, 20.0], index=DatetimeIndex([datetime(1975, 1, i, 0) for i in range(1, 6)], freq='D'))
    x = series.resample('D').max().rolling(window=1).max()
    tm.assert_series_equal(expected, x)
    expected = Series([0.0, 1.0, 2.0, 3.0, 10.0], index=DatetimeIndex([datetime(1975, 1, i, 0) for i in range(1, 6)], freq='D'))
    x = series.resample('D').median().rolling(window=1).max()
    tm.assert_series_equal(expected, x)
    v = (((4.0 + 10.0) + 20.0) / 3.0)
    expected = Series([0.0, 1.0, 2.0, 3.0, v], index=DatetimeIndex([datetime(1975, 1, i, 0) for i in range(1, 6)], freq='D'))
...
def test_rolling_min_resample():
    ...
    series = series.map((lambda x: float(x)))
    series = series.sort_index()
    expected = Series([0.0, 1.0, 2.0, 3.0, 4.0], index=DatetimeIndex([datetime(1975, 1, i, 0) for i in range(1, 6)], freq='D'))
...
def test_moment_functions_zero_length_pairwise(f):
    ...
    df2['a'] = df2['a'].astype('float64')
    df1_expected = DataFrame(index=MultiIndex.from_product([df1.index, df1.columns]), columns=Index([]))
...
def test_moments_consistency_var(consistency_data, window, min_periods, center, ddof):
    ...
    var_x = x.rolling(window=window, min_periods=min_periods, center=center).var(ddof=ddof)
    assert (not (var_x < 0).any().any())
    if (ddof == 0):
        mean_x2 = (x * x).rolling(window=window, min_periods=min_periods, center=center).mean()
        tm.assert_equal(var_x, (mean_x2 - (mean_x * mean_x)))

@pytest.mark.parametrize('window,min_periods,center', list(_rolling_consistency_cases()))
@pytest.mark.parametrize('ddof', [0, 1])
def test_moments_consistency_var_constant(consistency_data, window, min_periods, center, ddof):
    (x, is_constant, no_nans) = consistency_data
    if is_constant:
        count_x = x.rolling(window=window, min_periods=min_periods, center=center).count()
        var_x = x.rolling(window=window, min_periods=min_periods, center=center).var(ddof=ddof)
        assert (not (var_x > 0).any().any())
        expected = (x * np.nan)
...
def test_rolling_consistency_std(consistency_data, window, min_periods, center, ddof):
    (x, is_constant, no_nans) = consistency_data
    var_x = x.rolling(window=window, min_periods=min_periods, center=center).var(ddof=ddof)
    std_x = x.rolling(window=window, min_periods=min_periods, center=center).std(ddof=ddof)
    assert (not (var_x < 0).any().any())
...
def test_rolling_consistency_cov(consistency_data, window, min_periods, center, ddof):
    ...
    var_x = x.rolling(window=window, min_periods=min_periods, center=center).var(ddof=ddof)
    assert (not (var_x < 0).any().any())
    cov_x_x = x.rolling(window=window, min_periods=min_periods, center=center).cov(x, ddof=ddof)
    assert (not (cov_x_x < 0).any().any())
    tm.assert_equal(var_x, cov_x_x)

@pytest.mark.parametrize('window,min_periods,center', list(_rolling_consistency_cases()))
@pytest.mark.parametrize('ddof', [0, 1])
def test_rolling_consistency_series_cov_corr(consistency_data, window, min_periods, center, ddof):
    (x, is_constant, no_nans) = consistency_data
    if isinstance(x, Series):
        var_x_plus_y = (x + x).rolling(window=window, min_periods=min_periods, center=center).var(ddof=ddof)
        var_x = x.rolling(window=window, min_periods=min_periods, center=center).var(ddof=ddof)
        var_y = x.rolling(window=window, min_periods=min_periods, center=center).var(ddof=ddof)
        cov_x_y = x.rolling(window=window, min_periods=min_periods, center=center).cov(x, ddof=ddof)
        tm.assert_equal(cov_x_y, (0.5 * ((var_x_plus_y - var_x) - var_y)))
        corr_x_y = x.rolling(window=window, min_periods=min_periods, center=center).corr(x)
        std_x = x.rolling(window=window, min_periods=min_periods, center=center).std(ddof=ddof)
        std_y = x.rolling(window=window, min_periods=min_periods, center=center).std(ddof=ddof)
        tm.assert_equal(corr_x_y, (cov_x_y / (std_x * std_y)))
        if (ddof == 0):
            mean_x = x.rolling(window=window, min_periods=min_periods, center=center).mean()
            mean_y = x.rolling(window=window, min_periods=min_periods, center=center).mean()
            mean_x_times_y = (x * x).rolling(window=window, min_periods=min_periods, center=center).mean()
...
def test_rolling_consistency_mean(consistency_data, window, min_periods, center):
    ...
    expected = x.rolling(window=window, min_periods=min_periods, center=center).sum().divide(x.rolling(window=window, min_periods=min_periods, center=center).count())
...
def test_rolling_consistency_constant(consistency_data, window, min_periods, center):
    ...
    if is_constant:
        count_x = x.rolling(window=window, min_periods=min_periods, center=center).count()
        mean_x = x.rolling(window=window, min_periods=min_periods, center=center).mean()
        corr_x_x = x.rolling(window=window, min_periods=min_periods, center=center).corr(x)
...
@pytest.mark.parametrize('window,min_periods,center', list(_rolling_consistency_cases()))
def test_rolling_consistency_var_debiasing_factors(consistency_data, window, min_periods, center):
    (x, is_constant, no_nans) = consistency_data
    var_unbiased_x = x.rolling(window=window, min_periods=min_periods, center=center).var()
    var_biased_x = x.rolling(window=window, min_periods=min_periods, center=center).var(ddof=0)
    var_debiasing_factors_x = x.rolling(window=window, min_periods=min_periods, center=center).count().divide((x.rolling(window=window, min_periods=min_periods, center=center).count() - 1.0).replace(0.0, np.nan))
