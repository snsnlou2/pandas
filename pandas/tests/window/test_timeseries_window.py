...
class TestRollingTS():
    ...
    def test_frame_on2(self):
        df = DataFrame({'A': [0, 1, 2, 3, 4], 'B': [0, 1, 2, np.nan, 4], 'C': Index([Timestamp('20130101 09:00:00'), Timestamp('20130101 09:00:02'), Timestamp('20130101 09:00:03'), Timestamp('20130101 09:00:05'), Timestamp('20130101 09:00:06')])}, columns=['A', 'C', 'B'])
    ...
    @pytest.mark.parametrize('f', ['sum', 'mean', pytest.param('count', marks=pytest.mark.filterwarnings('ignore:min_periods:FutureWarning')), 'median', 'std', 'var', 'kurt', 'skew', 'min', 'max'])
    def test_all(self, f):
        df = (self.regular * 2)
    ...
    def test_rolling_on_decreasing_index(self):
        index = [Timestamp('20190101 09:00:30'), Timestamp('20190101 09:00:27'), Timestamp('20190101 09:00:20'), Timestamp('20190101 09:00:18'), Timestamp('20190101 09:00:10')]
