...
import pandas as pd
from pandas import CategoricalIndex, DataFrame, DatetimeIndex, Float64Index, Int64Index, IntervalIndex, PeriodIndex, RangeIndex, Series, TimedeltaIndex, UInt64Index, date_range, isna, period_range
...
class TestIndex(Base):
    ...
    @pytest.mark.parametrize('index', [date_range('2015-01-01 10:00', freq='D', periods=3, tz='US/Eastern', name='Green Eggs & Ham'), date_range('2015-01-01 10:00', freq='D', periods=3), pd.timedelta_range('1 days', freq='D', periods=3), period_range('2015-01-01', freq='D', periods=3)])
    ...
    @pytest.mark.parametrize('index,has_tz', [(date_range('2015-01-01 10:00', freq='D', periods=3, tz='US/Eastern'), True), (pd.timedelta_range('1 days', freq='D', periods=3), False), (period_range('2015-01-01', freq='D', periods=3), False)])
    ...
    @pytest.mark.parametrize('klass,dtype,na_val', [(Float64Index, np.float64, np.nan), (DatetimeIndex, 'datetime64[ns]', pd.NaT)])
    ...
    @pytest.mark.parametrize('empty,klass', [(PeriodIndex([], freq='B'), PeriodIndex), (PeriodIndex(iter([]), freq='B'), PeriodIndex), (PeriodIndex((_ for _ in []), freq='B'), PeriodIndex), (RangeIndex(step=1), RangeIndex), (MultiIndex(levels=[[1, 2], ['blue', 'red']], codes=[[], []]), MultiIndex)])
    ...
    @pytest.mark.parametrize('idx_class', [Int64Index, RangeIndex, Float64Index, UInt64Index])
    ...
    @pytest.mark.parametrize('index', ['unicode', 'string', 'datetime', 'int', 'uint', 'float'], indirect=True)
    ...
    @pytest.mark.parametrize('labels,dtype', [(pd.Int64Index([]), np.int64), (Float64Index([]), np.float64), (DatetimeIndex([]), np.datetime64)])
    ...
    def test_reindex_no_type_preserve_target_empty_mi(self):
        index = Index(list('abc'))
        result = index.reindex(MultiIndex([pd.Int64Index([]), Float64Index([])], [[], []]))[0]
...
class TestMixedIntIndex(Base):
    ...
    @pytest.mark.parametrize('how', ['any', 'all'])
    @pytest.mark.parametrize('index,expected', [(DatetimeIndex(['2011-01-01', '2011-01-02', '2011-01-03']), DatetimeIndex(['2011-01-01', '2011-01-02', '2011-01-03'])), (DatetimeIndex(['2011-01-01', '2011-01-02', '2011-01-03', pd.NaT]), DatetimeIndex(['2011-01-01', '2011-01-02', '2011-01-03'])), (pd.TimedeltaIndex(['1 days', '2 days', '3 days']), pd.TimedeltaIndex(['1 days', '2 days', '3 days'])), (pd.TimedeltaIndex([pd.NaT, '1 days', '2 days', '3 days', pd.NaT]), pd.TimedeltaIndex(['1 days', '2 days', '3 days'])), (PeriodIndex(['2012-02', '2012-04', '2012-05'], freq='M'), PeriodIndex(['2012-02', '2012-04', '2012-05'], freq='M')), (PeriodIndex(['2012-02', '2012-04', 'NaT', '2012-05'], freq='M'), PeriodIndex(['2012-02', '2012-04', '2012-05'], freq='M'))])
...
def test_convert_almost_null_slice(index):
    ...
    if isinstance(index, pd.IntervalIndex):
        ...
        with pytest.raises(TypeError, match=msg):
            index._convert_slice_indexer(key, 'loc')
dtlike_dtypes = [np.dtype('timedelta64[ns]'), np.dtype('datetime64[ns]'), pd.DatetimeTZDtype('ns', 'Asia/Tokyo'), pd.PeriodDtype('ns')]
