...
import pandas as pd
from pandas import CategoricalIndex, DatetimeIndex, Float64Index, Int64Index, MultiIndex, RangeIndex, Series, TimedeltaIndex, UInt64Index
import pandas._testing as tm
from pandas.api.types import is_datetime64tz_dtype, pandas_dtype
COMPATIBLE_INCONSISTENT_PAIRS = {(Int64Index, RangeIndex): (tm.makeIntIndex, tm.makeRangeIndex), (Float64Index, Int64Index): (tm.makeFloatIndex, tm.makeIntIndex), (Float64Index, RangeIndex): (tm.makeFloatIndex, tm.makeIntIndex), (Float64Index, UInt64Index): (tm.makeFloatIndex, tm.makeUIntIndex)}
    ...
    def test_union_base(self, index):
        ...
        for case in cases:
            if (not isinstance(index, CategoricalIndex)):
                ...
                assert tm.equalContents(result, everything), (result, everything, type(case))
        if isinstance(index, MultiIndex):
            msg = 'other must be a MultiIndex or a list of tuples'
            with pytest.raises(TypeError, match=msg):
                first.union([1, 2, 3])
