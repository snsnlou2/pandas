...
from pandas import CategoricalIndex, Index, IntervalIndex, NaT, Timedelta, Timestamp, interval_range
...
class TestFloatSubtype(AstypeTests):
    ...
    def test_subtype_integer_errors(self):
        ...
        dtype = IntervalDtype('uint64')
        msg = re.escape('Cannot convert interval[float64] to interval[uint64]; subtypes are incompatible')
        with pytest.raises(TypeError, match=msg):
            index.astype(dtype)
