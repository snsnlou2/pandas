...
import numpy as np
import pytest
from pandas import DataFrame, Float64Index, Interval, IntervalIndex, Series, Timedelta, Timestamp
...
class TestIntervalIndexRendering():
    ...
    def test_repr_floats(self):
        markers = Series(['foo', 'bar'], index=IntervalIndex([Interval(left, right) for (left, right) in zip(Float64Index([329.973, 345.137], dtype='float64'), Float64Index([345.137, 360.191], dtype='float64'))]))
        result = str(markers)
