...
class TestIntervalIndex():
    ...
    @pytest.mark.parametrize('breaks', [[1, 1, 2, 5, 15, 53, 217, 1014, 5335, 31240, 201608], [(- np.inf), (- 100), (- 10), 0.5, 1, 1.5, 3.8, 101, 202, np.inf], pd.to_datetime(['20170101', '20170202', '20170303', '20170404']), pd.to_timedelta(['1ns', '2ms', '3s', '4min', '5H', '6D'])])
    ...
    def test_comparison(self):
        ...
        tm.assert_numpy_array_equal(actual, np.array([False, False]))
        msg = '|'.join(["not supported between instances of 'int' and '.*.Interval'", 'Invalid comparison between dtype=interval\\[int64\\] and '])
        with pytest.raises(TypeError, match=msg):
            (self.index > 0)
        with pytest.raises(TypeError, match=msg):
            (self.index <= 0)
