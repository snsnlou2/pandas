...
import pytest
import pandas as pd
from pandas import Float64Index, Index, Int64Index, NaT, Timedelta, TimedeltaIndex, timedelta_range
...
class TestTimedeltaIndex():
    ...
    def test_astype_object(self):
        ...
        expected_list = [Timedelta('1 days'), Timedelta('2 days'), Timedelta('3 days'), Timedelta('4 days')]
    ...
    def test_astype(self):
        idx = TimedeltaIndex([100000000000000.0, 'NaT', NaT, np.NaN], name='idx')
        result = idx.astype(object)
        expected = Index(([Timedelta('1 days 03:46:40')] + ([NaT] * 3)), dtype=object, name='idx')
        tm.assert_index_equal(result, expected)
        with tm.assert_produces_warning(FutureWarning, check_stacklevel=False):
            result = idx.astype(int)
        expected = Int64Index(([100000000000000] + ([(- 9223372036854775808)] * 3)), dtype=np.int64, name='idx')
        tm.assert_index_equal(result, expected)
