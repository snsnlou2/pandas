...
def test_dropna():
    idx = MultiIndex.from_arrays([[1, np.nan, 3, np.nan, 5], [1, 2, np.nan, np.nan, 5], ['a', 'b', 'c', np.nan, 'e']])
    ...
    with pytest.raises(ValueError, match=msg):
        idx.dropna(how='xxx')
    idx = MultiIndex(levels=[[np.nan, None, pd.NaT, '128', 2], [np.nan, None, pd.NaT, '128', 2]], codes=[[0, (- 1), 1, 2, 3, 4], [0, (- 1), 3, 3, 3, 4]])
    ...
    expected = MultiIndex.from_arrays([[np.nan, np.nan, '128', 2], ['128', '128', '128', 2]])
