...
class TestDatetimeEngine():
    ...
    @pytest.mark.parametrize('scalar', [pd.Timedelta(pd.Timestamp('2016-01-01').asm8.view('m8[ns]')), pd.Timestamp('2016-01-01').value, pd.Timestamp('2016-01-01').to_pydatetime(), pd.Timestamp('2016-01-01').to_datetime64()])
...
class TestTimedeltaEngine():
    ...
    @pytest.mark.parametrize('scalar', [pd.Timestamp(pd.Timedelta(days=42).asm8.view('datetime64[ns]')), pd.Timedelta(days=42).value, pd.Timedelta(days=42).to_pytimedelta(), pd.Timedelta(days=42).to_timedelta64()])
    def test_not_contains_requires_timestamp(self, scalar):
