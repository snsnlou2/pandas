...
import pandas as pd
from pandas import DatetimeIndex, NaT, Period, PeriodIndex, Series, Timedelta, date_range, notna, period_range
...
class TestGetItem():
    ...
    def test_getitem(self):
        ...
        for idx in [idx1]:
            ...
            result = idx[0:10:2]
            expected = PeriodIndex(['2011-01-01', '2011-01-03', '2011-01-05', '2011-01-07', '2011-01-09'], freq='D', name='idx')
            tm.assert_index_equal(result, expected)
            assert (result.freq == expected.freq)
            assert (result.freq == 'D')
            result = idx[(- 20):(- 5):3]
            expected = PeriodIndex(['2011-01-12', '2011-01-15', '2011-01-18', '2011-01-21', '2011-01-24'], freq='D', name='idx')
            tm.assert_index_equal(result, expected)
            assert (result.freq == expected.freq)
            assert (result.freq == 'D')
            result = idx[4::(- 1)]
            expected = PeriodIndex(['2011-01-05', '2011-01-04', '2011-01-03', '2011-01-02', '2011-01-01'], freq='D', name='idx')
            tm.assert_index_equal(result, expected)
            assert (result.freq == expected.freq)
            assert (result.freq == 'D')
...
class TestGetLoc():
    ...
    def test_get_loc2(self):
        ...
        assert (idx.get_loc('2000-01-02T12', method='nearest', tolerance=Timedelta('1D')) == 1)
        ...
        with pytest.raises(ValueError, match='list-like tolerance size must match target index size'):
            idx.get_loc('2000-01-10', method='nearest', tolerance=[Timedelta('1 day').to_timedelta64(), Timedelta('1 day').to_timedelta64()])
...
class TestTake():
    ...
    def test_take_misc(self):
        ...
        expected = PeriodIndex([datetime(2010, 1, 6), datetime(2010, 1, 7), datetime(2010, 1, 9), datetime(2010, 1, 13)], freq='D', name='idx')
