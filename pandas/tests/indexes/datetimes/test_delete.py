...
class TestDelete():
    ...
    def test_delete_slice(self):
        ...
        expected_3_5 = DatetimeIndex(['2000-01-01', '2000-01-02', '2000-01-03', '2000-01-07', '2000-01-08', '2000-01-09', '2000-01-10'], freq=None, name='idx')
        ...
        for (n, expected) in cases.items():
            ...
            assert (result.freq == expected.freq)
        for tz in [None, 'Asia/Tokyo', 'US/Pacific']:
            ts = Series(1, index=date_range('2000-01-01 09:00', periods=10, freq='H', name='idx', tz=tz))
            ...
            expected = DatetimeIndex(['2000-01-01 09:00', '2000-01-01 11:00', '2000-01-01 13:00', '2000-01-01 15:00', '2000-01-01 17:00'], freq=None, name='idx', tz=tz)
            tm.assert_index_equal(result, expected)
            assert (result.name == expected.name)
            assert (result.freq == expected.freq)
            assert (result.tz == expected.tz)
