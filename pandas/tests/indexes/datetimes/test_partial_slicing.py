...
import numpy as np
import pytest
from pandas import DataFrame, DatetimeIndex, Index, Series, Timedelta, Timestamp, date_range
...
class TestSlicing():
    ...
    def test_partial_slicing_with_multiindex(self):
        ...
        s = DataFrame(np.random.rand(1000, 1000), index=date_range('2000-1-1', periods=1000)).stack()
        s2 = s[:(- 1)].copy()
    ...
    def test_selection_by_datetimelike(self, datetimelike, op, expected):
        df = DataFrame({'A': [Timestamp('20120101'), Timestamp('20130101'), np.nan, Timestamp('20130103')]})
