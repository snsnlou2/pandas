...
class TestGetItem():
    ...
    def test_ellipsis(self):
        idx = date_range('2011-01-01', '2011-01-31', freq='D', tz='Asia/Tokyo', name='idx')
        result = idx[...]
    ...
    def test_getitem(self):
        ...
        for idx in [idx1, idx2]:
            ...
            assert (result.freq == expected.freq)
            result = idx[4::(- 1)]
            expected = DatetimeIndex(['2011-01-05', '2011-01-04', '2011-01-03', '2011-01-02', '2011-01-01'], freq='-1D', tz=idx.tz, name='idx')
            tm.assert_index_equal(result, expected)
            assert (result.freq == expected.freq)
...
class TestGetLoc():
    ...
    def test_get_loc(self):
        ...
        assert (idx.get_loc('2000-01-01T12', method='nearest', tolerance=pd.Timedelta('1D')) == 1)
        assert (idx.get_loc('2000-01-01T12', method='nearest', tolerance=np.timedelta64(1, 'D')) == 1)
        assert (idx.get_loc('2000-01-01T12', method='nearest', tolerance=timedelta(1)) == 1)
        ...
        with pytest.raises(ValueError, match='tolerance size must match target index size'):
            idx.get_loc('2000-01-01', method='nearest', tolerance=[pd.Timedelta('1day').to_timedelta64(), pd.Timedelta('1day').to_timedelta64()])
        ...
        with pytest.raises(KeyError, match="'foobar'"):
            idx.get_loc('foobar')
        with pytest.raises(InvalidIndexError, match='slice\\(None, 2, None\\)'):
            idx.get_loc(slice(2))
        ...
        tm.assert_numpy_array_equal(idx.get_loc(time(12)), np.array([12]), check_dtype=False)
        tm.assert_numpy_array_equal(idx.get_loc(time(12, 30)), np.array([]), check_dtype=False)
    ...
    def test_get_loc_tz_aware(self):
        dti = date_range(Timestamp('2019-12-12 00:00:00', tz='US/Eastern'), Timestamp('2019-12-13 00:00:00', tz='US/Eastern'), freq='5s')
...
class TestGetIndexer():
    ...
    def test_get_indexer(self):
        ...
        tm.assert_numpy_array_equal(idx.get_indexer(target, 'nearest', tolerance=pd.Timedelta('1 hour')), np.array([0, (- 1), 1], dtype=np.intp))
        tol_raw = [pd.Timedelta('1 hour'), pd.Timedelta('1 hour'), pd.Timedelta('1 hour').to_timedelta64()]
        ...
        tol_bad = [pd.Timedelta('2 hour').to_timedelta64(), pd.Timedelta('1 hour').to_timedelta64(), 'foo']
    ...
    @pytest.mark.filterwarnings('ignore:Comparison of Timestamp with datetime.date:FutureWarning')
...
class TestGetSliceBounds():
    ...
    def test_get_slice_bounds_datetime_within(self, box, kind, side, expected, tz_aware_fixture):
        tz = tz_aware_fixture
    ...
    def test_get_slice_bounds_datetime_outside(self, box, kind, side, year, expected, tz_aware_fixture):
        tz = tz_aware_fixture
        index = bdate_range('2000-01-03', '2000-02-11').tz_localize(tz)
