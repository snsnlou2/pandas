...
class TestGetLoc():
    ...
    def test_get_loc_missing_nan(self):
        ...
        with pytest.raises(KeyError, match='^nan$'):
            idx.get_loc(np.nan)
        with pytest.raises(TypeError, match="'\\[nan\\]' is an invalid key"):
            idx.get_loc([np.nan])
...
class TestWhere():
    ...
    @pytest.mark.parametrize('index', [Float64Index(np.arange(5, dtype='float64')), Int64Index(range(0, 20, 2)), UInt64Index(np.arange(5, dtype='uint64'))])
