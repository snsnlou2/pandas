...
from pandas.core.dtypes.dtypes import CategoricalDtype, CategoricalDtypeType, DatetimeTZDtype, IntervalDtype, PeriodDtype
...
class TestPandasDtype():
    ...
    def test_period_dtype(self, dtype):
        ...
        assert (com.pandas_dtype(dtype) == dtype)
dtypes = {'datetime_tz': com.pandas_dtype('datetime64[ns, US/Eastern]'), 'datetime': com.pandas_dtype('datetime64[ns]'), 'timedelta': com.pandas_dtype('timedelta64[ns]'), 'period': PeriodDtype('D'), 'integer': np.dtype(np.int64), 'float': np.dtype(np.float64), 'object': np.dtype(object), 'category': com.pandas_dtype('category')}
...
@pytest.mark.parametrize('dtype', (((((integer_dtypes + [pd.Series([1, 2])]) + tm.ALL_INT_DTYPES) + to_numpy_dtypes(tm.ALL_INT_DTYPES)) + tm.ALL_EA_INT_DTYPES) + to_ea_dtypes(tm.ALL_EA_INT_DTYPES)))
def test_is_integer_dtype(dtype):
    assert com.is_integer_dtype(dtype)

@pytest.mark.parametrize('dtype', [str, float, np.datetime64, np.timedelta64, pd.Index([1, 2.0]), np.array(['a', 'b']), np.array([], dtype=np.timedelta64)])
def test_is_not_integer_dtype(dtype):
    assert (not com.is_integer_dtype(dtype))
signed_integer_dtypes = []

@pytest.mark.parametrize('dtype', (((((signed_integer_dtypes + [pd.Series([1, 2])]) + tm.SIGNED_INT_DTYPES) + to_numpy_dtypes(tm.SIGNED_INT_DTYPES)) + tm.SIGNED_EA_INT_DTYPES) + to_ea_dtypes(tm.SIGNED_EA_INT_DTYPES)))
def test_is_signed_integer_dtype(dtype):
    assert com.is_integer_dtype(dtype)

@pytest.mark.parametrize('dtype', (((([str, float, np.datetime64, np.timedelta64, pd.Index([1, 2.0]), np.array(['a', 'b']), np.array([], dtype=np.timedelta64)] + tm.UNSIGNED_INT_DTYPES) + to_numpy_dtypes(tm.UNSIGNED_INT_DTYPES)) + tm.UNSIGNED_EA_INT_DTYPES) + to_ea_dtypes(tm.UNSIGNED_EA_INT_DTYPES)))
def test_is_not_signed_integer_dtype(dtype):
    assert (not com.is_signed_integer_dtype(dtype))
unsigned_integer_dtypes = []

@pytest.mark.parametrize('dtype', (((((unsigned_integer_dtypes + [pd.Series([1, 2], dtype=np.uint32)]) + tm.UNSIGNED_INT_DTYPES) + to_numpy_dtypes(tm.UNSIGNED_INT_DTYPES)) + tm.UNSIGNED_EA_INT_DTYPES) + to_ea_dtypes(tm.UNSIGNED_EA_INT_DTYPES)))
def test_is_unsigned_integer_dtype(dtype):
    assert com.is_unsigned_integer_dtype(dtype)

@pytest.mark.parametrize('dtype', (((([str, float, np.datetime64, np.timedelta64, pd.Index([1, 2.0]), np.array(['a', 'b']), np.array([], dtype=np.timedelta64)] + tm.SIGNED_INT_DTYPES) + to_numpy_dtypes(tm.SIGNED_INT_DTYPES)) + tm.SIGNED_EA_INT_DTYPES) + to_ea_dtypes(tm.SIGNED_EA_INT_DTYPES)))
...
@pytest.mark.parametrize('to_type', [np.uint8, np.uint16, np.uint32, np.int8, np.int16, np.int32, np.float16, np.float32])
