...
class TestIsNA():
    ...
    def test_timedelta_other_units(self):
        ...
        tm.assert_numpy_array_equal(isna(idx.values), exp)
        tm.assert_numpy_array_equal(notna(idx.values), (~ exp))
        for dtype in ['timedelta64[D]', 'timedelta64[h]', 'timedelta64[m]', 'timedelta64[s]', 'timedelta64[ms]', 'timedelta64[us]', 'timedelta64[ns]']:
            values = idx.values.astype(dtype)
            exp = np.array([False, True, False])
            tm.assert_numpy_array_equal(isna(values), exp)
            tm.assert_numpy_array_equal(notna(values), (~ exp))
...
def test_array_equivalent(dtype_equal):
    ...
    assert array_equivalent(Float64Index([0, np.nan]), Float64Index([0, np.nan]), dtype_equal=dtype_equal)
    assert (not array_equivalent(Float64Index([0, np.nan]), Float64Index([1, np.nan]), dtype_equal=dtype_equal))
    ...
    assert array_equivalent(DatetimeIndex([0, np.nan], tz='US/Eastern'), DatetimeIndex([0, np.nan], tz='US/Eastern'), dtype_equal=dtype_equal)
    assert (not array_equivalent(DatetimeIndex([0, np.nan], tz='US/Eastern'), DatetimeIndex([1, np.nan], tz='US/Eastern'), dtype_equal=dtype_equal))
    assert (not array_equivalent(DatetimeIndex([0, np.nan]), DatetimeIndex([0, np.nan], tz='US/Eastern')))
    assert (not array_equivalent(DatetimeIndex([0, np.nan], tz='CET'), DatetimeIndex([0, np.nan], tz='US/Eastern')))
...
def test_array_equivalent_series(val):
    arr = np.array([1, 2])
    cm = (tm.assert_produces_warning(FutureWarning, check_stacklevel=False) if isinstance(val, str) else nullcontext())
    with cm:
...
@pytest.mark.parametrize('dtype, na_value', [(np.dtype('M8[ns]'), NaT), (np.dtype('m8[ns]'), NaT), (DatetimeTZDtype.construct_from_string('datetime64[ns, US/Eastern]'), NaT), (PeriodDtype('M'), NaT), ('u1', 0), ('u2', 0), ('u4', 0), ('u8', 0), ('i1', 0), ('i2', 0), ('i4', 0), ('i8', 0), ('bool', False), ('f2', np.nan), ('f4', np.nan), ('f8', np.nan), ('O', np.nan), (IntervalDtype(), np.nan)])
...
na_vals = (([None, NaT, float('NaN'), complex('NaN'), np.nan, np.float64('NaN'), np.float32('NaN'), np.complex64(np.nan), np.complex128(np.nan), np.datetime64('NaT'), np.timedelta64('NaT')] + [np.datetime64('NaT', unit) for unit in m8_units]) + [np.timedelta64('NaT', unit) for unit in m8_units])
inf_vals = [float('inf'), float('-inf'), complex('inf'), complex('-inf'), np.inf, np.NINF]
int_na_vals = [np.int64(NaT.value), int(NaT.value)]
sometimes_na_vals = [Decimal('NaN')]
never_na_vals = [(- 0.0), np.float64('-0.0'), (- 0j), np.complex64((- 0j))]
