...
def test_period_dtype_mismatch(dtype2):
    ...
    assert (find_common_type([dtype, dtype2]) == object)
    assert (find_common_type([dtype2, dtype]) == object)
interval_dtypes = [IntervalDtype(np.int64), IntervalDtype(np.float64), IntervalDtype(np.uint64), IntervalDtype(DatetimeTZDtype(unit='ns', tz='US/Eastern')), IntervalDtype('M8[ns]'), IntervalDtype('m8[ns]')]
