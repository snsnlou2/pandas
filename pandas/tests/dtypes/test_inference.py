...
from pandas.core.dtypes.common import ensure_int32, is_bool, is_datetime64_any_dtype, is_datetime64_dtype, is_datetime64_ns_dtype, is_datetime64tz_dtype, is_float, is_integer, is_number, is_scalar, is_scipy_sparse, is_timedelta64_dtype, is_timedelta64_ns_dtype
import pandas as pd
from pandas import Categorical, DataFrame, DateOffset, DatetimeIndex, Index, Interval, Period, PeriodIndex, Series, Timedelta, TimedeltaIndex, Timestamp
...
class TestInference():
    ...
    def test_maybe_convert_numeric_infinities(self, coerce_numeric, infinity, prefix):
        result = lib.maybe_convert_numeric(np.array([(prefix + infinity)], dtype=object), na_values={'', 'NULL', 'nan'}, coerce_numeric=coerce_numeric)
    ...
    def test_maybe_convert_numeric_infinities_raises(self):
        ...
        with pytest.raises(ValueError, match=msg):
            lib.maybe_convert_numeric(np.array(['foo_inf'], dtype=object), na_values={'', 'NULL', 'nan'}, coerce_numeric=False)
    ...
    def test_convert_numeric_string_uint64(self):
        result = lib.maybe_convert_numeric(np.array(['uint64'], dtype=object), set(), coerce_numeric=True)
...
class TestTypeInference():
    ...
    @pytest.mark.parametrize('arr', [np.array([np.timedelta64('nat'), np.datetime64('2011-01-02')], dtype=object), np.array([np.datetime64('2011-01-02'), np.timedelta64('nat')], dtype=object), np.array([np.datetime64('2011-01-01'), Timestamp('2011-01-02')]), np.array([Timestamp('2011-01-02'), np.datetime64('2011-01-01')]), np.array([np.nan, Timestamp('2011-01-02'), 1.1]), np.array([np.nan, '2011-01-01', Timestamp('2011-01-02')]), np.array([np.datetime64('nat'), np.timedelta64(1, 'D')], dtype=object), np.array([np.timedelta64(1, 'D'), np.datetime64('nat')], dtype=object)])
