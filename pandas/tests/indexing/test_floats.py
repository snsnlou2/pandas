...
class TestFloatIndexers():
    ...
    @pytest.mark.parametrize('index_func', [tm.makeStringIndex, tm.makeUnicodeIndex, tm.makeCategoricalIndex, tm.makeDateIndex, tm.makeTimedeltaIndex, tm.makePeriodIndex])
    ...
    @pytest.mark.parametrize('idxr,getitem', [((lambda x: x.loc), False), ((lambda x: x), True)])
    ...
    @pytest.mark.parametrize('index_func', [tm.makeStringIndex, tm.makeUnicodeIndex, tm.makeDateIndex, tm.makeTimedeltaIndex, tm.makePeriodIndex])
    ...
    def test_slice_non_numeric(self, index_func, idx, frame_or_series):
        ...
        with pytest.raises(TypeError, match=msg):
            s.iloc[idx]
        msg = f'cannot do (slice|positional) indexing on {type(index).__name__} with these indexers \[(3|4)(\.0)?\] of type (float|int)'
