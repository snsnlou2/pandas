...
import numpy as np
import pytest
from pandas import Categorical, CategoricalDtype, DataFrame, Index, NaT, Series, concat, date_range, isna
...
class TestiLoc(Base):
    ...
    def test_iloc_getitem_int(self):
        self.check_result('iloc', 2, typs=['labels', 'mixed', 'ts', 'floats', 'empty'], fails=IndexError)

    def test_iloc_getitem_neg_int(self):
        self.check_result('iloc', (- 1), typs=['labels', 'mixed', 'ts', 'floats', 'empty'], fails=IndexError)

    def test_iloc_getitem_list_int(self):
        self.check_result('iloc', [0, 1, 2], typs=['labels', 'mixed', 'ts', 'floats', 'empty'], fails=IndexError)
        ...
        with catch_warnings(record=True):
            ...
            for idx in [None, 'index', 'locs']:
                ...
                for method in ['', '.loc', '.iloc']:
                    ...
                    r = expected.get(key)
                    if (r != ans):
                        raise AssertionError(f'[{key}] does not match [{ans}], received [{r}]')
