...
def test_loc_getitem_tuple_plus_slice():
    df = DataFrame({'a': np.arange(10), 'b': np.arange(10), 'c': np.random.randn(10), 'd': np.random.randn(10)}).set_index(['a', 'b'])
    expected = df.loc[(0, 0)]
    result = df.loc[(0, 0), :]
    tm.assert_series_equal(result, expected)
...
def test_loc_nan_multiindex():
    ...
    expected = DataFrame(np.ones((1, 4)), index=Index([np.nan], dtype='object', name='u3'), columns=Index(['d1', 'd2', 'd3', 'd4'], dtype='object'))
    tm.assert_frame_equal(result, expected)
...
def test_loc_period_string_indexing():
    ...
    expected = Series([np.nan], dtype=object, name='OMS', index=MultiIndex.from_tuples([(pd.Period('2013Q1'), 1111)], names=['Periode', 'CVR']))
    tm.assert_series_equal(result, expected)
...
def test_3levels_leading_period_index():
    pi = pd.PeriodIndex(['20181101 1100', '20181101 1200', '20181102 1300', '20181102 1400'], name='datetime', freq='B')
    lev2 = ['A', 'A', 'Z', 'W']
    lev3 = ['B', 'C', 'Q', 'F']
