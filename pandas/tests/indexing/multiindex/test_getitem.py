...
@pytest.mark.parametrize('indexer', [(lambda s: s[(2000, 3, 10)]), (lambda s: s.loc[(2000, 3, 10)])])
def test_series_getitem_returns_scalar(multiindex_year_month_day_dataframe_random_data, indexer):
    s = multiindex_year_month_day_dataframe_random_data['A']
    expected = s.iloc[49]
    result = indexer(s)
...
@pytest.mark.parametrize('indexer,expected_error_msg', [((lambda df: df[('foo', 'four')]), "^\\('foo', 'four'\\)$"), ((lambda df: df['foobar']), "^'foobar'$")])
