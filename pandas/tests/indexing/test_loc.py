...
import pandas as pd
from pandas import Categorical, CategoricalIndex, DataFrame, Index, MultiIndex, Series, SparseDtype, Timedelta, Timestamp, date_range, timedelta_range, to_datetime, to_timedelta
...
class TestLoc(Base):
    ...
    def test_loc_getitem_label_out_of_range(self):
        self.check_result('loc', 'f', typs=['ints', 'uints', 'labels', 'mixed', 'ts'], fails=KeyError)
    ...
    def test_loc_getitem_label_list_with_missing(self):
        ...
        self.check_result('loc', [0, 2, 10], typs=['ints', 'uints', 'floats'], axes=0, fails=KeyError)
        self.check_result('loc', [3, 6, 7], typs=['ints', 'uints', 'floats'], axes=1, fails=KeyError)
        self.check_result('loc', [(1, 3), (1, 4), (2, 5)], typs=['multi'], axes=0, fails=KeyError)

    def test_loc_getitem_label_list_fails(self):
        self.check_result('loc', [20, 30, 40], typs=['ints', 'uints'], axes=1, fails=KeyError)
    ...
    def test_loc_getitem_label_slice(self):
        self.check_result('loc', slice(1, 3), typs=['labels', 'mixed', 'empty', 'ts', 'floats'], fails=TypeError)
        self.check_result('loc', slice('20130102', '20130104'), typs=['ts'], axes=1, fails=TypeError)
        ...
        self.check_result('loc', slice(2, 4, 2), typs=['mixed'], axes=0, fails=TypeError)
...
class TestLoc2():
    ...
    def test_loc_setitem_dtype(self):
        ...
        expected = DataFrame({'id': ['A'], 'a': [1.2], 'b': [0.0], 'c': [(- 2.5)]}, dtype='float32')
        tm.assert_frame_equal(df, expected)
    ...
    def test_loc_index(self):
        df = DataFrame(np.random.random(size=(5, 10)), index=['alpha_0', 'alpha_1', 'alpha_2', 'beta_0', 'beta_1'])
    ...
    def test_loc_setitem_frame(self):
        ...
        expected = DataFrame({'A': Series([1, 2, 3], index=[4, 3, 5])}).reindex(index=[3, 5, 4])
        tm.assert_frame_equal(df, expected)
        ...
        df['B'] = np.nan
        df.loc[(keys2, 'B')] = val2
        expected = DataFrame({'A': Series(val1, index=keys1), 'B': Series(val2, index=keys2)}).reindex(index=index)
        tm.assert_frame_equal(df, expected)
    ...
    def test_loc_non_unique_memory_error(self):
        ...
        def gen_test(length, l2):
            return pd.concat([DataFrame(np.random.randn(length, len(columns)), index=np.arange(length), columns=columns), DataFrame(np.ones((l2, len(columns))), index=([0] * l2), columns=columns)])
...
class TestLocWithMultiIndex():
    ...
    def test_loc_setitem_multiindex_slice(self):
        index = pd.MultiIndex.from_tuples(zip(['bar', 'bar', 'baz', 'baz', 'foo', 'foo', 'qux', 'qux'], ['one', 'two', 'one', 'two', 'one', 'two', 'one', 'two']), names=['first', 'second'])
...
class TestLabelSlicing():
    ...
    def test_loc_getitem_slice_columns_mixed_dtype(self):
        ...
        expected = DataFrame(data=[[2, 3]], index=[0], columns=pd.Index([1, 2], dtype=object))
...
class TestLocListlike():
    ...
    def test_loc_getitem_list_of_labels_categoricalindex_with_na(self, box):
        ...
        ser2 = ser[:(- 1)]
        ci2 = ci[1:]
        msg = 'Passing list-likes to .loc or \\[\\] with any missing labels is no longer supported. The following labels were missing: (Categorical)?Index\\(\\[nan\\], .*\\). See https'
        with pytest.raises(KeyError, match=msg):
            ser2.loc[box(ci2)]
        with pytest.raises(KeyError, match=msg):
            ser2[box(ci2)]
...
def test_series_getitem_label_list_missing_integer_values():
    s = Series(index=np.array([9730701000001104, 10049011000001109]), data=np.array([999000011000001104, 999000011000001104]))
    with pytest.raises(KeyError, match='with any missing labels'):
...
def test_loc_axis_1_slice():
    ...
    df = DataFrame(np.ones((10, 8)), index=tuple('ABCDEFGHIJ'), columns=pd.MultiIndex.from_tuples(cols))
    result = df.loc(axis=1)[(2014, 9):(2015, 8)]
    expected = DataFrame(np.ones((10, 4)), index=tuple('ABCDEFGHIJ'), columns=pd.MultiIndex.from_tuples([(2014, 9), (2014, 10), (2015, 7), (2015, 8)]))
    tm.assert_frame_equal(result, expected)

def test_loc_set_dataframe_multiindex():
    expected = DataFrame('a', index=range(2), columns=pd.MultiIndex.from_product([range(2), range(2)]))
...
def test_loc_datetimelike_mismatched_dtypes():
    df = DataFrame(np.random.randn(5, 3), columns=['a', 'b', 'c'], index=pd.date_range('2012', freq='H', periods=5))
