...
class TestSetitemCoercion(CoercionBase):
    ...
    @pytest.mark.parametrize('val,exp_dtype', [(1, object), (1.1, object), ((1 + 1j), object), (True, object)])
    ...
    @pytest.mark.parametrize('val,exp_dtype', [(1, np.int64), (1.1, np.float64), ((1 + 1j), np.complex128), (True, object)])
    ...
    def test_setitem_series_int8(self, val, exp_dtype, request):
        ...
        if (exp_dtype is np.int16):
            ...
            mark = pytest.mark.xfail(reason='BUG: it must be pd.Series([1, 1, 3, 4], dtype=np.int16')
            request.node.add_marker(mark)
        exp = pd.Series([1, val, 3, 4], dtype=np.int8)
        self._assert_setitem_series_conversion(obj, val, exp, exp_dtype)

    @pytest.mark.parametrize('val,exp_dtype', [(1, np.float64), (1.1, np.float64), ((1 + 1j), np.complex128), (True, object)])
    ...
    @pytest.mark.parametrize('val,exp_dtype', [(1, np.complex128), (1.1, np.complex128), ((1 + 1j), np.complex128), (True, object)])
    ...
    @pytest.mark.parametrize('val,exp_dtype', [(1, np.int64), (3, np.int64), (1.1, np.float64), ((1 + 1j), np.complex128), (True, np.bool_)])
    ...
    @pytest.mark.parametrize('val,exp_dtype', [(pd.Timestamp('2012-01-01'), 'datetime64[ns]'), (1, object), ('x', object)])
    ...
    @pytest.mark.parametrize('val,exp_dtype', [(pd.Timestamp('2012-01-01', tz='US/Eastern'), 'datetime64[ns, US/Eastern]'), (pd.Timestamp('2012-01-01', tz='US/Pacific'), object), (pd.Timestamp('2012-01-01'), object), (1, object)])
    ...
    @pytest.mark.parametrize('val,exp_dtype', [(pd.Timedelta('12 day'), 'timedelta64[ns]'), (1, object), ('x', object)])
    ...
    @pytest.mark.parametrize('val,exp_dtype', [(5, IndexError), (5.1, np.float64), ('x', object)])
...
class TestInsertIndexCoercion(CoercionBase):
    ...
    @pytest.mark.parametrize('insert, coerced_val, coerced_dtype', [(1, 1, np.int64), (1.1, 1.1, np.float64), (False, False, object), ('x', 'x', object)])
    ...
    @pytest.mark.parametrize('insert, coerced_val, coerced_dtype', [(1, 1.0, np.float64), (1.1, 1.1, np.float64), (False, False, object), ('x', 'x', object)])
...
class TestWhereCoercion(CoercionBase):
    ...
    @pytest.mark.parametrize('fill_val', [pd.Timestamp('2012-01-01'), pd.Timestamp('2012-01-01').to_datetime64(), pd.Timestamp('2012-01-01').to_pydatetime()])
    ...
    def test_where_index_period(self):
        ...
        msg = 'Input has different freq=D from PeriodArray\\(freq=Q-DEC\\)'
        with pytest.raises(ValueError, match=msg):
...
class TestFillnaSeriesCoercion(CoercionBase):
    ...
    @pytest.mark.parametrize('fill_val,fill_dtype', [(pd.Timestamp('2012-01-01', tz='US/Eastern'), 'datetime64[ns, US/Eastern]'), (pd.Timestamp('2012-01-01'), object), (pd.Timestamp('2012-01-01', tz='Asia/Tokyo'), object), (1, object), ('x', object)])
...
class TestReplaceSeriesCoercion(CoercionBase):
    ...
    @pytest.mark.parametrize('how', ['dict', 'series'])
    @pytest.mark.parametrize('to_key', ['timedelta64[ns]', 'bool', 'object', 'complex128', 'float64', 'int64'])
    @pytest.mark.parametrize('from_key', ['datetime64[ns, UTC]', 'datetime64[ns, US/Eastern]'])
