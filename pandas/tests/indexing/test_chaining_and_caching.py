...
class TestChaining():
    ...
    def test_detect_chained_assignment(self):
        ...
        with pytest.raises(com.SettingWithCopyError, match=msg):
            df.iloc[0:5]['group'] = 'a'
        df = DataFrame({'A': date_range('20130101', periods=5), 'B': np.random.randn(5), 'C': np.arange(5, dtype='int64'), 'D': ['a', 'b', 'c', 'd', 'e']})
        with pytest.raises(com.SettingWithCopyError, match=msg):
            df.loc[2]['D'] = 'foo'
        with pytest.raises(com.SettingWithCopyError, match=msg):
            df.loc[2]['C'] = 'foo'
    ...
    def test_detect_chained_assignment_warnings_filter_and_dupe_cols(self):
        with option_context('chained_assignment', 'warn'):
            ...
            expected = DataFrame([[1, 2, 3], [4, 5, 6], [7, 8, (- 9)]], columns=['a', 'a', 'c'])
            tm.assert_frame_equal(df, expected)
