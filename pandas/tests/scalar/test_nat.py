...
from pandas import DatetimeIndex, DatetimeTZDtype, Index, NaT, Period, Series, Timedelta, TimedeltaIndex, Timestamp, isna, offsets
import pandas._testing as tm
from pandas.core.arrays import DatetimeArray, PeriodArray, TimedeltaArray
from pandas.core.ops import roperator

@pytest.mark.parametrize('nat,idx', [(Timestamp('NaT'), DatetimeIndex), (Timedelta('NaT'), TimedeltaIndex), (Period('NaT', freq='M'), PeriodArray)])
...
@pytest.mark.parametrize('klass,expected', [(Timestamp, ['freqstr', 'normalize', 'to_julian_date', 'to_period', 'tz']), (Timedelta, ['components', 'delta', 'is_populated', 'resolution_string', 'to_pytimedelta', 'to_timedelta64', 'view'])])
...
@pytest.mark.parametrize('compare', (_get_overlap_public_nat_methods(Timestamp, True) + _get_overlap_public_nat_methods(Timedelta, True)))
def test_nat_doc_strings(compare):
    (klass, method) = compare
    klass_doc = getattr(klass, method).__doc__
    nat_doc = getattr(NaT, method).__doc__
    assert (klass_doc == nat_doc)
_ops = {'left_plus_right': (lambda a, b: (a + b)), 'right_plus_left': (lambda a, b: (b + a)), 'left_minus_right': (lambda a, b: (a - b)), 'right_minus_left': (lambda a, b: (b - a)), 'left_times_right': (lambda a, b: (a * b)), 'right_times_left': (lambda a, b: (b * a)), 'left_div_right': (lambda a, b: (a / b)), 'right_div_left': (lambda a, b: (b / a))}
...
@pytest.mark.parametrize('value', [DatetimeIndex(['2011-01-01', '2011-01-02'], name='x'), DatetimeIndex(['2011-01-01', '2011-01-02'], tz='US/Eastern', name='x'), DatetimeArray._from_sequence(['2011-01-01', '2011-01-02']), DatetimeArray._from_sequence(['2011-01-01', '2011-01-02'], dtype=DatetimeTZDtype(tz='US/Pacific')), TimedeltaIndex(['1 day', '2 day'], name='x')])
