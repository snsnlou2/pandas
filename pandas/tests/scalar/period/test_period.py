...
class TestPeriodConstruction():
    ...
    @pytest.mark.parametrize('sec_float, expected', [('.000000001', 1), ('.000000999', 999), ('.123456789', 789), ('.999999999', 999)])
...
class TestPeriodComparisons():
    ...
    def test_period_cmp_nat(self):
        ...
        for (left, right) in [(NaT, p), (p, NaT), (NaT, t), (t, NaT)]:
            assert (not (left < right))
            assert (not (left > right))
            assert (not (left == right))
            assert (left != right)
...
class TestArithmetic():
    ...
    def test_add_timestamp_raises(self, rbox, lbox):
        ...
        msg = '|'.join(['cannot add', 'unsupported operand', 'can only operate on a', 'incompatible type', 'ufunc add cannot use operands'])
        with pytest.raises(TypeError, match=msg):
            (lbox(ts) + rbox(per))
        with pytest.raises(TypeError, match=msg):
            (lbox(per) + rbox(ts))
