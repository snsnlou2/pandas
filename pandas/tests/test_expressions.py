...
_integer2 = DataFrame(np.random.randint(1, 100, size=(101, 4)), columns=list('ABCD'), dtype='int64')
...
class TestExpressions():
    ...
    @staticmethod
    def call_op(df, other, flex, opname):
        if flex:
    ...
    def run_arithmetic(self, df, other, flex):
    ...
    def run_binary(self, df, other, flex):
    ...
    def run_frame(self, df, other, flex):
    ...
    @pytest.mark.parametrize('op', ['__mod__', pytest.param('__rmod__', marks=pytest.mark.xfail(reason='GH-36552')), '__floordiv__', '__rfloordiv__'])
