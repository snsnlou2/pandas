...
class TestMethods(BaseSparseTests, base.BaseMethodsTests):
    ...
    def test_combine_le(self, data_repeated):
        ...
        expected = pd.Series(SparseArray([(a <= b) for (a, b) in zip(list(orig_data1), list(orig_data2))], fill_value=False))
        self.assert_series_equal(result, expected)
        val = s1.iloc[0]
        result = s1.combine(val, (lambda x1, x2: (x1 <= x2)))
        expected = pd.Series(SparseArray([(a <= val) for a in list(orig_data1)], fill_value=False))
        self.assert_series_equal(result, expected)
