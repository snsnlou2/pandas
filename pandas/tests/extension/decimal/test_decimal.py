...
class BaseDecimal():
    ...
    def assert_frame_equal(cls, left, right, *args, **kwargs):
        tm.assert_index_equal(left.columns, right.columns, exact=kwargs.get('check_column_type', 'equiv'), check_names=kwargs.get('check_names', True), check_exact=kwargs.get('check_exact', False), check_categorical=kwargs.get('check_categorical', True), obj=f"{kwargs.get('obj', 'DataFrame')}.columns")
...
class Reduce():
    ...
    def check_reduce(self, s, op_name, skipna):
        if (op_name in ['median', 'skew', 'kurt']):
            msg = 'decimal does not support the .* operation'
...
@pytest.mark.xfail(reason='raising AssertionError as this is not implemented, though easy enough to do')
...
def test_combine_from_sequence_raises():
    ser = pd.Series(DecimalArrayWithoutFromSequence([decimal.Decimal('1.0'), decimal.Decimal('2.0')]))
