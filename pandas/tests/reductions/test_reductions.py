...
import pytest
import pandas as pd
from pandas import Categorical, DataFrame, DatetimeIndex, Index, NaT, Period, PeriodIndex, RangeIndex, Series, Timedelta, TimedeltaIndex, Timestamp, date_range, isna, timedelta_range, to_timedelta
import pandas._testing as tm
from pandas.core import nanops

def get_objs():
    indexes = [tm.makeBoolIndex(10, name='a'), tm.makeIntIndex(10, name='a'), tm.makeFloatIndex(10, name='a'), tm.makeDateIndex(10, name='a'), tm.makeDateIndex(10, name='a').tz_localize(tz='US/Eastern'), tm.makePeriodIndex(10, name='a'), tm.makeStringIndex(10, name='a'), tm.makeUnicodeIndex(10, name='a')]
...
class TestIndexReductions():
    ...
    def test_invalid_td64_reductions(self, opname):
        s = Series([(Timestamp('20130101') + timedelta(seconds=(i * i))) for i in range(10)])
        td = s.diff()
        msg = '|'.join([f"reduction operation '{opname}' not allowed for this dtype", f'cannot perform {opname} with type timedelta64\[ns\]', f"'TimedeltaArray' does not implement reduction '{opname}'"])
    ...
    def test_minmax_tz(self, tz_naive_fixture):
        ...
        assert idx1.is_monotonic
        idx2 = DatetimeIndex(['2011-01-01', pd.NaT, '2011-01-03', '2011-01-02', pd.NaT], tz=tz)
        assert (not idx2.is_monotonic)
        for idx in [idx1, idx2]:
...
class TestSeriesReductions():
    ...
    def test_empty_multi(self, method, unit):
        s = Series([1, np.nan, np.nan, np.nan], index=pd.MultiIndex.from_product([('a', 'b'), (0, 1)]))
    ...
    def test_ops_consistency_on_empty(self, method):
        ...
        if (method == 'var'):
            msg = '|'.join(["operation 'var' not allowed", 'cannot perform var with type timedelta64\\[ns\\]', "'TimedeltaArray' does not implement reduction 'var'"])
            with pytest.raises(TypeError, match=msg):
    ...
    def test_all_any_boolean(self):
        ...
        assert (s1.all(skipna=False) is pd.NA)
        assert s1.all(skipna=True)
        assert (s2.any(skipna=False) is pd.NA)
        assert (not s2.any(skipna=True))
        ...
        s = Series([False, False, True, True, False, True], index=[0, 0, 1, 1, 2, 2], dtype='boolean')
        tm.assert_series_equal(s.all(level=0), Series([False, True, False]))
        tm.assert_series_equal(s.any(level=0), Series([False, True, True]))
...
class TestDatetime64SeriesReductions():
    ...
    @pytest.mark.parametrize('nat_ser', [Series([pd.NaT, pd.NaT]), Series([pd.NaT, Timedelta('nat')]), Series([Timedelta('nat'), Timedelta('nat')])])
    ...
    @pytest.mark.parametrize('nat_df', [DataFrame([pd.NaT, pd.NaT]), DataFrame([pd.NaT, Timedelta('nat')]), DataFrame([Timedelta('nat'), Timedelta('nat')])])
