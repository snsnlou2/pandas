...
from pandas.core.computation.ops import ARITH_OPS_SYMS, SPECIAL_CASE_ARITH_OPS_SYMS, _binary_math_ops, _binary_ops_dict, _unary_math_ops

@pytest.fixture(params=(pytest.param(engine, marks=pytest.mark.skipif(((engine == 'numexpr') and (not USE_NUMEXPR)), reason=f'numexpr enabled->{USE_NUMEXPR}, installed->{NUMEXPR_INSTALLED}')) for engine in ENGINES))
def engine(request):
...
def _series_and_2d_ndarray(lhs, rhs):
    return ((isinstance(lhs, Series) and isinstance(rhs, np.ndarray) and (rhs.ndim > 1)) or (isinstance(rhs, Series) and isinstance(lhs, np.ndarray) and (lhs.ndim > 1)))

def _series_and_frame(lhs, rhs):
    return ((isinstance(lhs, Series) and isinstance(rhs, DataFrame)) or (isinstance(rhs, Series) and isinstance(lhs, DataFrame)))
...
def lhs(request):
    ...
    opts = (DataFrame(np.random.randn(10, 5)), Series(np.random.randn(5)), Series([1, 2, np.nan, np.nan, 5]), nan_df1, np.random.randn())
...
class TestEvalNumexprPandas():
    ...
    def test_binary_arith_ops(self, op, lhs, rhs, request):
        if ((op == '/') and isinstance(lhs, DataFrame) and isinstance(rhs, DataFrame) and (not lhs.isna().any().any()) and (rhs.shape == (10, 5)) and np_version_under1p17 and is_platform_windows() and compat.PY38):
            mark = pytest.mark.xfail(reason='GH#37328 floating point precision on Windows builds')
    ...
    @pytest.mark.parametrize('lhs', [DataFrame(np.random.randn(5, 2)), DataFrame(np.random.randint(5, size=(5, 2))), DataFrame((np.random.rand(5, 2) > 0.5))])
    ...
    @pytest.mark.parametrize('lhs', [Series(np.random.randn(5)), Series(np.random.randint(5, size=5)), Series((np.random.rand(5) > 0.5))])
    ...
    def test_disallow_scalar_bool_ops(self):
        ...
        (x, a, b, df) = (np.random.randn(3), 1, 2, DataFrame(np.random.randn(3, 2)))
        for ex in exprs:
...
class TestOperationsNumExprPandas():
    ...
    @classmethod
    def setup_class(cls):
        cls.arith_ops = [op for op in (expr.ARITH_OPS_SYMS + expr.CMP_OPS_SYMS) if (op not in cls.exclude_arith)]
...
_parsers = {'python': PythonExprVisitor, 'pytables': pytables.PyTablesExprVisitor, 'pandas': PandasExprVisitor}
...
@pytest.mark.parametrize('other', ["'x'", pytest.param('...', marks=pytest.mark.xfail((not compat.PY38), reason='GH-28116'))])
