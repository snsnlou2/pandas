...
_any_string_method = ([('cat', (), {'sep': ','}), ('cat', (Series(list('zyx')),), {'sep': ',', 'join': 'left'}), ('center', (10,), {}), ('contains', ('a',), {}), ('count', ('a',), {}), ('decode', ('UTF-8',), {}), ('encode', ('UTF-8',), {}), ('endswith', ('a',), {}), ('endswith', ('a',), {'na': True}), ('endswith', ('a',), {'na': False}), ('extract', ('([a-z]*)',), {'expand': False}), ('extract', ('([a-z]*)',), {'expand': True}), ('extractall', ('([a-z]*)',), {}), ('find', ('a',), {}), ('findall', ('a',), {}), ('get', (0,), {}), ('index', ('',), {}), ('join', (',',), {}), ('ljust', (10,), {}), ('match', ('a',), {}), ('fullmatch', ('a',), {}), ('normalize', ('NFC',), {}), ('pad', (10,), {}), ('partition', (' ',), {'expand': False}), ('partition', (' ',), {'expand': True}), ('repeat', (3,), {}), ('replace', ('a', 'z'), {}), ('rfind', ('a',), {}), ('rindex', ('',), {}), ('rjust', (10,), {}), ('rpartition', (' ',), {'expand': False}), ('rpartition', (' ',), {'expand': True}), ('slice', (0, 1), {}), ('slice_replace', (0, 1, 'z'), {}), ('split', (' ',), {'expand': False}), ('split', (' ',), {'expand': True}), ('startswith', ('a',), {}), ('startswith', ('a',), {'na': True}), ('startswith', ('a',), {'na': False}), ('translate', ({97: 100},), {}), ('wrap', (2,), {}), ('zfill', (10,), {})] + list(zip(['capitalize', 'cat', 'get_dummies', 'isalnum', 'isalpha', 'isdecimal', 'isdigit', 'islower', 'isnumeric', 'isspace', 'istitle', 'isupper', 'len', 'lower', 'lstrip', 'partition', 'rpartition', 'rsplit', 'rstrip', 'slice', 'slice_replace', 'split', 'strip', 'swapcase', 'title', 'upper', 'casefold'], ([()] * 100), ([{}] * 100))))
(ids, _, _) = zip(*_any_string_method)
missing_methods = ({f for f in dir(strings.StringMethods) if (not f.startswith('_'))} - set(ids))
...
_any_allowed_skipna_inferred_dtype = [('string', ['a', np.nan, 'c']), ('bytes', [b'a', np.nan, b'c']), ('empty', [np.nan, np.nan, np.nan]), ('empty', []), ('mixed-integer', ['a', np.nan, 2])]
...
class TestStringMethods():
    ...
    @pytest.mark.parametrize('dtype', [object, 'category'])
    def test_api_per_method(self, index_or_series, dtype, any_allowed_skipna_inferred_dtype, any_string_method, request):
        box = index_or_series
        (inferred_dtype, values) = any_allowed_skipna_inferred_dtype
        (method_name, args, kwargs) = any_string_method
        reason = None
    ...
    def test_iter_object_try_string(self):
        ds = Series([slice(None, np.random.randint(10), np.random.randint(10, 20)) for _ in range(4)])
        (i, s) = (100, 'h')
        with tm.assert_produces_warning(FutureWarning):
            for (i, s) in enumerate(ds.str):
                pass
    ...
    def test_str_cat_categorical(self, index_or_series, dtype_caller, dtype_target, sep):
        box = index_or_series
    ...
    def test_str_cat_align_mixed_inputs(self, join):
        ...
        rhs_idx = (t.index.intersection(s.index) if (join == 'inner') else t.index.union(s.index))
    ...
    def test_contains(self):
        ...
        xp = Series(np.array([False, np.nan, False, np.nan, np.nan, True, np.nan, np.nan, np.nan], dtype=np.object_))
        tm.assert_series_equal(rs, xp)
        rs = mixed.str.contains('o')
    ...
    def test_replace(self):
        ...
        result = values.str.replace('(?<=\\w),(?=\\w)', ', ', flags=re.UNICODE, regex=True)
        tm.assert_series_equal(result, exp)
    ...
    def test_fullmatch(self):
        ...
        tm.assert_series_equal(result, exp)
        string_values = Series(['fooBAD__barBAD', 'BAD_BADleroybrown', np.nan, 'foo'], dtype='string')
    ...
    def test_extractall_same_as_extract_subject_index(self):
        i = MultiIndex.from_tuples([('A', 'first'), ('B', 'second'), ('C', 'third')], names=('capital', 'ordinal'))
    ...
    def test_empty_str_methods(self):
        ...
        tm.assert_frame_equal(DataFrame(columns=[0], dtype=str), empty.str.extract('()', expand=True))
    ...
    def test_pad(self):
        ...
        xp = Series(['    a', np.nan, '    b', np.nan, np.nan, '   ee', np.nan, np.nan, np.nan])
        assert isinstance(rs, Series)
        tm.assert_almost_equal(rs, xp)
        mixed = Series(['a', np.nan, 'b', True, datetime.today(), 'ee', None, 1, 2.0])
    ...
    def test_center_ljust_rjust(self):
        ...
        xp = Series(['  a  ', np.nan, '  b  ', np.nan, np.nan, '  c  ', ' eee ', np.nan, np.nan, np.nan])
        assert isinstance(rs, Series)
        tm.assert_almost_equal(rs, xp)
    ...
    def test_split(self):
        ...
        exp = Series([['a', 'b', 'c'], np.nan, ['d', 'e', 'f'], np.nan, np.nan, np.nan, np.nan, np.nan])
        assert isinstance(result, Series)
        tm.assert_almost_equal(result, exp)
    ...
    def test_split_to_dataframe(self):
        ...
        result = s.str.split('_', expand=True)
        exp = DataFrame({0: ['some', 'one'], 1: ['unequal', 'of'], 2: ['splits', 'these'], 3: [np.nan, 'things'], 4: [np.nan, 'is'], 5: [np.nan, 'not']})
        tm.assert_frame_equal(result, exp)
        s = Series(['some_splits', 'with_index'], index=['preserve', 'me'])
        result = s.str.split('_', expand=True)
        exp = DataFrame({0: ['some', 'with'], 1: ['splits', 'index']}, index=['preserve', 'me'])
        tm.assert_frame_equal(result, exp)
    ...
    def test_partition_series(self):
        ...
        exp = Series([('a', '_', 'b_c'), ('c', '_', 'd_e'), np.nan, ('f', '_', 'g_h'), None])
        tm.assert_series_equal(result, exp)
        result = values.str.rpartition('_', expand=False)
        exp = Series([('a_b', '_', 'c'), ('c_d', '_', 'e'), np.nan, ('f_g', '_', 'h'), None])
        tm.assert_series_equal(result, exp)
        values = Series(['a__b__c', 'c__d__e', np.nan, 'f__g__h', None])
        result = values.str.partition('__', expand=False)
        exp = Series([('a', '__', 'b__c'), ('c', '__', 'd__e'), np.nan, ('f', '__', 'g__h'), None])
        tm.assert_series_equal(result, exp)
        result = values.str.rpartition('__', expand=False)
        exp = Series([('a__b', '__', 'c'), ('c__d', '__', 'e'), np.nan, ('f__g', '__', 'h'), None])
        tm.assert_series_equal(result, exp)
        values = Series(['a b c', 'c d e', np.nan, 'f g h', None])
        result = values.str.partition(expand=False)
        exp = Series([('a', ' ', 'b c'), ('c', ' ', 'd e'), np.nan, ('f', ' ', 'g h'), None])
        tm.assert_series_equal(result, exp)
    ...
    def test_partition_to_dataframe(self):
        values = Series(['a_b_c', 'c_d_e', np.nan, 'f_g_h', None])
        result = values.str.partition('_')
        exp = DataFrame({0: ['a', 'c', np.nan, 'f', None], 1: ['_', '_', np.nan, '_', None], 2: ['b_c', 'd_e', np.nan, 'g_h', None]})
        tm.assert_frame_equal(result, exp)
        result = values.str.rpartition('_')
        exp = DataFrame({0: ['a_b', 'c_d', np.nan, 'f_g', None], 1: ['_', '_', np.nan, '_', None], 2: ['c', 'e', np.nan, 'h', None]})
        tm.assert_frame_equal(result, exp)
        values = Series(['a_b_c', 'c_d_e', np.nan, 'f_g_h', None])
    ...
    def test_contains_moar(self):
        ...
        expected = Series([False, False, False, True, True, False, np.nan, False, False, True])
        tm.assert_series_equal(result, expected)
        result = s.str.contains('a', case=False)
        expected = Series([True, False, False, True, True, False, np.nan, True, False, True])
        tm.assert_series_equal(result, expected)
        result = s.str.contains('Aa')
        expected = Series([False, False, False, True, False, False, np.nan, False, False, False])
        tm.assert_series_equal(result, expected)
        result = s.str.contains('ba')
        expected = Series([False, False, False, True, False, False, np.nan, False, False, False])
        tm.assert_series_equal(result, expected)
        result = s.str.contains('ba', case=False)
        expected = Series([False, False, False, True, True, False, np.nan, True, False, False])
        tm.assert_series_equal(result, expected)
...
@pytest.mark.parametrize('method,expected', [('isdigit', [False, None, True]), ('isalpha', [True, None, False]), ('isalnum', [True, None, True]), ('isdigit', [False, None, True])])
