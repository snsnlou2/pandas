...
def create_block(typestr, placement, item_shape=None, num_offset=0):
    ...
    if (typestr in ('float', 'f8', 'f4', 'f2', 'int', 'i8', 'i4', 'i2', 'i1', 'uint', 'u8', 'u4', 'u2', 'u1')):
        ...
        values = DatetimeIndex((np.arange(N) * 1000000000.0), tz=tz)
...
class TestBlock():
    ...
    def test_split(self):
        ...
        assert (len(result) == 3)
        expected = [make_block(values[[0]], placement=[3], ndim=2), make_block(values[[1]], placement=[1], ndim=2), make_block(values[[2]], placement=[6], ndim=2)]
...
class TestBlockManager():
    ...
    def test_iget(self):
        ...
        block = make_block(values=values.copy(), placement=np.arange(3), ndim=values.ndim)
    ...
    def test_get_numeric_data(self):
        ...
        tm.assert_index_equal(numeric.items, Index(['int', 'float', 'complex', 'bool']))
        numeric2.iset(numeric2.items.get_loc('float'), np.array([1000.0, 2000.0, 3000.0]))
        tm.assert_almost_equal(mgr.iget(mgr.items.get_loc('float')).internal_values(), np.array([100.0, 200.0, 300.0]))
...
class TestIndexing():
    MANAGERS = [create_single_mgr('f8', N), create_single_mgr('i8', N), create_mgr('a,b,c,d,e,f: f8', item_shape=(N,)), create_mgr('a,b,c,d,e,f: i8', item_shape=(N,)), create_mgr('a,b: f8; c,d: i8; e,f: string', item_shape=(N,)), create_mgr('a,b: f8; c,d: i8; e,f: f8', item_shape=(N,))]
    ...
    def test_reindex_indexer(self, fill_value, mgr):
        ...
        for ax in range(mgr.ndim):
            ...
            assert_reindex_indexer_is_ok(mgr, ax, Index(['foo', 'bar', 'baz']), [0, 0, 0], fill_value)
            assert_reindex_indexer_is_ok(mgr, ax, Index(['foo', 'bar', 'baz']), [(- 1), 0, (- 1)], fill_value)
            assert_reindex_indexer_is_ok(mgr, ax, Index(['foo', mgr.axes[ax][0], 'baz']), [(- 1), (- 1), (- 1)], fill_value)
            if (mgr.shape[ax] >= 3):
                assert_reindex_indexer_is_ok(mgr, ax, Index(['foo', 'bar', 'baz']), [0, 1, 2], fill_value)
...
class TestBlockPlacement():
    ...
    def test_slice_to_array_conversion(self, slc, arr):
        tm.assert_numpy_array_equal(BlockPlacement(slc).as_array, np.asarray(arr, dtype=np.int64))
...
@pytest.mark.parametrize('typestr, holder', [('category', Categorical), ('M8[ns]', DatetimeArray), ('M8[ns, US/Central]', DatetimeArray), ('m8[ns]', TimedeltaArray), ('sparse', SparseArray)])
