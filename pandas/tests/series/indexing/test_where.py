...
def test_where_error():
    ...
    with pytest.raises(ValueError, match=msg):
        s[[True, False]] = [0, 2, 3]
    msg = 'NumPy boolean array indexing assignment cannot assign 0 input values to the 1 output values where the mask is true'
    with pytest.raises(ValueError, match=msg):
        s[[True, False]] = []
...
def test_where_dt_tz_values(tz_naive_fixture):
    ser1 = Series(pd.DatetimeIndex(['20150101', '20150102', '20150103'], tz=tz_naive_fixture))
    ...
    exp = Series(pd.DatetimeIndex(['20150101', '20150102', '20160516'], tz=tz_naive_fixture))
...
def test_where_categorical(klass):
    exp = klass(pd.Categorical(['A', 'A', 'B', 'B', np.nan], categories=['A', 'B', 'C']), dtype='category')
