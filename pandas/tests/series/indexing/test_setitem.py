...
import numpy as np
import pytest
from pandas import DatetimeIndex, Index, MultiIndex, NaT, Series, Timestamp, date_range, period_range
...
class TestSetitemDT64Values():
    ...
    def test_setitem_with_different_tz_casts_to_object(self):
        ...
        expected = Series([Timestamp('2000-01-01 00:00:00-05:00', tz='US/Eastern'), Timestamp('2000-01-02 00:00:00-06:00', tz='US/Central')], dtype=object)
...
@pytest.mark.parametrize('obj,expected,key', [(Series([2, 3, 4, 5, 6, 7, 8, 9, 10]), Series([np.nan, 3, np.nan, 5, np.nan, 7, np.nan, 9, np.nan]), slice(None, None, 2)), (Series([True, True, False, False]), Series([np.nan, 1, np.nan, 0]), slice(None, None, 2)), (Series(np.arange(10)), Series([np.nan, np.nan, np.nan, np.nan, np.nan, 5, 6, 7, 8, 9]), slice(None, 5)), (Series([1, 2, 3]), Series([np.nan, 2, 3]), 0), (Series([False]), Series([np.nan]), 0), (Series([False, True]), Series([np.nan, 1.0]), 0)])
