...
import pandas as pd
from pandas import Categorical, DataFrame, IndexSlice, MultiIndex, Series, Timedelta, Timestamp, date_range, period_range, timedelta_range
...
def test_basic_getitem_with_labels(datetime_series):
    ...
    with pytest.raises(KeyError, match='with any missing labels'):
        s[arr_inds]
    s = Series(pd.date_range('2011-01-01', periods=3, tz='US/Eastern'), index=['a', 'b', 'c'])
...
def test_getitem_get(datetime_series, string_series, object_series):
    ...
    msg = "Timestamp\\('1999-12-31 00:00:00', freq='B'\\)"
...
def test_setitem_with_tz(tz):
    ...
    exp = Series([Timestamp('2016-01-01 00:00', tz=tz), Timestamp('2011-01-01 00:00', tz=tz), Timestamp('2016-01-01 02:00', tz=tz)])
    ...
    tm.assert_series_equal(s, exp)
    vals = Series([Timestamp('2011-01-01', tz=tz), Timestamp('2012-01-01', tz=tz)], index=[1, 2])
    assert (vals.dtype == f'datetime64[ns, {tz}]')
    s[[1, 2]] = vals
    exp = Series([Timestamp('2016-01-01 00:00', tz=tz), Timestamp('2011-01-01 00:00', tz=tz), Timestamp('2012-01-01 00:00', tz=tz)])
    tm.assert_series_equal(s, exp)
    s = orig.copy()
    s.loc[[1, 2]] = vals
    tm.assert_series_equal(s, exp)
...
def test_setitem_with_tz_dst():
    ...
    s = orig.copy()
    s[1] = Timestamp('2011-01-01', tz=tz)
    exp = Series([Timestamp('2016-11-06 00:00-04:00', tz=tz), Timestamp('2011-01-01 00:00-05:00', tz=tz), Timestamp('2016-11-06 01:00-05:00', tz=tz)])
    tm.assert_series_equal(s, exp)
    s = orig.copy()
    s.loc[1] = Timestamp('2011-01-01', tz=tz)
    tm.assert_series_equal(s, exp)
    ...
    assert (vals.dtype == f'datetime64[ns, {tz}]')
    s[[1, 2]] = vals
    exp = Series([Timestamp('2016-11-06 00:00', tz=tz), Timestamp('2011-01-01 00:00', tz=tz), Timestamp('2012-01-01 00:00', tz=tz)])
    tm.assert_series_equal(s, exp)
    s = orig.copy()
    s.loc[[1, 2]] = vals
    tm.assert_series_equal(s, exp)
...
@pytest.mark.parametrize('nat_val,should_cast', [(pd.NaT, True), (np.timedelta64('NaT', 'ns'), False), (np.datetime64('NaT', 'ns'), True)])
...
@pytest.mark.parametrize('nat_val,should_cast', [(pd.NaT, True), (np.timedelta64('NaT', 'ns'), True), (np.datetime64('NaT', 'ns'), False)])
