...
def test_get():
    ...
    s = Series(np.array([43, 48, 60, 48, 50, 51, 50, 45, 57, 48, 56, 45, 51, 39, 55, 43, 54, 52, 51, 54]), index=pd.Float64Index([25.0, 36.0, 49.0, 64.0, 81.0, 100.0, 121.0, 144.0, 169.0, 196.0, 1225.0, 1296.0, 1369.0, 1444.0, 1521.0, 1600.0, 1681.0, 1764.0, 1849.0, 1936.0]))
    result = s.get(25, 0)
    expected = 43
    assert (result == expected)
...
@pytest.mark.parametrize('arr', [np.random.randn(10), tm.makeDateIndex(10, name='a').tz_localize(tz='US/Eastern')])
def test_get2(arr):
