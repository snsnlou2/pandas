...
import pandas as pd
from pandas import Categorical, DataFrame, DatetimeIndex, Index, Interval, IntervalIndex, MultiIndex, NaT, Period, RangeIndex, Series, Timestamp, date_range, isna, period_range, timedelta_range
...
class TestSeriesConstructors():
    ...
    @pytest.mark.parametrize('input_vals', [[1, 2], ['1', '2'], list(pd.date_range('1/1/2011', periods=2, freq='H')), list(pd.date_range('1/1/2011', periods=2, freq='H', tz='US/Eastern')), [Interval(left=0, right=5)]])
    ...
    def test_constructor_maskedarray(self):
        ...
        index = ['a', 'b', 'c']
        result = Series(data, index=index)
        expected = Series([datetime(2001, 1, 1), iNaT, datetime(2001, 1, 3)], index=index, dtype='M8[ns]')
        tm.assert_series_equal(result, expected)
        data[1] = datetime(2001, 1, 2)
        result = Series(data, index=index)
        expected = Series([datetime(2001, 1, 1), datetime(2001, 1, 2), datetime(2001, 1, 3)], index=index, dtype='M8[ns]')
        tm.assert_series_equal(result, expected)
    ...
    @pytest.mark.parametrize('input', [[1, 2, 3], (1, 2, 3), list(range(3)), Categorical(['a', 'b', 'a']), (i for i in range(3)), map((lambda x: x), range(3))])
    ...
    @pytest.mark.parametrize('index', [pd.date_range('20170101', periods=3, tz='US/Eastern'), pd.date_range('20170101', periods=3), pd.timedelta_range('1 day', periods=3), pd.period_range('2012Q1', periods=3, freq='Q'), Index(list('abc')), pd.Int64Index([1, 2, 3]), RangeIndex(0, 3)], ids=(lambda x: type(x).__name__))
    ...
    def test_constructor_with_datetime_tz(self):
        ...
        s = Series([Timestamp('2013-01-01 13:00:00-0800', tz='US/Pacific'), Timestamp('2013-01-02 14:00:00-0800', tz='US/Pacific')])
        assert (s.dtype == 'datetime64[ns, US/Pacific]')
        assert (lib.infer_dtype(s, skipna=True) == 'datetime64')
        s = Series([Timestamp('2013-01-01 13:00:00-0800', tz='US/Pacific'), Timestamp('2013-01-02 14:00:00-0800', tz='US/Eastern')])
        assert (s.dtype == 'object')
    ...
    def test_constructor_dict_multiindex(self):
        ...
        _d = sorted(d.items())
        result = Series(d)
        expected = Series([x[1] for x in _d], index=pd.MultiIndex.from_tuples([x[0] for x in _d]))
        tm.assert_series_equal(result, expected)
        d['z'] = 111.0
        _d.insert(0, ('z', d['z']))
        result = Series(d)
