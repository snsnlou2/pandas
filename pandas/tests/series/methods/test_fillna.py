...
import pytest
import pytz
from pandas import Categorical, DataFrame, DatetimeIndex, NaT, Period, Series, Timedelta, Timestamp, isna
...
class TestSeriesFillNA():
    ...
    def test_fillna_consistency(self):
        ...
        tm.assert_series_equal(result, expected)
        result = ser.where([True, False], Timestamp('20130101', tz='US/Eastern'), errors='ignore')
        tm.assert_series_equal(result, expected)
        result = ser.where([True, False], Timestamp('20130101', tz='US/Eastern'), errors='ignore')
        tm.assert_series_equal(result, expected)
    ...
    def test_timedelta_fillna(self, frame_or_series):
        ...
        expected = frame_or_series(expected)
        tm.assert_equal(result, expected)
        result = obj.fillna(np.timedelta64(int(1000000000.0)))
        expected = Series([timedelta(seconds=1), timedelta(0), timedelta(1), timedelta(days=1, seconds=(((9 * 3600) + 60) + 1))])
        expected = frame_or_series(expected)
        tm.assert_equal(result, expected)
    ...
    def test_datetime64_tz_fillna(self, tz):
        ser = Series([Timestamp('2011-01-01 10:00'), NaT, Timestamp('2011-01-03 10:00'), NaT])
        null_loc = Series([False, True, False, True])
        result = ser.fillna(Timestamp('2011-01-02 10:00'))
        expected = Series([Timestamp('2011-01-01 10:00'), Timestamp('2011-01-02 10:00'), Timestamp('2011-01-03 10:00'), Timestamp('2011-01-02 10:00')])
        ...
        expected = Series([Timestamp('2011-01-01 10:00'), Timestamp('2011-01-02 10:00'), Timestamp('2011-01-03 10:00'), Timestamp('2011-01-04 10:00')])
        tm.assert_series_equal(expected, result)
        tm.assert_series_equal(isna(ser), null_loc)
        ...
        tm.assert_series_equal(isna(ser), null_loc)
        result = ser.fillna(Timestamp('2011-01-02 10:00', tz=tz))
        idx = DatetimeIndex(['2011-01-01 10:00', '2011-01-02 10:00', '2011-01-03 10:00', '2011-01-02 10:00'], tz=tz)
        expected = Series(idx)
        tm.assert_series_equal(expected, result)
        tm.assert_series_equal(isna(ser), null_loc)
        ...
        expected = Series([Timestamp('2011-01-01 10:00', tz=tz), Timestamp('2011-01-02 10:00', tz=tz), Timestamp('2011-01-03 10:00', tz=tz), Timestamp('2011-01-04 10:00', tz=tz)])
        tm.assert_series_equal(expected, result)
        tm.assert_series_equal(isna(ser), null_loc)
        ...
        expected = Series([Timestamp('2011-01-01 10:00', tz=tz), Timestamp('2013-01-01', tz='US/Pacific'), Timestamp('2011-01-03 10:00', tz=tz), Timestamp('2013-01-01', tz='US/Pacific')])
        tm.assert_series_equal(expected, result)
        tm.assert_series_equal(isna(ser), null_loc)
