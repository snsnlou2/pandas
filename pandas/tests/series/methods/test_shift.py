...
import pandas as pd
from pandas import DatetimeIndex, Index, NaT, Series, TimedeltaIndex, date_range, offsets
import pandas._testing as tm
from pandas.tseries.offsets import BDay

class TestShift():

    @pytest.mark.parametrize('ser', [Series([np.arange(5)]), date_range('1/1/2011', periods=24, freq='H'), Series(range(5), index=date_range('2017', periods=5))])
    ...
    def test_shift(self, datetime_series):
        ...
        tm.assert_numpy_array_equal(unshifted.dropna().values, datetime_series.values[:(- 1)])
        offset = BDay()
    ...
    def test_shift_categorical_fill_value(self):
        ...
        expected = Series(pd.Categorical(['a', 'a', 'b', 'c'], categories=['a', 'b', 'c', 'd'], ordered=False))
    ...
    def test_shift_preserve_freqstr(self, periods):
        ser = Series(range(periods), index=pd.date_range('2016-1-1 00:00:00', periods=periods, freq='H'))
        result = ser.shift(1, '2H')
        expected = Series(range(periods), index=pd.date_range('2016-1-1 02:00:00', periods=periods, freq='H'))
        tm.assert_series_equal(result, expected)
