...
class TestSeriesDescribe():
    ...
    def test_describe(self):
        ...
        tm.assert_series_equal(result, expected)
        s = Series([Timedelta('1 days'), Timedelta('2 days'), Timedelta('3 days'), Timedelta('4 days'), Timedelta('5 days')], name='timedelta_data')
        result = s.describe()
        expected = Series([5, s[2], s.std(), s[0], s[1], s[2], s[3], s[4]], name='timedelta_data', index=['count', 'mean', 'std', 'min', '25%', '50%', '75%', 'max'])
        tm.assert_series_equal(result, expected)
        s = Series([Period('2020-01', 'M'), Period('2020-01', 'M'), Period('2019-12', 'M')], name='period_data')
        result = s.describe()
        expected = Series([3, 2, s[0], 2], name='period_data', index=['count', 'unique', 'top', 'freq'])
        tm.assert_series_equal(result, expected)
    ...
    def test_datetime_is_numeric_includes_datetime(self):
        ...
        result = s.describe(datetime_is_numeric=True)
        expected = Series([3, Timestamp('2012-01-02'), Timestamp('2012-01-01'), Timestamp('2012-01-01T12:00:00'), Timestamp('2012-01-02'), Timestamp('2012-01-02T12:00:00'), Timestamp('2012-01-03')], index=['count', 'mean', 'min', '25%', '50%', '75%', 'max'])
        tm.assert_series_equal(result, expected)
