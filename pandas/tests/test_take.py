...
@pytest.fixture(params=[(np.float64, True), (np.float32, True), (np.uint64, False), (np.uint32, False), (np.uint16, False), (np.uint8, False), (np.int64, False), (np.int32, False), (np.int16, False), (np.int8, False), (np.object_, True), (np.bool_, False)])
...
class TestExtensionTake():
    ...
    def test_take_empty(self, allow_fill):
        ...
        msg = 'cannot do a non-empty take from an empty axes.|indices are out-of-bounds'
        with pytest.raises(IndexError, match=msg):
