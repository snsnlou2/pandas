...
@pytest.fixture(params=[Series(([1, 2, 3] * 3), dtype='int32'), Series(([None, 2.5, 3.5] * 3), dtype='float32'), Series((['a', 'b', 'c'] * 3), dtype='category'), Series((['d', 'e', 'f'] * 3)), Series(([True, False, True] * 3)), Series(pd.date_range('20130101', periods=9)), Series(pd.date_range('20130101', periods=9, tz='US/Eastern')), Series(pd.timedelta_range('2000', periods=9))])
def series(request):
...
def test_consistency():
    ...
    expected = Series(np.array([3600424527151052760, 1374399572096150070, 477881037637427054], dtype='uint64'), index=['foo', 'bar', 'baz'])
...
@pytest.mark.parametrize('arr2', [np.array([3, 4, 'All']), np.array([3, 4, 'All'], dtype=object)])
...
@pytest.mark.parametrize('s1', [Series(['a', 'b', 'c', 'd']), Series([1000, 2000, 3000, 4000]), Series(pd.date_range(0, periods=4))])
