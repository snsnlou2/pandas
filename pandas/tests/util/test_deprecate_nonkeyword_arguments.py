...
def test_three_positional_argument_with_warning_message_analysis():
    with warnings.catch_warnings(record=True) as w:
        ...
        for actual_warning in w:
            ...
            assert (str(actual_warning.message) == "Starting with Pandas version 1.1 all arguments of g except for the argument 'a' will be keyword-only")
...
def test_one_positional_argument_with_warning_message_analysis():
    with warnings.catch_warnings(record=True) as w:
        ...
        for actual_warning in w:
            assert (actual_warning.category == FutureWarning)
            assert (str(actual_warning.message) == 'Starting with Pandas version 1.1 all arguments of h will be keyword-only')
