...
@pytest.mark.parametrize('a,b,check_less_precise', [(1.1, 1.1, False), (1.1, 1.100001, True), (1.1, 1.1001, 2)])
...
@pytest.mark.parametrize('a,b', [(np.inf, np.inf), (np.inf, float('inf')), (np.array([np.inf, np.nan, (- np.inf)]), np.array([np.inf, np.nan, (- np.inf)])), (np.array([np.inf, None, (- np.inf)], dtype=np.object_), np.array([np.inf, np.nan, (- np.inf)], dtype=np.object_))])
