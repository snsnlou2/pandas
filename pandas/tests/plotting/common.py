...
class TestPlotBase():
    ...
    def setup_method(self, method):
        ...
        with tm.RNGContext(42):
            ...
            self.hist_df = DataFrame({'gender': gender, 'classroom': classroom, 'height': np.random.normal(66, 4, size=n), 'weight': np.random.normal(161, 32, size=n), 'category': np.random.randint(4, size=n), 'datetime': to_datetime(np.random.randint(self.start_date_to_int64, self.end_date_to_int64, size=n, dtype=np.int64))})
    ...
    def _check_patches_all_filled(self, axes, filled=True):
