...
class TestDataFramePlots(TestPlotBase):
    ...
    def test_boxplot_numeric_data(self):
        df = DataFrame({'a': date_range('2012-01-01', periods=100), 'b': np.random.randn(100), 'c': (np.random.randn(100) + 2), 'd': date_range('2012-01-01', periods=100).astype(str), 'e': date_range('2012-01-01', periods=100, tz='UTC'), 'f': timedelta_range('1 days', periods=100)})
        ax = df.plot(kind='box')
...
class TestDataFrameGroupByPlots(TestPlotBase):
    ...
    def test_grouped_box_layout(self):
        ...
        self._check_axes_shape(self.plt.gcf().axes, axes_num=3, layout=(2, 2))
        with tm.assert_produces_warning(UserWarning):
            box = _check_plot_works(df.groupby('category').boxplot, column='height', layout=(3, 2), return_type='dict')
        self._check_axes_shape(self.plt.gcf().axes, axes_num=4, layout=(3, 2))
        with tm.assert_produces_warning(UserWarning):
            box = _check_plot_works(df.groupby('category').boxplot, column='height', layout=(3, (- 1)), return_type='dict')
        self._check_axes_shape(self.plt.gcf().axes, axes_num=4, layout=(3, 2))
        ...
        box = df.groupby('classroom').boxplot(column=['height', 'weight', 'category'], layout=(1, 4), return_type='dict')
        self._check_axes_shape(self.plt.gcf().axes, axes_num=3, layout=(1, 4))
        box = df.groupby('classroom').boxplot(column=['height', 'weight', 'category'], layout=(1, (- 1)), return_type='dict')
    ...
    def test_groupby_boxplot_subplots_false(self, col, expected_xticklabel):
        df = DataFrame({'cat': np.random.choice(list('abcde'), 100), 'v': np.random.rand(100), 'v1': np.random.rand(100)})
