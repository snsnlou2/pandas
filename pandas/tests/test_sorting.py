...
from pandas.core.sorting import decons_group_index, get_group_index, is_int64_overflow_possible, lexsort_indexer, nargsort
...
class TestSorting():
    ...
    def test_int64_overflow(self):
        ...
        A = np.arange(2500)
        df = DataFrame({'A': A, 'B': B, 'C': A, 'D': B, 'E': A, 'F': B, 'G': A, 'H': B, 'values': np.random.randn(2500)})
    ...
    def test_nargsort(self):
        ...
        exp = ((list(range(104, 4, (- 1))) + list(range(5))) + list(range(105, 110)))
        tm.assert_numpy_array_equal(result, np.array(exp), check_dtype=False)
        result = nargsort(items2, kind='mergesort', ascending=False, na_position='first')
        exp = ((list(range(5)) + list(range(105, 110))) + list(range(104, 4, (- 1))))
        tm.assert_numpy_array_equal(result, np.array(exp), check_dtype=False)
...
def test_decons():
    ...
    shape = (4, 5, 6)
    codes_list = [np.tile([0, 1, 2, 3, 0, 1, 2, 3], 100).astype(np.int64), np.tile([0, 2, 4, 3, 0, 1, 2, 3], 100).astype(np.int64), np.tile([5, 1, 0, 2, 3, 0, 5, 4], 100).astype(np.int64)]
    testit(codes_list, shape)
    shape = (10000, 10000)
    codes_list = [np.tile(np.arange(10000, dtype=np.int64), 5), np.tile(np.arange(10000, dtype=np.int64), 5)]
    testit(codes_list, shape)
