...
class TestAppend():
    ...
    indexes_can_append = [pd.RangeIndex(3), Index([4, 5, 6]), Index([4.5, 5.5, 6.5]), Index(list('abc')), pd.CategoricalIndex('A B C'.split()), pd.CategoricalIndex('D E F'.split(), ordered=True), pd.IntervalIndex.from_breaks([7, 8, 9, 10]), pd.DatetimeIndex([dt.datetime(2013, 1, 3, 0, 0), dt.datetime(2013, 1, 3, 6, 10), dt.datetime(2013, 1, 3, 7, 12)])]
    indexes_cannot_append_with_other = [pd.MultiIndex.from_arrays(['A B C'.split(), 'D E F'.split()])]
    ...
    def test_append_same_columns_type(self, index):
        ...
        expected = DataFrame([[1.0, 2.0, 3.0], [4, 5, 6], [7, 8, np.nan]], index=[0, 1, 2], columns=index)
        tm.assert_frame_equal(result, expected)
        ser_index = index
        index = index[:2]
    ...
    def test_append_empty_tz_frame_with_datetime64ns(self):
        ...
        tm.assert_frame_equal(result, expected)
        df = DataFrame(columns=['a']).astype('datetime64[ns, UTC]')
        result = df.append(Series({'a': pd.NaT}, dtype='datetime64[ns]'), ignore_index=True)
        expected = DataFrame({'a': [pd.NaT]}).astype('datetime64[ns]')
        tm.assert_frame_equal(result, expected)
