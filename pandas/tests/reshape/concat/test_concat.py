...
class TestConcatenate():
    ...
    def test_concat_mapping(self, mapping, non_dict_mapping_subclass):
        ...
        frames = constructor({'foo': DataFrame(np.random.randn(4, 3)), 'bar': DataFrame(np.random.randn(4, 3)), 'baz': DataFrame(np.random.randn(4, 3)), 'qux': DataFrame(np.random.randn(4, 3))})
    ...
    def test_concat_bug_3602(self):
        df1 = DataFrame({'firmNo': [0, 0, 0, 0], 'prc': [6, 6, 6, 6], 'stringvar': ['rrr', 'rrr', 'rrr', 'rrr']})
        df2 = DataFrame({'C': [9, 10, 11, 12], 'misc': [1, 2, 3, 4], 'prc': [6, 6, 6, 6]})
        expected = DataFrame([[0, 6, 'rrr', 9, 1, 6], [0, 6, 'rrr', 10, 2, 6], [0, 6, 'rrr', 11, 3, 6], [0, 6, 'rrr', 12, 4, 6]])
    ...
    def test_concat_ordered_dict(self):
        expected = pd.concat([Series(range(3)), Series(range(4))], keys=['First', 'Another'])
...
def test_concat_no_unnecessary_upcast(dt, pdt):
    ...
    dfs = [pdt(np.array([1], dtype=dt, ndmin=dims)), pdt(np.array([np.nan], dtype=dt, ndmin=dims)), pdt(np.array([5], dtype=dt, ndmin=dims))]
    x = pd.concat(dfs)
