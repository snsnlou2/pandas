...
class TestEmptyConcat():
    ...
    def test_concat_empty_series(self):
        ...
        exp = DataFrame({'x': [1, 2, 3], 'y': [np.nan, np.nan, np.nan]}, index=Index([0, 1, 2], dtype='O'))
        ...
        res = pd.concat([s1, s2], axis=1)
        exp = DataFrame({'x': [1, 2, 3], 0: [np.nan, np.nan, np.nan]}, columns=['x', 0], index=Index([0, 1, 2], dtype='O'))
        tm.assert_frame_equal(res, exp)
    ...
    @pytest.mark.parametrize('left,right,expected', [(np.bool_, np.int32, np.int32), (np.bool_, np.float32, np.object_), ('m8[ns]', np.bool_, np.object_), ('m8[ns]', np.int64, np.object_), ('M8[ns]', np.bool_, np.object_), ('M8[ns]', np.int64, np.object_), ('category', 'category', 'category'), ('category', 'object', 'object')])
