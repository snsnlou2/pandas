...
class TestJoin():
    ...
    def test_join_on(self):
        ...
        source_copy['A'] = 0
        msg = 'You are trying to merge on float64 and object columns. If you wish to proceed you should use pd.concat'
    ...
    def test_join_datetime_string(self):
        ...
        expected = DataFrame([[pd.Timestamp('2012-08-02 00:00:00'), 'J', 1, 15], [pd.Timestamp('2013-04-06 00:00:00'), 'L', 2, 20]], index=[2, 4], columns=['x', 'y', 'z', 'a'])
        tm.assert_frame_equal(result, expected)
...
def test_join_inner_multiindex_deterministic_order():
    ...
    expected = DataFrame({'e': [5], 'f': [6]}, index=MultiIndex.from_tuples([(2, 1, 4, 3)], names=('b', 'a', 'd', 'c')))
    tm.assert_frame_equal(result, expected)
