...
def right():
    ...
    index = MultiIndex(levels=[['foo', 'bar', 'baz', 'qux'], ['one', 'two', 'three']], codes=[[0, 0, 0, 1, 1, 2, 2, 3, 3, 3], [0, 1, 2, 0, 1, 1, 2, 0, 1, 2]], names=['key1', 'key2'])
    return DataFrame(np.random.randn(10, 3), index=index, columns=['j_one', 'j_two', 'j_three'])
...
class TestMergeMulti():
    ...
    def test_left_join_multi_index(self, left, right, sort):
        ...
        def bind_cols(df):
            ...
            return ((f(df['1st']) + (f(df['3rd']) * 100.0)) + (df['2nd'].fillna(0) * 10000.0))
    ...
    def test_merge_multiple_cols_with_mixed_cols_index(self):
        s = pd.Series(range(6), MultiIndex.from_product([['A', 'B'], [1, 2, 3]], names=['lev1', 'lev2']), name='Amount')
    ...
    def test_left_merge_na_buglet(self):
        left = DataFrame({'id': list('abcde'), 'v1': np.random.randn(5), 'v2': np.random.randn(5), 'dummy': list('abcde'), 'v3': np.random.randn(5)}, columns=['id', 'v1', 'v2', 'dummy', 'v3'])
    ...
    def test_merge_datetime_multi_index_empty_df(self, merge_type):
        ...
        if (merge_type == 'left'):
            expected = DataFrame(data={'data': [1.5, 1.5], 'state': [None, None]}, index=expected_index)
            results_merge = left.merge(right, how='left', on=['date', 'panel'])
            results_join = left.join(right, how='left')
        else:
            expected = DataFrame(data={'state': [None, None], 'data': [1.5, 1.5]}, index=expected_index)
    ...
    def test_join_multi_levels2(self):
        ...
        tm.assert_frame_equal(result, expected)
        expected = DataFrame({'household_id': [1, 2, 2, 2, 2, 3, 3, 3, 3, 3, 3, 4], 'asset_id': ['nl0000301109', 'nl0000301109', 'gb00b03mlx29', 'gb00b03mlx29', 'gb00b03mlx29', 'gb00b03mlx29', 'gb00b03mlx29', 'gb00b03mlx29', 'lu0197800237', 'lu0197800237', 'nl0000289965', None], 't': [None, None, 233, 234, 235, 233, 234, 235, 180, 181, None, None], 'share': [1.0, 0.4, 0.6, 0.6, 0.6, 0.15, 0.15, 0.15, 0.6, 0.6, 0.25, 1.0], 'log_return': [None, None, 0.09604978, (- 0.06524096), 0.03532373, 0.09604978, (- 0.06524096), 0.03532373, 0.03025441, 0.036997, None, None]}).set_index(['household_id', 'asset_id', 't']).reindex(columns=['share', 'log_return'])
...
class TestJoinMultiMulti():
    ...
    def test_single_common_level(self):
        ...
        expected = pd.merge(left.reset_index(), right.reset_index(), on=['key'], how='inner').set_index(['key', 'X', 'Y'])
        tm.assert_frame_equal(result, expected)
