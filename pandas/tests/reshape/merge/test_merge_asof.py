...
class TestAsOfMerge():
    ...
    def test_examples2(self):
        ' doc-string examples '
        trades = pd.DataFrame({'time': pd.to_datetime(['20160525 13:30:00.023', '20160525 13:30:00.038', '20160525 13:30:00.048', '20160525 13:30:00.048', '20160525 13:30:00.048']), 'ticker': ['MSFT', 'MSFT', 'GOOG', 'GOOG', 'AAPL'], 'price': [51.95, 51.95, 720.77, 720.92, 98.0], 'quantity': [75, 155, 100, 100, 100]}, columns=['time', 'ticker', 'price', 'quantity'])
        quotes = pd.DataFrame({'time': pd.to_datetime(['20160525 13:30:00.023', '20160525 13:30:00.023', '20160525 13:30:00.030', '20160525 13:30:00.041', '20160525 13:30:00.048', '20160525 13:30:00.049', '20160525 13:30:00.072', '20160525 13:30:00.075']), 'ticker': ['GOOG', 'MSFT', 'MSFT', 'MSFT', 'GOOG', 'AAPL', 'GOOG', 'MSFT'], 'bid': [720.5, 51.95, 51.97, 51.99, 720.5, 97.99, 720.5, 52.01], 'ask': [720.93, 51.96, 51.98, 52.0, 720.93, 98.01, 720.88, 52.03]}, columns=['time', 'ticker', 'bid', 'ask'])
        pd.merge_asof(trades, quotes, on='time', by='ticker')
        pd.merge_asof(trades, quotes, on='time', by='ticker', tolerance=Timedelta('2ms'))
        expected = pd.DataFrame({'time': pd.to_datetime(['20160525 13:30:00.023', '20160525 13:30:00.038', '20160525 13:30:00.048', '20160525 13:30:00.048', '20160525 13:30:00.048']), 'ticker': ['MSFT', 'MSFT', 'GOOG', 'GOOG', 'AAPL'], 'price': [51.95, 51.95, 720.77, 720.92, 98.0], 'quantity': [75, 155, 100, 100, 100], 'bid': [np.nan, 51.97, np.nan, np.nan, np.nan], 'ask': [np.nan, 51.98, np.nan, np.nan, np.nan]}, columns=['time', 'ticker', 'price', 'quantity', 'bid', 'ask'])
        result = pd.merge_asof(trades, quotes, on='time', by='ticker', tolerance=Timedelta('10ms'), allow_exact_matches=False)
        tm.assert_frame_equal(result, expected)
    ...
    def test_multiby(self):
        trades = pd.DataFrame({'time': pd.to_datetime(['20160525 13:30:00.023', '20160525 13:30:00.023', '20160525 13:30:00.046', '20160525 13:30:00.048', '20160525 13:30:00.050']), 'ticker': ['MSFT', 'MSFT', 'GOOG', 'GOOG', 'AAPL'], 'exch': ['ARCA', 'NSDQ', 'NSDQ', 'BATS', 'NSDQ'], 'price': [51.95, 51.95, 720.77, 720.92, 98.0], 'quantity': [75, 155, 100, 100, 100]}, columns=['time', 'ticker', 'exch', 'price', 'quantity'])
        ...
        expected = pd.DataFrame({'time': pd.to_datetime(['20160525 13:30:00.023', '20160525 13:30:00.023', '20160525 13:30:00.046', '20160525 13:30:00.048', '20160525 13:30:00.050']), 'ticker': ['MSFT', 'MSFT', 'GOOG', 'GOOG', 'AAPL'], 'exch': ['ARCA', 'NSDQ', 'NSDQ', 'BATS', 'NSDQ'], 'price': [51.95, 51.95, 720.77, 720.92, 98.0], 'quantity': [75, 155, 100, 100, 100], 'bid': [np.nan, 51.95, 720.5, 720.51, np.nan], 'ask': [np.nan, 51.96, 720.93, 720.92, np.nan]}, columns=['time', 'ticker', 'exch', 'price', 'quantity', 'bid', 'ask'])
        result = pd.merge_asof(trades, quotes, on='time', by=['ticker', 'exch'])
        tm.assert_frame_equal(result, expected)

    def test_multiby_heterogeneous_types(self):
        trades = pd.DataFrame({'time': pd.to_datetime(['20160525 13:30:00.023', '20160525 13:30:00.023', '20160525 13:30:00.046', '20160525 13:30:00.048', '20160525 13:30:00.050']), 'ticker': [0, 0, 1, 1, 2], 'exch': ['ARCA', 'NSDQ', 'NSDQ', 'BATS', 'NSDQ'], 'price': [51.95, 51.95, 720.77, 720.92, 98.0], 'quantity': [75, 155, 100, 100, 100]}, columns=['time', 'ticker', 'exch', 'price', 'quantity'])
        ...
        expected = pd.DataFrame({'time': pd.to_datetime(['20160525 13:30:00.023', '20160525 13:30:00.023', '20160525 13:30:00.046', '20160525 13:30:00.048', '20160525 13:30:00.050']), 'ticker': [0, 0, 1, 1, 2], 'exch': ['ARCA', 'NSDQ', 'NSDQ', 'BATS', 'NSDQ'], 'price': [51.95, 51.95, 720.77, 720.92, 98.0], 'quantity': [75, 155, 100, 100, 100], 'bid': [np.nan, 51.95, 720.5, 720.51, np.nan], 'ask': [np.nan, 51.96, 720.93, 720.92, np.nan]}, columns=['time', 'ticker', 'exch', 'price', 'quantity', 'bid', 'ask'])
        result = pd.merge_asof(trades, quotes, on='time', by=['ticker', 'exch'])
        tm.assert_frame_equal(result, expected)
    ...
    @pytest.mark.parametrize('tolerance', [Timedelta('1day'), datetime.timedelta(days=1)], ids=['Timedelta', 'datetime.timedelta'])
    ...
    def test_index_tolerance(self):
        ...
        trades = self.trades.set_index('time')
        quotes = self.quotes.set_index('time')
        result = pd.merge_asof(trades, quotes, left_index=True, right_index=True, by='ticker', tolerance=Timedelta('1day'))
        tm.assert_frame_equal(result, expected)
    ...
    def test_allow_exact_matches_forward(self):
        ...
        result = pd.merge_asof(left, right, on='a', direction='forward', allow_exact_matches=False)
        tm.assert_frame_equal(result, expected)
    ...
    def test_allow_exact_matches_nearest(self):
        ...
        result = pd.merge_asof(left, right, on='a', direction='nearest', allow_exact_matches=False)
        tm.assert_frame_equal(result, expected)
    ...
    def test_allow_exact_matches_and_tolerance2(self):
        ...
        expected = pd.DataFrame({'time': pd.to_datetime(['2016-07-15 13:30:00.030']), 'username': ['bob'], 'version': [2]})
        tm.assert_frame_equal(result, expected)
        result = pd.merge_asof(df1, df2, on='time', allow_exact_matches=False)
        expected = pd.DataFrame({'time': pd.to_datetime(['2016-07-15 13:30:00.030']), 'username': ['bob'], 'version': [1]})
        tm.assert_frame_equal(result, expected)
        result = pd.merge_asof(df1, df2, on='time', allow_exact_matches=False, tolerance=Timedelta('10ms'))
        expected = pd.DataFrame({'time': pd.to_datetime(['2016-07-15 13:30:00.030']), 'username': ['bob'], 'version': [np.nan]})
        tm.assert_frame_equal(result, expected)
    ...
    def test_allow_exact_matches_and_tolerance_forward(self):
        ...
        result = pd.merge_asof(left, right, on='a', direction='forward', allow_exact_matches=False, tolerance=1)
        tm.assert_frame_equal(result, expected)
    ...
    def test_by_int(self):
        df1 = pd.DataFrame({'time': pd.to_datetime(['20160525 13:30:00.020', '20160525 13:30:00.030', '20160525 13:30:00.040', '20160525 13:30:00.050', '20160525 13:30:00.060']), 'key': [1, 2, 1, 3, 2], 'value1': [1.1, 1.2, 1.3, 1.4, 1.5]}, columns=['time', 'key', 'value1'])
        df2 = pd.DataFrame({'time': pd.to_datetime(['20160525 13:30:00.015', '20160525 13:30:00.020', '20160525 13:30:00.025', '20160525 13:30:00.035', '20160525 13:30:00.040', '20160525 13:30:00.055', '20160525 13:30:00.060', '20160525 13:30:00.065']), 'key': [2, 1, 1, 3, 2, 1, 2, 3], 'value2': [2.1, 2.2, 2.3, 2.4, 2.5, 2.6, 2.7, 2.8]}, columns=['time', 'key', 'value2'])
        result = pd.merge_asof(df1, df2, on='time', by='key')
        expected = pd.DataFrame({'time': pd.to_datetime(['20160525 13:30:00.020', '20160525 13:30:00.030', '20160525 13:30:00.040', '20160525 13:30:00.050', '20160525 13:30:00.060']), 'key': [1, 2, 1, 3, 2], 'value1': [1.1, 1.2, 1.3, 1.4, 1.5], 'value2': [2.2, 2.1, 2.3, 2.4, 2.7]}, columns=['time', 'key', 'value1', 'value2'])
        tm.assert_frame_equal(result, expected)
    ...
    def test_timedelta_tolerance_nearest(self):
        ...
        result = pd.merge_asof(left, right, on='time', tolerance=Timedelta('1ms'), direction='nearest')
        tm.assert_frame_equal(result, expected)
    ...
    def test_merge_index_column_tz(self):
        ...
        result = pd.merge_asof(left=left, right=right, left_index=True, right_on=['from_date'])
        expected = pd.DataFrame({'xyz': [0.9, 0.8, 0.7, 0.6], 'from_date': index[1:], 'abc': (([2.46] * 3) + [2.19])}, index=pd.Index([1, 2, 3, 4]))
        tm.assert_frame_equal(result, expected)
        result = pd.merge_asof(left=right, right=left, right_index=True, left_on=['from_date'])
        expected = pd.DataFrame({'from_date': index, 'abc': (([2.46] * 4) + [2.19]), 'xyz': [np.nan, 0.9, 0.8, 0.7, 0.6]}, index=pd.Index([0, 1, 2, 3, 4]))
        tm.assert_frame_equal(result, expected)

    def test_left_index_right_index_tolerance(self):
        dr1 = (pd.date_range(start='1/1/2020', end='1/20/2020', freq='2D') + Timedelta(seconds=0.4))
        ...
        result = pd.merge_asof(df1, df2, left_index=True, right_index=True, tolerance=Timedelta(seconds=0.5))
        tm.assert_frame_equal(result, expected)
