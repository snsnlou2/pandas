...
import pandas as pd
from pandas import Categorical, CategoricalIndex, DataFrame, DatetimeIndex, Float64Index, Int64Index, IntervalIndex, MultiIndex, PeriodIndex, RangeIndex, Series, TimedeltaIndex, UInt64Index
...
class TestMerge():
    ...
    def test_handle_join_key_pass_array(self):
        left = DataFrame({'key': [1, 1, 2, 2, 3], 'value': np.arange(5)}, columns=['value', 'key'])
        ...
        tm.assert_series_equal(merged['key_0'], Series([1, 1, 1, 1, 2, 2, 3, 4, 5], name='key_0'))
    ...
    def test_merge_left_empty_right_empty(self, join_type, kwarg):
        ...
        exp_in = DataFrame(columns=['a', 'b', 'c', 'x', 'y', 'z'], index=pd.Index([], dtype=object), dtype=object)
    ...
    def test_merge_left_notempty_right_empty(self):
        ...
        right = DataFrame(columns=['x', 'y', 'z'])
        exp_out = DataFrame({'a': [1, 4, 7], 'b': [2, 5, 8], 'c': [3, 6, 9], 'x': np.array(([np.nan] * 3), dtype=object), 'y': np.array(([np.nan] * 3), dtype=object), 'z': np.array(([np.nan] * 3), dtype=object)}, columns=['a', 'b', 'c', 'x', 'y', 'z'])
        exp_in = exp_out[0:0]
        exp_in.index = exp_in.index.astype(object)
    ...
    def test_merge_empty_frame(self, series_of_dtype, series_of_dtype2):
        ...
        df_empty = df[:0]
        expected = DataFrame({'value_x': Series(dtype=df.dtypes['value']), 'key': Series(dtype=df.dtypes['key']), 'value_y': Series(dtype=df.dtypes['value'])}, columns=['value_x', 'key', 'value_y'])
    ...
    def test_other_datetime_unit(self):
        ...
        for dtype in ['datetime64[D]', 'datetime64[h]', 'datetime64[m]', 'datetime64[s]', 'datetime64[ms]', 'datetime64[us]', 'datetime64[ns]']:
            ...
            exp = DataFrame({'entity_id': [101, 102], 'days': np.array(['nat', 'nat'], dtype='datetime64[ns]')}, columns=['entity_id', 'days'])
            tm.assert_frame_equal(result, exp)
    ...
    def test_overlapping_columns_error_message(self):
        ...
        msg = "Data columns not unique: Index\\(\\['foo'\\], dtype='object'\\)"
        with pytest.raises(MergeError, match=msg):
            merge(df, df2)
    ...
    def test_merge_on_datetime64tz_empty(self):
        ...
        right = DataFrame({'date': [pd.Timestamp('2018', tz=dtz.tz)], 'value': [4.0], 'date2': [pd.Timestamp('2019', tz=dtz.tz)]}, columns=['date', 'value', 'date2'])
        left = right[:0]
        result = left.merge(right, on='date')
        expected = DataFrame({'value_x': Series(dtype=float), 'date2_x': Series(dtype=dtz), 'date': Series(dtype=dtz), 'value_y': Series(dtype=float), 'date2_y': Series(dtype=dtz)}, columns=['value_x', 'date2_x', 'date', 'value_y', 'date2_y'])
        tm.assert_frame_equal(result, expected)

    def test_merge_datetime64tz_with_dst_transition(self):
        df1 = DataFrame(pd.date_range('2017-10-29 01:00', periods=4, freq='H', tz='Europe/Madrid'), columns=['date'])
        df1['value'] = 1
        df2 = DataFrame({'date': pd.to_datetime(['2017-10-29 03:00:00', '2017-10-29 04:00:00', '2017-10-29 05:00:00']), 'value': 2})
        df2['date'] = df2['date'].dt.tz_localize('UTC').dt.tz_convert('Europe/Madrid')
        result = pd.merge(df1, df2, how='outer', on='date')
        expected = DataFrame({'date': pd.date_range('2017-10-29 01:00', periods=7, freq='H', tz='Europe/Madrid'), 'value_x': (([1] * 4) + ([np.nan] * 3)), 'value_y': (([np.nan] * 4) + ([2] * 3))})
        tm.assert_frame_equal(result, expected)
    ...
    def test_validation(self):
        ...
        left_w_dups = left.append(DataFrame({'a': ['a'], 'c': ['cow']}, index=[3]), sort=True)
...
class TestMergeDtypes():
    ...
    def test_merge_incompat_dtypes_error(self, df1_vals, df2_vals):
        df1 = DataFrame({'A': df1_vals})
        df2 = DataFrame({'A': df2_vals})
        msg = f"You are trying to merge on {df1['A'].dtype} and {df2['A'].dtype} columns. If you wish to proceed you should use pd.concat"
        msg = re.escape(msg)
        with pytest.raises(ValueError, match=msg):
            pd.merge(df1, df2, on=['A'])
        msg = f"You are trying to merge on {df2['A'].dtype} and {df1['A'].dtype} columns. If you wish to proceed you should use pd.concat"
        msg = re.escape(msg)
        with pytest.raises(ValueError, match=msg):
...
def left():
    np.random.seed(1234)
    return DataFrame({'X': Series(np.random.choice(['foo', 'bar'], size=(10,))).astype(CDT(['foo', 'bar'])), 'Y': np.random.choice(['one', 'two', 'three'], size=(10,))})
...
class TestMergeCategorical():
    ...
    def test_identical(self, left):
        ...
        expected = Series([CategoricalDtype(categories=['foo', 'bar']), np.dtype('O'), np.dtype('O')], index=['X', 'Y_x', 'Y_y'])
        tm.assert_series_equal(result, expected)

    def test_basic(self, left, right):
        merged = pd.merge(left, right, on='X')
        result = merged.dtypes.sort_index()
        expected = Series([CategoricalDtype(categories=['foo', 'bar']), np.dtype('O'), np.dtype('int64')], index=['X', 'Y', 'Z'])
        tm.assert_series_equal(result, expected)
    ...
    def test_other_columns(self, left, right):
        ...
        merged = pd.merge(left, right, on='X')
        result = merged.dtypes.sort_index()
        expected = Series([CategoricalDtype(categories=['foo', 'bar']), np.dtype('O'), CategoricalDtype(categories=[1, 2])], index=['X', 'Y', 'Z'])
        tm.assert_series_equal(result, expected)
...
class TestMergeOnIndexes():
    ...
    def test_merge_on_indexes(self, left_df, right_df, how, sort, expected):
        result = pd.merge(left_df, right_df, left_index=True, right_index=True, how=how, sort=sort)
        tm.assert_frame_equal(result, expected)

@pytest.mark.parametrize('index', [CategoricalIndex(['A', 'B'], categories=['A', 'B'], name='index_col'), Float64Index([1.0, 2.0], name='index_col'), Int64Index([1, 2], name='index_col'), UInt64Index([1, 2], name='index_col'), RangeIndex(start=0, stop=2, name='index_col'), DatetimeIndex(['2018-01-01', '2018-01-02'], name='index_col')], ids=(lambda x: type(x).__name__))
...
def test_merge_series(on, left_on, right_on, left_index, right_index, nm):
    a = DataFrame({'A': [1, 2, 3, 4]}, index=pd.MultiIndex.from_product([['a', 'b'], [0, 1]], names=['outer', 'inner']))
    b = Series([1, 2, 3, 4], index=pd.MultiIndex.from_product([['a', 'b'], [1, 2]], names=['outer', 'inner']), name=nm)
    expected = DataFrame({'A': [2, 4], 'B': [1, 3]}, index=pd.MultiIndex.from_product([['a', 'b'], [1]], names=['outer', 'inner']))
    if (nm is not None):
        result = pd.merge(a, b, on=on, left_on=left_on, right_on=right_on, left_index=left_index, right_index=right_index)
        tm.assert_frame_equal(result, expected)
    else:
...
@pytest.mark.parametrize('how,expected', [('right', DataFrame({'A': [100, 200, 300], 'B1': [60, 70, np.nan], 'B2': [600, 700, 800]})), ('outer', DataFrame({'A': [100, 200, 1, 300], 'B1': [60, 70, 80, np.nan], 'B2': [600, 700, np.nan, 800]}))])
...
def test_merge_multiindex_columns():
    ...
    expected_index = pd.MultiIndex.from_product([expected_labels, numbers], names=['outer', 'inner'])
...
def test_categorical_non_unique_monotonic(n_categories):
    ...
    df2 = DataFrame([[6]], columns=['value'], index=CategoricalIndex([0], categories=np.arange(n_categories)))
