...
class TestPivotTable():
    ...
    def setup_method(self, method):
        self.data = DataFrame({'A': ['foo', 'foo', 'foo', 'foo', 'bar', 'bar', 'bar', 'bar', 'foo', 'foo', 'foo'], 'B': ['one', 'one', 'one', 'two', 'one', 'one', 'one', 'two', 'two', 'two', 'one'], 'C': ['dull', 'dull', 'shiny', 'dull', 'dull', 'shiny', 'shiny', 'dull', 'shiny', 'shiny', 'shiny'], 'D': np.random.randn(11), 'E': np.random.randn(11), 'F': np.random.randn(11)})
    ...
    def test_pivot_table_dropna_categoricals(self, dropna):
        categories = ['a', 'b', 'c', 'd']
        df = DataFrame({'A': ['a', 'a', 'a', 'b', 'b', 'b', 'c', 'c', 'c'], 'B': [1, 2, 3, 1, 2, 3, 1, 2, 3], 'C': range(0, 9)})
        ...
        expected = DataFrame([[0, 3, 6], [1, 4, 7], [2, 5, 8]], index=expected_index, columns=expected_columns)
        if (not dropna):
            expected = expected.reindex(columns=Categorical(categories)).astype('float')
        tm.assert_frame_equal(result, expected)

    def test_pivot_with_non_observable_dropna(self, dropna):
        df = DataFrame({'A': Categorical([np.nan, 'low', 'high', 'low', 'high'], categories=['low', 'high'], ordered=True), 'B': range(5)})
        result = df.pivot_table(index='A', values='B', dropna=dropna)
        expected = DataFrame({'B': [2, 3]}, index=Index(Categorical.from_codes([0, 1], categories=['low', 'high'], ordered=True), name='A'))
        tm.assert_frame_equal(result, expected)
        df = DataFrame({'A': Categorical(['left', 'low', 'high', 'low', 'high'], categories=['low', 'high', 'left'], ordered=True), 'B': range(5)})
        result = df.pivot_table(index='A', values='B', dropna=dropna)
        expected = DataFrame({'B': [2, 3, 0]}, index=Index(Categorical.from_codes([0, 1, 2], categories=['low', 'high', 'left'], ordered=True), name='A'))
        tm.assert_frame_equal(result, expected)
    ...
    def test_pivot_no_values(self):
        ...
        df = DataFrame({'A': [1, 2, 3, 4, 5], 'dt': pd.date_range('2011-01-01', freq='D', periods=5)}, index=idx)
    ...
    def test_pivot_multi_functions(self):
        ...
        tm.assert_frame_equal(result, expected)
        f = (lambda func: pivot_table(self.data, values=['D', 'E'], index=['A', 'B'], columns='C', aggfunc=func, margins=True))
        result = f([np.mean, np.std])
        means = f(np.mean)
        stds = f(np.std)
        expected = concat([means, stds], keys=['mean', 'std'], axis=1)
    ...
    @pytest.mark.parametrize('columns, aggfunc, values, expected_columns', [('A', np.mean, [[5.5, 5.5, 2.2, 2.2], [8.0, 8.0, 4.4, 4.4]], Index(['bar', 'All', 'foo', 'All'], name='A')), (['A', 'B'], 'sum', [[9, 13, 22, 5, 6, 11], [14, 18, 32, 11, 11, 22]], MultiIndex.from_tuples([('bar', 'one'), ('bar', 'two'), ('bar', 'All'), ('foo', 'one'), ('foo', 'two'), ('foo', 'All')], names=['A', 'B']))])
    ...
    def test_margins_dtype(self):
        ...
        result = df.pivot_table(values='D', index=['A', 'B'], columns='C', margins=True, aggfunc=np.sum, fill_value=0)
        tm.assert_frame_equal(expected, result)
    ...
    def test_pivot_table_multiindex_only(self, cols):
        ...
        expected = DataFrame([[4, 5, 6]], columns=MultiIndex.from_tuples([(1, 1), (2, 2), (3, 3)], names=cols), index=Index(['v']))
        tm.assert_frame_equal(result, expected)
    ...
    def test_pivot_no_level_overlap(self):
        data = DataFrame({'a': (['a', 'a', 'a', 'a', 'b', 'b', 'b', 'b'] * 2), 'b': ([0, 0, 0, 0, 1, 1, 1, 1] * 2), 'c': (((['foo'] * 4) + (['bar'] * 4)) * 2), 'value': np.random.randn(16)})
    ...
    def test_pivot_timegrouper(self):
        ...
        expected = DataFrame(np.array([10, 18, 3], dtype='int64').reshape(1, 3), index=pd.DatetimeIndex([datetime(2013, 12, 31)], freq='A'), columns='Carl Joe Mark'.split())
        ...
        expected = DataFrame(np.array([np.nan, 3, np.nan, np.nan, 6, np.nan, 1, 9, np.nan, 9, np.nan, np.nan, np.nan, np.nan, 3, np.nan]).reshape(4, 4), index=pd.DatetimeIndex([datetime(2013, 9, 30), datetime(2013, 10, 31), datetime(2013, 11, 30), datetime(2013, 12, 31)], freq='M'), columns=pd.DatetimeIndex([datetime(2013, 9, 30), datetime(2013, 10, 31), datetime(2013, 11, 30), datetime(2013, 12, 31)], freq='M'))
        expected.index.name = 'Date'
    ...
    def test_pivot_datetime_tz(self):
        ...
        expected = DataFrame(np.array([[0, 3, 1, 2, 0, 3, 1, 2], [1, 4, 2, 1, 1, 4, 2, 1], [2, 5, 1, 2, 2, 5, 1, 2]], dtype='int64'), index=exp_idx, columns=exp_col)
    ...
    def test_pivot_dtaccessor(self):
        ...
        expected = DataFrame({7: [0, 3], 8: [1, 4], 9: [2, 5]}, index=Index([1, 2], name='dt2'), columns=Index([7, 8, 9], name='dt1'))
        tm.assert_frame_equal(result, expected)
    ...
    def test_pivot_table_margins_name_with_aggfunc_list(self):
        margins_name = 'Weekly'
        costs = DataFrame({'item': ['bacon', 'cheese', 'bacon', 'cheese'], 'cost': [2.5, 4.5, 3.2, 3.3], 'day': ['M', 'M', 'T', 'T']})
    ...
    def test_margins_casted_to_float(self, observed):
        ...
        expected = DataFrame({'A': [3, 7, 5], 'B': [2.5, 6.5, 4.5], 'C': [2, 5, 3.5]}, index=Index(['X', 'Y', 'All'], name='D'))
        tm.assert_frame_equal(result, expected)
