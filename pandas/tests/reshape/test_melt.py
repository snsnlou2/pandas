...
class TestMelt():
    ...
    def setup_method(self, method):
        ...
        self.var_name = 'var'
        self.value_name = 'val'
        self.df1 = DataFrame([[1.067683, (- 1.110463), 0.20867], [(- 1.321405), 0.368915, (- 1.055342)], [(- 0.807333), 0.08298, (- 0.873361)]])
    ...
    def test_single_vars_work_with_multiindex(self, id_vars, value_vars, col_level, expected):
...
class TestWideToLong():
    ...
    def test_nonnumeric_suffix(self):
        ...
        expected = DataFrame({'A': ['X1', 'X1', 'X2', 'X2'], 'colname': ['placebo', 'test', 'placebo', 'test'], 'result': [5.0, np.nan, 6.0, np.nan], 'treatment': [1.0, 3.0, 2.0, 4.0]})
    ...
    def test_float_suffix(self):
        ...
        expected = DataFrame({'A': ['X1', 'X1', 'X1', 'X1', 'X2', 'X2', 'X2', 'X2'], 'colname': [1, 1.1, 1.2, 2.1, 1, 1.1, 1.2, 2.1], 'result': [0.0, np.nan, 5.0, np.nan, 9.0, np.nan, 6.0, np.nan], 'treatment': [np.nan, 1.0, np.nan, 3.0, np.nan, 2.0, np.nan, 4.0]})
        expected = expected.set_index(['A', 'colname'])
    ...
    def test_col_substring_of_stubname(self):
        ...
        expected = pd.wide_to_long(wide_df, stubnames=['PA'], i=['node_id', 'A'], j='time')
    ...
    def test_warn_of_column_name_value(self):
        ...
        expected = DataFrame([['A', 'col', 'A'], ['B', 'col', 'B'], ['C', 'col', 'C']], columns=['value', 'variable', 'value'])
        with tm.assert_produces_warning(FutureWarning):
