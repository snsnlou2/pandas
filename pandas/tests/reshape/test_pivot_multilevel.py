...
def test_pivot_list_like_index(input_index, input_columns, input_values, expected_values, expected_columns, expected_index):
...
def test_pivot_multiindexed_rows_and_cols():
    ...
    expected = pd.DataFrame(data=[[5.0, np.nan], [10.0, 7.0]], columns=MultiIndex.from_tuples([(0, 1, 0), (0, 1, 1)], names=['col_L0', 'col_L1', 'idx_L1']), index=Int64Index([0, 1], dtype='int64', name='idx_L0'))
