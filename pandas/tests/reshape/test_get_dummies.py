...
class TestGetDummies():
    ...
    def test_get_dummies_unicode(self, sparse):
        ...
        exp = DataFrame({'letter_e': [1, 0, 0], f'letter_{eacute}': [0, 1, 1]}, dtype=np.uint8)
        if sparse:
    ...
    def test_dataframe_dummies_all_obj(self, df, sparse):
        ...
        if sparse:
            expected = DataFrame({'A_a': SparseArray([1, 0, 1], dtype='uint8'), 'A_b': SparseArray([0, 1, 0], dtype='uint8'), 'B_b': SparseArray([1, 1, 0], dtype='uint8'), 'B_c': SparseArray([0, 0, 1], dtype='uint8')})
        tm.assert_frame_equal(result, expected)
    ...
    def test_dataframe_dummies_prefix_str(self, df, sparse):
        ...
        if sparse:
            expected = pd.concat([Series([1, 2, 3], name='C'), Series([1, 0, 1], name='bad_a', dtype='Sparse[uint8]'), Series([0, 1, 0], name='bad_b', dtype='Sparse[uint8]'), Series([1, 1, 0], name='bad_b', dtype='Sparse[uint8]'), Series([0, 0, 1], name='bad_c', dtype='Sparse[uint8]')], axis=1)
        tm.assert_frame_equal(result, expected)
    ...
    @pytest.mark.parametrize('get_dummies_kwargs,expected', [({'data': DataFrame({'ä': ['a']})}, DataFrame({'ä_a': [1]}, dtype=np.uint8)), ({'data': DataFrame({'x': ['ä']})}, DataFrame({'x_ä': [1]}, dtype=np.uint8)), ({'data': DataFrame({'x': ['a']}), 'prefix': 'ä'}, DataFrame({'ä_a': [1]}, dtype=np.uint8)), ({'data': DataFrame({'x': ['a']}), 'prefix_sep': 'ä'}, DataFrame({'xäa': [1]}, dtype=np.uint8))])
    ...
    def test_get_dummies_int_int(self):
        ...
        result = get_dummies(data)
        expected = DataFrame([[1, 0], [0, 1], [1, 0]], columns=Categorical(['a', 'b']), dtype=np.uint8)
        tm.assert_frame_equal(result, expected)
    ...
    def test_get_dummies_all_sparse(self):
        ...
        expected = DataFrame({'A_1': SparseArray([1, 0], dtype=dtype), 'A_2': SparseArray([0, 1], dtype=dtype)})
        tm.assert_frame_equal(result, expected)
