...
import pytest
import pandas as pd
from pandas import Categorical, DataFrame, DatetimeIndex, Index, Interval, IntervalIndex, Series, TimedeltaIndex, Timestamp, cut, date_range, isna, qcut, timedelta_range, to_datetime
...
@pytest.mark.parametrize('x, bins, expected', [(date_range('2017-12-31', periods=3), [Timestamp.min, Timestamp('2018-01-01'), Timestamp.max], IntervalIndex.from_tuples([(Timestamp.min, Timestamp('2018-01-01')), (Timestamp('2018-01-01'), Timestamp.max)])), ([(- 1), 0, 1], np.array([np.iinfo(np.int64).min, 0, np.iinfo(np.int64).max], dtype='int64'), IntervalIndex.from_tuples([(np.iinfo(np.int64).min, 0), (0, np.iinfo(np.int64).max)])), ([np.timedelta64((- 1)), np.timedelta64(0), np.timedelta64(1)], np.array([np.timedelta64((- np.iinfo(np.int64).max)), np.timedelta64(0), np.timedelta64(np.iinfo(np.int64).max)]), IntervalIndex.from_tuples([(np.timedelta64((- np.iinfo(np.int64).max)), np.timedelta64(0)), (np.timedelta64(0), np.timedelta64(np.iinfo(np.int64).max))]))])
...
def test_cut_return_intervals():
    ...
    exp_bins[0] -= 0.008
    expected = Series(IntervalIndex.from_breaks(exp_bins, closed='right').take([0, 0, 0, 1, 1, 1, 2, 2, 2])).astype(CDT(ordered=True))
...
def test_datetime_bin(conv):
    ...
    expected = Series(IntervalIndex([Interval(Timestamp(bin_data[0]), Timestamp(bin_data[1])), Interval(Timestamp(bin_data[1]), Timestamp(bin_data[2]))])).astype(CDT(ordered=True))
...
def test_datetime_cut(data):
    ...
    expected = Series(IntervalIndex([Interval(Timestamp('2012-12-31 23:57:07.200000'), Timestamp('2013-01-01 16:00:00')), Interval(Timestamp('2013-01-01 16:00:00'), Timestamp('2013-01-02 08:00:00')), Interval(Timestamp('2013-01-02 08:00:00'), Timestamp('2013-01-03 00:00:00'))])).astype(CDT(ordered=True))
    tm.assert_series_equal(Series(result), expected)

@pytest.mark.parametrize('bins', [3, [Timestamp('2013-01-01 04:57:07.200000'), Timestamp('2013-01-01 21:00:00'), Timestamp('2013-01-02 13:00:00'), Timestamp('2013-01-03 05:00:00')]])
...
def test_datetime_tz_cut(bins, box):
    ...
    if (not isinstance(bins, int)):
        bins = box(bins)
    result = cut(s, bins)
    expected = Series(IntervalIndex([Interval(Timestamp('2012-12-31 23:57:07.200000', tz=tz), Timestamp('2013-01-01 16:00:00', tz=tz)), Interval(Timestamp('2013-01-01 16:00:00', tz=tz), Timestamp('2013-01-02 08:00:00', tz=tz)), Interval(Timestamp('2013-01-02 08:00:00', tz=tz), Timestamp('2013-01-03 00:00:00', tz=tz))])).astype(CDT(ordered=True))
    tm.assert_series_equal(result, expected)
