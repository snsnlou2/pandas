...
class TestDataFrameNonuniqueIndexes():
    ...
    def test_masking_duplicate_columns_mixed_dtypes(self, data1, data2, expected_data):
        df1 = DataFrame(np.array(data1))
        df2 = DataFrame(np.array(data2))
        df = pd.concat([df1, df2], axis=1)
        result = df[(df > 2)]
        expected = DataFrame({i: np.array(col) for (i, col) in enumerate(expected_data)}).rename(columns={2: 0, 3: 1})
        tm.assert_frame_equal(result, expected)
