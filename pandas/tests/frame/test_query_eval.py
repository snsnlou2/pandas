...
class TestDataFrameQueryNumExprPandas():
    ...
    def test_query_index_with_name(self):
        (engine, parser) = (self.engine, self.parser)
        df = DataFrame(np.random.randint(10, size=(10, 3)), index=Index(range(10), name='blob'), columns=['a', 'b', 'c'])
    ...
    def test_check_tz_aware_index_query(self, tz_aware_fixture):
        tz = tz_aware_fixture
        df_index = pd.date_range(start='2019-01-01', freq='1d', periods=10, tz=tz, name='time')
