...
class TestDataFrameToDict():
    ...
    def test_to_dict_orient_dtype(self):
        df = DataFrame({'bool': [True, True, False], 'datetime': [datetime(2018, 1, 1), datetime(2019, 2, 2), datetime(2020, 3, 3)], 'float': [1.0, 2.0, 3.0], 'int': [1, 2, 3], 'str': ['X', 'Y', 'Z']})
        expected = {'int': int, 'float': float, 'str': str, 'datetime': Timestamp, 'bool': bool}
