...
class TestDataFrameSelectReindex():
    ...
    def test_reindex_level(self):
        ...
        def verify(df, level, idx, indexer, check_index_type=True):
            ...
            tm.assert_frame_equal(left, right, check_index_type=check_index_type)
        df = DataFrame({'jim': list(((('B' * 4) + ('A' * 2)) + ('C' * 3))), 'joe': list('abcdeabcd')[::(- 1)], 'jolie': ([10, 20, 30] * 3), 'joline': np.random.randint(0, 1000, 9)})
    ...
    def test_reindex_nan(self):
        df = DataFrame([[1, 2], [3, 5], [7, 11], [9, 23]], index=[2, np.nan, 1, 5], columns=['joe', 'jim'])
    ...
    def test_reindex_with_categoricalindex(self):
        df = DataFrame({'A': np.arange(3, dtype='int64')}, index=CategoricalIndex(list('abc'), dtype=CDT(list('cabe')), name='B'))
        ...
        tm.assert_frame_equal(result, expected, check_index_type=True)
        df2 = DataFrame({'A': np.arange(6, dtype='int64')}, index=CategoricalIndex(list('aabbca'), dtype=CDT(list('cabe')), name='B'))
        msg = 'cannot reindex from a duplicate axis'
        with pytest.raises(ValueError, match=msg):
            df2.reindex(['a', 'b'])
