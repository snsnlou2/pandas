...
import pytest
import pandas as pd
from pandas import CategoricalDtype, CategoricalIndex, DataFrame, Index, IntervalIndex, MultiIndex, Series, Timestamp
...
class TestDataFrameSortIndex():
    ...
    def test_sort_index_reorder_on_ops(self):
        df = DataFrame(np.random.randn(8, 2), index=MultiIndex.from_product([['a', 'b'], ['big', 'small'], ['red', 'blu']], names=['letter', 'size', 'color']), columns=['near', 'far'])
        df = df.sort_index()
    ...
    def test_sort_index_level2(self):
        mi = MultiIndex(levels=[['foo', 'bar', 'baz', 'qux'], ['one', 'two', 'three']], codes=[[0, 0, 0, 1, 1, 2, 2, 3, 3, 3], [0, 1, 2, 0, 1, 1, 2, 0, 1, 2]], names=['first', 'second'])
        frame = DataFrame(np.random.randn(10, 3), index=mi, columns=Index(['A', 'B', 'C'], name='exp'))
        df = frame.copy()
    ...
    @pytest.mark.parametrize('categories', [pytest.param(['a', 'b', 'c'], id='str'), pytest.param([pd.Interval(0, 1), pd.Interval(1, 2), pd.Interval(2, 3)], id='pd.Interval')])
    def test_sort_index_with_categories(self, categories):
        df = DataFrame({'foo': range(len(categories))}, index=CategoricalIndex(data=categories, categories=categories, ordered=True))
        df.index = df.index.reorder_categories(df.index.categories[::(- 1)])
        result = df.sort_index()
        expected = DataFrame({'foo': reversed(range(len(categories)))}, index=CategoricalIndex(data=categories[::(- 1)], categories=categories[::(- 1)], ordered=True))
        tm.assert_frame_equal(result, expected)
...
class TestDataFrameSortIndexKey():
    ...
    def test_sort_index_multiindex_sparse_column(self):
        expected = DataFrame({i: pd.array([0.0, 0.0, 0.0, 0.0], dtype=pd.SparseDtype('float64', 0.0)) for i in range(0, 4)}, index=MultiIndex.from_product([[1, 2], [1, 2]]))
