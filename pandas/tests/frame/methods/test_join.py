...
def frame_with_period_index():
    return DataFrame(data=np.arange(20).reshape(4, 5), columns=list('abcde'), index=period_range(start='2000', freq='A', periods=4))
...
class TestDataFrameJoin():
    ...
    def test_join_multiindex_leftright(self):
        ...
        exp_idx = pd.MultiIndex.from_product([['a', 'b'], ['x', 'y', 'z']], names=['first', 'second'])
    ...
    def test_frame_join_tzaware(self):
        test1 = DataFrame(np.zeros((6, 3)), index=date_range('2012-11-15 00:00:00', periods=6, freq='100L', tz='US/Central'))
        test2 = DataFrame(np.zeros((3, 3)), index=date_range('2012-11-15 00:00:00', periods=3, freq='250L', tz='US/Central'), columns=range(3, 6))
