...
def test_basic():
    df = pd.DataFrame({'A': pd.Series([[0, 1, 2], np.nan, [], (3, 4)], index=list('abcd')), 'B': 1})
    result = df.explode('A')
    expected = pd.DataFrame({'A': pd.Series([0, 1, 2, np.nan, np.nan, 3, 4], index=list('aaabcdd'), dtype=object), 'B': 1})
...
def test_multi_index_columns():
    ...
    expected = pd.DataFrame({('A', 1): pd.Series([0, 1, 2, np.nan, np.nan, 3, 4], index=pd.Index([0, 0, 0, 1, 2, 3, 3]), dtype=object), ('A', 2): 1})
    tm.assert_frame_equal(result, expected)
