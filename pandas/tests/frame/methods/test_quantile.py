...
class TestDataFrameQuantile():
    ...
    def test_quantile_datetime(self):
        ...
        expected = Series([Timestamp('2010-07-02 12:00:00'), Timestamp('2011-07-02 12:00:00')], index=[0, 1], name=0.5)
        tm.assert_series_equal(result, expected)
        result = df[['a', 'c']].quantile([0.5], axis=1, numeric_only=False)
        expected = DataFrame([[Timestamp('2010-07-02 12:00:00'), Timestamp('2011-07-02 12:00:00')]], index=[0.5], columns=[0, 1])
        tm.assert_frame_equal(result, expected)
    ...
    def test_quantile_nat(self):
        ...
        tm.assert_frame_equal(res, exp)
        df = DataFrame({'a': [Timestamp('2012-01-01'), Timestamp('2012-01-02'), Timestamp('2012-01-03')], 'b': [pd.NaT, pd.NaT, pd.NaT]})
        res = df.quantile(0.5, numeric_only=False)
