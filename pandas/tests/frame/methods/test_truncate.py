...
class TestDataFrameTruncate():
    ...
    def test_truncate_nonsortedindex_axis1(self):
        df = DataFrame({3: np.random.randn(5), 20: np.random.randn(5), 2: np.random.randn(5), 0: np.random.randn(5)}, columns=[3, 20, 2, 0])
        msg = 'truncate requires a sorted index'
        with pytest.raises(ValueError, match=msg):
    ...
    def test_truncate_decreasing_index(self, before, after, indices, klass, frame_or_series):
        idx = klass([3, 2, 1, 0])
