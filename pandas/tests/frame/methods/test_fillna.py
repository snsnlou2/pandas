...
import numpy as np
import pytest
from pandas import Categorical, DataFrame, DatetimeIndex, NaT, PeriodIndex, Series, TimedeltaIndex, Timestamp, date_range
...
class TestFillNA():
    ...
    def test_fillna_datetime_columns(self):
        df = DataFrame({'A': [(- 1), (- 2), np.nan], 'B': date_range('20130101', periods=3), 'C': ['foo', 'bar', None], 'D': ['foo2', 'bar2', None]}, index=date_range('20130110', periods=3))
        result = df.fillna('?')
        expected = DataFrame({'A': [(- 1), (- 2), '?'], 'B': date_range('20130101', periods=3), 'C': ['foo', 'bar', '?'], 'D': ['foo2', 'bar2', '?']}, index=date_range('20130110', periods=3))
        tm.assert_frame_equal(result, expected)
        df = DataFrame({'A': [(- 1), (- 2), np.nan], 'B': [Timestamp('2013-01-01'), Timestamp('2013-01-02'), NaT], 'C': ['foo', 'bar', None], 'D': ['foo2', 'bar2', None]}, index=date_range('20130110', periods=3))
        result = df.fillna('?')
        expected = DataFrame({'A': [(- 1), (- 2), '?'], 'B': [Timestamp('2013-01-01'), Timestamp('2013-01-02'), '?'], 'C': ['foo', 'bar', '?'], 'D': ['foo2', 'bar2', '?']}, index=date_range('20130110', periods=3))
        tm.assert_frame_equal(result, expected)
