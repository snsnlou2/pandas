...
class TestDataFrameCombineFirst():
    ...
    def test_combine_first_mixed(self):
        ...
        g = DataFrame({'A': a, 'B': b})
        exp = DataFrame({'A': list('abab'), 'B': [0.0, 1.0, 0.0, 1.0]}, index=[0, 1, 5, 6])
    ...
    @pytest.mark.parametrize('data1, data2, data_expected', (([datetime(2000, 1, 1), datetime(2000, 1, 2), datetime(2000, 1, 3)], [None, None, None], [datetime(2000, 1, 1), datetime(2000, 1, 2), datetime(2000, 1, 3)]), ([None, None, None], [datetime(2000, 1, 1), datetime(2000, 1, 2), datetime(2000, 1, 3)], [datetime(2000, 1, 1), datetime(2000, 1, 2), datetime(2000, 1, 3)]), ([datetime(2000, 1, 2), None, None], [datetime(2000, 1, 1), datetime(2000, 1, 2), datetime(2000, 1, 3)], [datetime(2000, 1, 2), datetime(2000, 1, 2), datetime(2000, 1, 3)]), ([datetime(2000, 1, 1), datetime(2000, 1, 2), datetime(2000, 1, 3)], [datetime(2000, 1, 2), None, None], [datetime(2000, 1, 1), datetime(2000, 1, 2), datetime(2000, 1, 3)])))
    ...
    def test_combine_first_align_nan(self):
        ...
        exp = DataFrame({'a': [pd.Timestamp('2011-01-01'), pd.NaT], 'b': [2.0, 5.0]}, columns=['a', 'b'])
    ...
    def test_combine_first_timezone(self):
        ...
        df1 = DataFrame(columns=['UTCdatetime', 'abc'], data=data1, index=pd.date_range('20140627', periods=1), dtype='object')
        data2 = pd.to_datetime('20121212 12:12').tz_localize('UTC')
        df2 = DataFrame(columns=['UTCdatetime', 'xyz'], data=data2, index=pd.date_range('20140628', periods=1), dtype='object')
        res = df2[['UTCdatetime']].combine_first(df1)
        exp = DataFrame({'UTCdatetime': [pd.Timestamp('2010-01-01 01:01', tz='UTC'), pd.Timestamp('2012-12-12 12:12', tz='UTC')], 'abc': [pd.Timestamp('2010-01-01 01:01:00', tz='UTC'), pd.NaT]}, columns=['UTCdatetime', 'abc'], index=pd.date_range('20140627', periods=2, freq='D'), dtype='object')
        ...
        res = df1.combine_first(df2)
        exp_dts = pd.DatetimeIndex(['2011-01-01', '2012-01-01', 'NaT', '2012-01-02', '2011-01-03', '2011-01-04'], tz='US/Eastern')
    ...
    def test_combine_first_string_dtype_only_na(self):
        ...
        result = df.combine_first(df2)
        expected = DataFrame({'a': ['962', '85'], 'b': ([pd.NA] * 2)}, dtype='string').set_index(['a', 'b'])
        tm.assert_frame_equal(result, expected)
