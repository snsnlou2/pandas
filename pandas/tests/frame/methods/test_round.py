...
class TestDataFrameRound():
    ...
    def test_round(self):
        ...
        msg = 'an integer is required \\(got type list\\)'
        with pytest.raises(TypeError, match=msg):
            df.round(non_int_round_dict)
        non_int_round_Series = Series(non_int_round_dict)
        with pytest.raises(TypeError, match=msg):
    ...
    def test_round_mixed_type(self):
        df = DataFrame({'col1': [1.1, 2.2, 3.3, 4.4], 'col2': ['1', 'a', 'c', 'f'], 'col3': date_range('20111111', periods=4)})
        round_0 = DataFrame({'col1': [1.0, 2.0, 3.0, 4.0], 'col2': ['1', 'a', 'c', 'f'], 'col3': date_range('20111111', periods=4)})
    ...
    def test_round_with_duplicate_columns(self):
        df = DataFrame(np.random.random([3, 3]), columns=['A', 'B', 'C'], index=['first', 'second', 'third'])
