...
class TestDataFrameCount():
    ...
    def test_count_level_corner(self, multiindex_dataframe_random_data):
        ...
        expected = DataFrame(index=ser.index.levels[0].set_names(['first']), columns=df.columns).fillna(0).astype(np.int64)
        tm.assert_frame_equal(result, expected)

    def test_count_index_with_nan(self):
        df = DataFrame({'Person': ['John', 'Myla', None, 'John', 'Myla'], 'Age': [24.0, 5, 21.0, 33, 26], 'Single': [False, True, True, True, False]})
        res = df.set_index(['Person', 'Single']).count(level='Person')
        expected = DataFrame(index=Index(['John', 'Myla'], name='Person'), columns=Index(['Age']), data=[2, 2])
        tm.assert_frame_equal(res, expected)
        res = df.set_index(['Person', 'Single']).T.count(level='Person', axis=1)
        expected = DataFrame(columns=Index(['John', 'Myla'], name='Person'), index=Index(['Age']), data=[[2, 2]])
        tm.assert_frame_equal(res, expected)

    def test_count_level(self, multiindex_year_month_day_dataframe_random_data, multiindex_dataframe_random_data):
