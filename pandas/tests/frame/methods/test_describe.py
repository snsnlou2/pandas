...
class TestDataFrameDescribe():
    ...
    def test_datetime_is_numeric_includes_datetime(self):
        ...
        expected = DataFrame({'a': [3, Timestamp('2012-01-02'), Timestamp('2012-01-01'), Timestamp('2012-01-01T12:00:00'), Timestamp('2012-01-02'), Timestamp('2012-01-02T12:00:00'), Timestamp('2012-01-03'), np.nan], 'b': [3, 2, 1, 1.5, 2, 2.5, 3, 1]}, index=['count', 'mean', 'min', '25%', '50%', '75%', 'max', 'std'])
        tm.assert_frame_equal(result, expected)
    ...
    def test_describe_tz_values2(self):
        ...
        df = DataFrame({'s1': s1, 's2': s2})
        s1_ = s1.describe()
        s2_ = Series([5, 5, s2.value_counts().index[0], 1, start.tz_localize(tz), end.tz_localize(tz)], index=['count', 'unique', 'top', 'freq', 'first', 'last'])
        idx = ['count', 'unique', 'top', 'freq', 'first', 'last', 'mean', 'std', 'min', '25%', '50%', '75%', 'max']
