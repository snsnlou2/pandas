...
class TestDataFrameReplace():
    ...
    def test_regex_replace_scalar_inplace(self, mix_ab):
        ...
        dfmix = DataFrame(mix_ab)
        res = dfobj.copy()
        return_value = res.replace('\\s*\\.\\s*', np.nan, regex=True, inplace=True)
        assert (return_value is None)
        tm.assert_frame_equal(dfobj, res.fillna('.'))
        res = dfmix.copy()
        return_value = res.replace('\\s*\\.\\s*', np.nan, regex=True, inplace=True)
        ...
        tm.assert_frame_equal(dfmix, res.fillna('.'))
        res = dfobj.copy()
        return_value = res.replace('\\s*(\\.)\\s*', '\\1\\1\\1', regex=True, inplace=True)
        assert (return_value is None)
        objc = obj.copy()
        objc['a'] = ['a', 'b', '...', '...']
        expec = DataFrame(objc)
        ...
        tm.assert_frame_equal(res, expec)
        res = dfobj.copy()
        return_value = res.replace(re.compile('\\s*\\.\\s*'), np.nan, regex=True, inplace=True)
        assert (return_value is None)
        tm.assert_frame_equal(dfobj, res.fillna('.'))
        res = dfmix.copy()
        return_value = res.replace(re.compile('\\s*\\.\\s*'), np.nan, regex=True, inplace=True)
        ...
        tm.assert_frame_equal(dfmix, res.fillna('.'))
        res = dfobj.copy()
        return_value = res.replace(re.compile('\\s*(\\.)\\s*'), '\\1\\1\\1', regex=True, inplace=True)
        assert (return_value is None)
        objc = obj.copy()
        objc['a'] = ['a', 'b', '...', '...']
        expec = DataFrame(objc)
        ...
        tm.assert_frame_equal(res, expec)
        res = dfobj.copy()
        return_value = res.replace(regex='\\s*\\.\\s*', value=np.nan, inplace=True)
        assert (return_value is None)
        tm.assert_frame_equal(dfobj, res.fillna('.'))
        res = dfmix.copy()
        return_value = res.replace(regex='\\s*\\.\\s*', value=np.nan, inplace=True)
        ...
        tm.assert_frame_equal(dfmix, res.fillna('.'))
        res = dfobj.copy()
        return_value = res.replace(regex='\\s*(\\.)\\s*', value='\\1\\1\\1', inplace=True)
        assert (return_value is None)
        objc = obj.copy()
        objc['a'] = ['a', 'b', '...', '...']
        expec = DataFrame(objc)
        ...
        tm.assert_frame_equal(res, expec)
        res = dfobj.copy()
        return_value = res.replace(regex=re.compile('\\s*\\.\\s*'), value=np.nan, inplace=True)
        assert (return_value is None)
        tm.assert_frame_equal(dfobj, res.fillna('.'))
        res = dfmix.copy()
        return_value = res.replace(regex=re.compile('\\s*\\.\\s*'), value=np.nan, inplace=True)
        ...
        tm.assert_frame_equal(dfmix, res.fillna('.'))
        res = dfobj.copy()
        return_value = res.replace(regex=re.compile('\\s*(\\.)\\s*'), value='\\1\\1\\1', inplace=True)
        assert (return_value is None)
        objc = obj.copy()
        objc['a'] = ['a', 'b', '...', '...']
        expec = DataFrame(objc)
    ...
    @pytest.mark.parametrize('frame, to_replace, value, expected', [(DataFrame({'ints': [1, 2, 3]}), 1, 0, DataFrame({'ints': [0, 2, 3]})), (DataFrame({'ints': [1, 2, 3]}, dtype=np.int32), 1, 0, DataFrame({'ints': [0, 2, 3]}, dtype=np.int32)), (DataFrame({'ints': [1, 2, 3]}, dtype=np.int16), 1, 0, DataFrame({'ints': [0, 2, 3]}, dtype=np.int16)), (DataFrame({'bools': [True, False, True]}), False, True, DataFrame({'bools': [True, True, True]})), (DataFrame({'complex': [1j, 2j, 3j]}), 1j, 0, DataFrame({'complex': [0j, 2j, 3j]})), (DataFrame({'datetime64': Index([datetime(2018, 5, 28), datetime(2018, 7, 28), datetime(2018, 5, 28)])}), datetime(2018, 5, 28), datetime(2018, 7, 28), DataFrame({'datetime64': Index(([datetime(2018, 7, 28)] * 3))})), (DataFrame({'dt': [datetime(3017, 12, 20)], 'str': ['foo']}), 'foo', 'bar', DataFrame({'dt': [datetime(3017, 12, 20)], 'str': ['bar']})), (DataFrame({'A': date_range('20130101', periods=3, tz='US/Eastern'), 'B': [0, np.nan, 2]}), Timestamp('20130102', tz='US/Eastern'), Timestamp('20130104', tz='US/Eastern'), DataFrame({'A': [Timestamp('20130101', tz='US/Eastern'), Timestamp('20130104', tz='US/Eastern'), Timestamp('20130103', tz='US/Eastern')], 'B': [0, np.nan, 2]})), (DataFrame([[1, 1.0], [2, 2.0]]), 1.0, 5, DataFrame([[5, 5.0], [2, 2.0]])), (DataFrame([[1, 1.0], [2, 2.0]]), 1, 5, DataFrame([[5, 5.0], [2, 2.0]])), (DataFrame([[1, 1.0], [2, 2.0]]), 1.0, 5.0, DataFrame([[5, 5.0], [2, 2.0]])), (DataFrame([[1, 1.0], [2, 2.0]]), 1, 5.0, DataFrame([[5, 5.0], [2, 2.0]]))])
    ...
    def test_replace_datetimetz(self):
        ...
        expected = DataFrame({'A': [Timestamp('20130101', tz='US/Eastern'), Timestamp('20130104', tz='US/Pacific'), Timestamp('20130103', tz='US/Eastern')], 'B': [0, np.nan, 2]})
        tm.assert_frame_equal(result, expected)
        result = df.copy()
        result.iloc[(1, 0)] = np.nan
    ...
    @pytest.mark.xfail(reason='category dtype gets changed to object type after replace, see #35268', strict=True)
