...
import pytest
import pandas as pd
from pandas import Categorical, CategoricalDtype, DataFrame, DatetimeTZDtype, Interval, IntervalDtype, NaT, Series, Timedelta, Timestamp, UInt64Index, concat, date_range, option_context
...
class TestAstype():
    ...
    def test_astype_str(self):
        ...
        expected = DataFrame({'a': list(map(str, map((lambda x: Timestamp(x)._date_repr), a._values))), 'b': list(map(str, map(Timestamp, b._values))), 'c': list(map((lambda x: Timedelta(x)._repr_base()), c._values)), 'd': list(map(str, d._values)), 'e': list(map(str, e._values))})
    ...
    def test_astype_dict_like(self, dtype_class):
        ...
        expected = DataFrame({'a': a, 'b': Series(['0', '1', '2', '3', '4']), 'c': c, 'd': Series([1.0, 2.0, 3.14, 4.0, 5.4], dtype='float32')})
        tm.assert_frame_equal(result, expected)
    ...
    def test_astype_to_incorrect_datetimelike(self, unit):
        ...
        msg = f'cannot astype a datetimelike from \[datetime64\[ns\]\] to \[timedelta64\[{unit}\]\]'
        with pytest.raises(TypeError, match=msg):
            df.astype(other)
        msg = f'cannot astype a timedelta from \[timedelta64\[ns\]\] to \[datetime64\[{unit}\]\]'
    ...
    def test_astype_dt64tz_to_str(self, timezone_frame):
        ...
        expected = DataFrame([['2013-01-01', '2013-01-01 00:00:00-05:00', '2013-01-01 00:00:00+01:00'], ['2013-01-02', 'NaT', 'NaT'], ['2013-01-03', '2013-01-03 00:00:00-05:00', '2013-01-03 00:00:00+01:00']], columns=timezone_frame.columns)
        tm.assert_frame_equal(result, expected)
    ...
    @pytest.mark.parametrize('df', [DataFrame(Series(['x', 'y', 'z'], dtype='string')), DataFrame(Series(['x', 'y', 'z'], dtype='category')), DataFrame(Series((3 * [Timestamp('2020-01-01', tz='UTC')]))), DataFrame(Series((3 * [Interval(0, 1)])))])
    ...
    def test_astype_dt64_to_string(self, frame_or_series, tz_naive_fixture, request):
        tz = tz_naive_fixture
        if (tz is None):
            mark = pytest.mark.xfail(reason='GH#36153 uses ndarray formatting instead of DTA formatting')
