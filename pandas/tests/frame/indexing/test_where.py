...
class TestDataFrameIndexingWhere():
    ...
    def test_where_bug_mixed(self, sint_dtype):
        df = DataFrame({'a': np.array([1, 2, 3, 4], dtype=sint_dtype), 'b': np.array([4.0, 3.0, 2.0, 1.0], dtype='float64')})
        expected = DataFrame({'a': [np.nan, np.nan, 3.0, 4.0], 'b': [4.0, 3.0, np.nan, np.nan]}, dtype='float64')
        result = df.where((df > 2), np.nan)
        tm.assert_frame_equal(result, expected)
        result = df.copy()
    ...
    def test_where_datetime(self):
        df = DataFrame({'A': date_range('20130102', periods=5), 'B': date_range('20130104', periods=5), 'C': np.random.randn(5)})
    ...
    def test_where_axis(self):
        ...
        assert (return_value is None)
        tm.assert_frame_equal(result, expected)
        df = pd.concat([DataFrame(np.random.randn(10, 2)), DataFrame(np.random.randint(0, 10, size=(10, 2)), dtype='int64')], ignore_index=True, axis=1)
