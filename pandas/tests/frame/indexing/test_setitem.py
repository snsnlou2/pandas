...
from pandas import Categorical, DataFrame, Index, Interval, NaT, Period, PeriodIndex, Series, Timestamp, date_range, notna, period_range
...
class TestDataFrameSetItem():
    ...
    def test_setitem_dict_preserves_dtypes(self):
        expected = DataFrame({'a': Series([0, 1, 2], dtype='int64'), 'b': Series([1, 2, 3], dtype=float), 'c': Series([1, 2, 3], dtype=float)})
        df = DataFrame({'a': Series([], dtype='int64'), 'b': Series([], dtype=float), 'c': Series([], dtype=float)})
        for (idx, b) in enumerate([1, 2, 3]):
            df.loc[df.shape[0]] = {'a': int(idx), 'b': float(b), 'c': float(b)}
        tm.assert_frame_equal(df, expected)

    @pytest.mark.parametrize('obj,dtype', [(Period('2020-01'), PeriodDtype('M')), (Interval(left=0, right=5), IntervalDtype('int64')), (Timestamp('2011-01-01', tz='US/Eastern'), DatetimeTZDtype(tz='US/Eastern'))])
    ...
    @pytest.mark.parametrize('ea_name', ([dtype.name for dtype in ea_registry.dtypes if (not isinstance(dtype.name, property))] + ['datetime64[ns, UTC]', 'period[D]']))
    ...
    def test_setitem_dt64_ndarray_with_NaT_and_diff_time_units(self):
        ...
        result['new'] = data_s
        expected = DataFrame({0: [1, None], 'new': [1000000000.0, None]}, dtype='datetime64[ns]')
        tm.assert_frame_equal(result, expected)
