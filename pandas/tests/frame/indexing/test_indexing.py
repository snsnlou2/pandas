...
import pandas as pd
from pandas import DataFrame, DatetimeIndex, Index, MultiIndex, Series, Timestamp, date_range, isna, notna
...
class TestDataFrameIndexing():
    ...
    def test_getitem_setitem_float_labels(self):
        ...
        df = DataFrame(np.random.randn(5, 5), index=index)
        msg = 'cannot do positional indexing on Float64Index with these indexers \\[1.0\\] of type float'
        with pytest.raises(TypeError, match=msg):
            df.iloc[1.0:5]
        result = df.iloc[4:5]
    ...
    def test_loc_duplicates(self):
        trange = pd.date_range(start=Timestamp(year=2017, month=1, day=1), end=Timestamp(year=2017, month=1, day=5))
...
def test_object_casting_indexing_wraps_datetimelike():
    df = DataFrame({'A': [1, 2], 'B': pd.date_range('2000', periods=2), 'C': pd.timedelta_range('1 Day', periods=2)})
    ser = df.loc[0]
