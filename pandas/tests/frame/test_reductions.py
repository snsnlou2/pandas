...
import pandas as pd
from pandas import Categorical, DataFrame, Index, MultiIndex, Series, Timestamp, date_range, isna, notna, to_datetime, to_timedelta
...
def assert_stat_op_calc(opname, alternative, frame, has_skipna=True, check_dtype=True, check_dates=False, rtol=1e-05, atol=1e-08, skipna_alternative=None):
    ...
    if has_skipna:
        ...
        tm.assert_series_equal(result1, frame.apply(wrapper, axis=1), check_dtype=False, rtol=rtol, atol=atol)
    else:
...
def assert_bool_op_calc(opname, alternative, frame, has_skipna=True):
    ...
    if has_skipna:
        ...
        tm.assert_series_equal(result1, frame.apply(wrapper, axis=1), check_dtype=False)
    else:
        skipna_wrapper = alternative
        wrapper = alternative
    result0 = f(axis=0)
...
def assert_bool_op_api(opname, bool_frame_with_na, float_string_frame, has_bool_only=False):
...
class TestDataFrameAnalytics():
    ...
    @pytest.mark.parametrize('dropna, expected', [(True, {'A': [12], 'B': [10.0], 'C': [1.0], 'D': ['a'], 'E': Categorical(['a'], categories=['a']), 'F': to_datetime(['2000-1-2']), 'G': to_timedelta(['1 days'])}), (False, {'A': [12], 'B': [10.0], 'C': [np.nan], 'D': np.array([np.nan], dtype=object), 'E': Categorical([np.nan], categories=['a']), 'F': [pd.NaT], 'G': to_timedelta([pd.NaT])}), (True, {'H': [8, 9, np.nan, np.nan], 'I': [8, 9, np.nan, np.nan], 'J': [1, np.nan, np.nan, np.nan], 'K': Categorical(['a', np.nan, np.nan, np.nan], categories=['a']), 'L': to_datetime(['2000-1-2', 'NaT', 'NaT', 'NaT']), 'M': to_timedelta(['1 days', 'nan', 'nan', 'nan']), 'N': [0, 1, 2, 3]}), (False, {'H': [8, 9, np.nan, np.nan], 'I': [8, 9, np.nan, np.nan], 'J': [1, np.nan, np.nan, np.nan], 'K': Categorical([np.nan, 'a', np.nan, np.nan], categories=['a']), 'L': to_datetime(['NaT', '2000-1-2', 'NaT', 'NaT']), 'M': to_timedelta(['nan', '1 days', 'nan', 'nan']), 'N': [0, 1, 2, 3]})])
    def test_mode_dropna(self, dropna, expected):
        df = DataFrame({'A': [12, 12, 19, 11], 'B': [10, 10, np.nan, 3], 'C': [1, np.nan, np.nan, np.nan], 'D': [np.nan, np.nan, 'a', np.nan], 'E': Categorical([np.nan, np.nan, 'a', np.nan]), 'F': to_datetime(['NaT', '2000-1-2', 'NaT', 'NaT']), 'G': to_timedelta(['1 days', 'nan', 'nan', 'nan']), 'H': [8, 8, 9, 9], 'I': [9, 9, 8, 8], 'J': [1, 1, np.nan, np.nan], 'K': Categorical(['a', np.nan, 'a', np.nan]), 'L': to_datetime(['2000-1-2', '2000-1-2', 'NaT', 'NaT']), 'M': to_timedelta(['1 days', 'nan', '1 days', 'nan']), 'N': np.arange(4, dtype='int64')})
    ...
    def test_sum_mixed_datetime(self):
        df = DataFrame({'A': pd.date_range('2000', periods=4), 'B': [1, 2, 3, 4]}).reindex([2, 3, 4])
        result = df.sum()
    ...
    def test_mean_datetimelike(self):
        df = DataFrame({'A': np.arange(3), 'B': pd.date_range('2016-01-01', periods=3), 'C': pd.timedelta_range('1D', periods=3), 'D': pd.period_range('2016', periods=3, freq='A')})
    ...
    def test_mean_datetimelike_numeric_only_false(self):
        df = DataFrame({'A': np.arange(3), 'B': pd.date_range('2016-01-01', periods=3), 'C': pd.timedelta_range('1D', periods=3)})
...
class TestDataFrameReductions():
    ...
    def test_min_max_dt64_with_NaT_skipna_false(self, tz_naive_fixture):
        ...
        if (isinstance(tz, tzlocal) and is_platform_windows()):
            pytest.xfail(reason='GH#37659 OSError raised within tzlocal bc Windows chokes in times before 1970-01-01')
    ...
    def test_preserve_timezone(self, initial, method):
...
def test_sum_timedelta64_skipna_false():
    ...
    expected = Series([pd.Timedelta(seconds=1), pd.Timedelta(seconds=5), pd.Timedelta(seconds=9), pd.NaT])
    tm.assert_series_equal(result, expected)
