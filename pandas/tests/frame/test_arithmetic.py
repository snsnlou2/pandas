...
class TestFrameComparisons():
    ...
    def test_comparison_invalid(self):
        ...
        df2 = DataFrame({'a': pd.date_range('20010101', periods=len(df)), 'b': pd.date_range('20100101', periods=len(df))})
        check(df, df2)

    def test_timestamp_compare(self):
        df = DataFrame({'dates1': pd.date_range('20010101', periods=10), 'dates2': pd.date_range('20010102', periods=10), 'intcol': np.random.randint(1000000000, size=10), 'floatcol': np.random.randn(10), 'stringcol': list(tm.rands(10))})
...
class TestFrameArithmetic():
    ...
    def test_frame_with_frame_reindex(self):
        df = DataFrame({'foo': [pd.Timestamp('2019'), pd.Timestamp('2020')], 'bar': [pd.Timestamp('2018'), pd.Timestamp('2021')]}, columns=['foo', 'bar'])
        df2 = df[['foo']]
        result = (df - df2)
...
class TestFrameArithmeticUnsorted():
    ...
    def test_combine_series(self, float_frame, mixed_float_frame, mixed_int_frame, datetime_frame):
    ...
    def test_alignment_non_pandas(self):
        ...
        tm.assert_frame_equal(align(df, val, 'index')[1], DataFrame(val, index=df.index, columns=df.columns))
        tm.assert_frame_equal(align(df, val, 'columns')[1], DataFrame(val, index=df.index, columns=df.columns))
...
@pytest.mark.parametrize('df, col_dtype', [(DataFrame([[1.0, 2.0], [4.0, 5.0]], columns=list('ab')), 'float64'), (DataFrame([[1.0, 'b'], [4.0, 'b']], columns=list('ab')), 'object')])
