...
import pandas as pd
from pandas import Categorical, DataFrame, Series, Timestamp, compat, date_range, option_context
...
class TestDataFrameBlockInternals():
    ...
    def test_constructor_with_convert(self):
        ...
        result = df['A']
        expected = Series(np.asarray([(1.0 + 2j), 3], np.complex_), name='A')
        tm.assert_series_equal(result, expected)
        df = DataFrame({'A': [(1.0 + 2j), 3.0]})
        result = df['A']
        expected = Series(np.asarray([(1.0 + 2j), 3.0], np.complex_), name='A')
        tm.assert_series_equal(result, expected)
        ...
        result = df['A']
        expected = Series(np.asarray([(1.0 + 2j), np.nan], np.complex_), name='A')
        tm.assert_series_equal(result, expected)
    ...
    def test_add_column_with_pandas_array(self):
        ...
        df2 = DataFrame({'a': [1, 2, 3, 4], 'b': ['a', 'b', 'c', 'd'], 'c': pd.arrays.PandasArray(np.array([1, 2, None, 3], dtype=object))})
