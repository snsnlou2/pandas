...
import pandas as pd
from pandas import Categorical, CategoricalIndex, DataFrame, Index, Interval, MultiIndex, Period, RangeIndex, Series, Timedelta, Timestamp, date_range, isna
...
class TestDataFrameConstructors():
    ...
    @pytest.mark.parametrize('data,dtype', [(Period('2020-01'), PeriodDtype('M')), (Interval(left=0, right=5), IntervalDtype('int64')), (Timestamp('2011-01-01', tz='US/Eastern'), DatetimeTZDtype(tz='US/Eastern'))])
...
class TestFromScalar():
    ...
    def test_from_timedelta64_scalar_object(self, constructor, request):
        if ((getattr(constructor, 'func', None) is DataFrame) and _np_version_under1p20):
            mark = pytest.mark.xfail(reason='np.array(td64, dtype=object) converts to int')
