...
class TestDataFrameApply():
    ...
    @pytest.mark.parametrize('args,kwds', [pytest.param([], {}, id='no_args_or_kwds'), pytest.param([1], {}, id='axis_from_args'), pytest.param([], {'axis': 1}, id='axis_from_kwds'), pytest.param([], {'numeric_only': True}, id='optional_kwds'), pytest.param([1, None], {'numeric_only': True}, id='args_and_kwds')])
    ...
    def test_applymap(self, float_frame):
        ...
        empty_frames = [DataFrame(), DataFrame(columns=list('ABC')), DataFrame(index=list('ABC')), DataFrame({'A': [], 'B': [], 'C': []})]
...
class TestDataFrameAggregate():
    ...
    def test_agg_multiple_mixed_no_warning(self):
        mdf = DataFrame({'A': [1, 2, 3], 'B': [1.0, 2.0, 3.0], 'C': ['foo', 'bar', 'baz'], 'D': pd.date_range('20130101', periods=3)})
        expected = DataFrame({'A': [1, 6], 'B': [1.0, 6.0], 'C': ['bar', 'foobarbaz'], 'D': [Timestamp('2013-01-01'), pd.NaT]}, index=['min', 'sum'])
    ...
    def test_nuiscance_columns(self):
        df = DataFrame({'A': [1, 2, 3], 'B': [1.0, 2.0, 3.0], 'C': ['foo', 'bar', 'baz'], 'D': pd.date_range('20130101', periods=3)})
        result = df.agg('min')
        ...
        tm.assert_series_equal(result, expected)
        result = df.agg(['sum'])
        expected = DataFrame([[6, 6.0, 'foobarbaz']], index=['sum'], columns=['A', 'B', 'C'])
        tm.assert_frame_equal(result, expected)
