...
import pandas as pd
from pandas import Categorical, DataFrame, MultiIndex, Series, Timestamp, concat, date_range
...
def test_transform_fast():
    ...
    tm.assert_series_equal(result, expected)
    df = DataFrame({'grouping': [0, 1, 1, 3], 'f': [1.1, 2.1, 3.1, 4.5], 'd': pd.date_range('2014-1-1', '2014-1-4'), 'i': [1, 2, 3, 4]}, columns=['grouping', 'f', 'i', 'd'])
    result = df.groupby('grouping').transform('first')
    dates = [Timestamp('2014-1-1'), Timestamp('2014-1-2'), Timestamp('2014-1-2'), Timestamp('2014-1-4')]
...
def test_transform_casting():
    ...
    df = pd.read_csv(StringIO(data), sep='\\s+', index_col=[0], parse_dates=['DATETIME'])
...
def test_transform_transformation_func(transformation_func):
    df = DataFrame({'A': ['foo', 'foo', 'foo', 'foo', 'bar', 'bar', 'baz'], 'B': [1, 2, np.nan, 3, 3, np.nan, 4]}, index=pd.date_range('2020-01-01', '2020-01-07'))
    if (transformation_func == 'cumcount'):
        ...
        msg = 'Current behavior of groupby.tshift is inconsistent with other transformations. See GH34452 for more details'
        pytest.xfail(msg)
    else:
...
def test_series_fast_transform_date():
    df = DataFrame({'grouping': [np.nan, 1, 1, 3], 'd': pd.date_range('2014-1-1', '2014-1-4')})
...
def test_cython_transform_frame(op, args, targop):
    ...
    df = DataFrame({'float': s, 'float_missing': s_missing, 'int': ([1, 1, 1, 1, 2] * 200), 'datetime': pd.date_range('1990-1-1', periods=1000), 'timedelta': pd.timedelta_range(1, freq='s', periods=1000), 'string': (strings * 50), 'string_missing': (strings_missing * 50)}, columns=['float', 'float_missing', 'int', 'datetime', 'timedelta', 'string', 'string_missing'])
    df['cat'] = df['string'].astype('category')
    df2 = df.copy()
    df2.index = pd.MultiIndex.from_product([range(100), range(10)])
    for df in [df, df2]:
        for gb_target in [{'by': labels}, {'level': 0}, {'by': 'string'}]:
...
def test_transform_with_non_scalar_group():
    cols = pd.MultiIndex.from_tuples([('syn', 'A'), ('mis', 'A'), ('non', 'A'), ('syn', 'C'), ('mis', 'C'), ('non', 'C'), ('syn', 'T'), ('mis', 'T'), ('non', 'T'), ('syn', 'G'), ('mis', 'G'), ('non', 'G')])
...
@pytest.mark.parametrize('cols,exp,comp_func', [('a', Series([1, 1, 1], name='a'), tm.assert_series_equal), (['a', 'c'], DataFrame({'a': [1, 1, 1], 'c': [1, 1, 1]}), tm.assert_frame_equal)])
...
def test_transform_numeric_ret(cols, exp, comp_func, agg_func, request):
    ...
    df = DataFrame({'a': pd.date_range('2018-01-01', periods=3), 'b': range(3), 'c': range(7, 10)})
...
def test_pct_change(test_series, freq, periods, fill_method, limit):
    ...
    if test_series:
        result = df.groupby('key')['vals'].pct_change(periods=periods, fill_method=fill_method, limit=limit, freq=freq)
        tm.assert_series_equal(result, expected)
    else:
        result = df.groupby('key').pct_change(periods=periods, fill_method=fill_method, limit=limit, freq=freq)
...
@pytest.mark.parametrize('obj', [DataFrame({'a': [0, 0, 0, 1, 1, 1], 'b': range(6)}, index=['A', 'B', 'C', 'D', 'E', 'F']), Series([0, 0, 0, 1, 1, 1], index=['A', 'B', 'C', 'D', 'E', 'F'])])
