...
def test_nth():
    ...
    expected_dates = pd.to_datetime(['2014/4/1', '2014/4/4', '2014/4/29', '2014/4/30', '2014/5/1', '2014/5/6', '2014/5/29', '2014/5/30', '2014/6/2', '2014/6/5', '2014/6/27', '2014/6/30'])
...
def test_nth_multi_index_as_expected():
    ...
    grouped = three_group.groupby(['A', 'B'])
    result = grouped.nth(0)
    expected = DataFrame({'C': ['dull', 'dull', 'dull', 'dull']}, index=MultiIndex.from_arrays([['bar', 'bar', 'foo', 'foo'], ['one', 'two', 'one', 'two']], names=['A', 'B']))
    tm.assert_frame_equal(result, expected)

@pytest.mark.parametrize('op, n, expected_rows', [('head', (- 1), []), ('head', 0, []), ('head', 1, [0, 2]), ('head', 7, [0, 1, 2]), ('tail', (- 1), []), ('tail', 0, []), ('tail', 1, [1, 2]), ('tail', 7, [0, 1, 2])])
...
@pytest.mark.parametrize('op, n, expected_cols', [('head', (- 1), []), ('head', 0, []), ('head', 1, [0, 2]), ('head', 7, [0, 1, 2]), ('tail', (- 1), []), ('tail', 0, []), ('tail', 1, [1, 2]), ('tail', 7, [0, 1, 2])])
...
def test_nth_empty():
    ...
    expected = DataFrame(index=MultiIndex([[], []], [[], []], names=['a', 'b']), columns=['c'])
    tm.assert_frame_equal(result, expected)

def test_nth_column_order():
    df = DataFrame([[1, 'b', 100], [1, 'a', 50], [1, 'a', np.nan], [2, 'c', 200], [2, 'd', 150]], columns=['A', 'C', 'B'])
    result = df.groupby('A').nth(0)
    expected = DataFrame([['b', 100.0], ['c', 200.0]], columns=['C', 'B'], index=Index([1, 2], name='A'))
    tm.assert_frame_equal(result, expected)
    result = df.groupby('A').nth((- 1), dropna='any')
    expected = DataFrame([['a', 50.0], ['d', 150.0]], columns=['C', 'B'], index=Index([1, 2], name='A'))
    tm.assert_frame_equal(result, expected)

@pytest.mark.parametrize('dropna', [None, 'any', 'all'])
def test_nth_nan_in_grouper(dropna):
    df = DataFrame([[np.nan, 0, 1], ['abc', 2, 3], [np.nan, 4, 5], ['def', 6, 7], [np.nan, 8, 9]], columns=list('abc'))
    result = df.groupby('a').nth(0, dropna=dropna)
    expected = DataFrame([[2, 3], [6, 7]], columns=list('bc'), index=Index(['abc', 'def'], name='a'))
    tm.assert_frame_equal(result, expected)
