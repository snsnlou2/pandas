...
from pandas.core.groupby.base import groupby_other_methods, reduction_kernels, transformation_kernels
AGG_FUNCTIONS = ['sum', 'prod', 'min', 'max', 'median', 'mean', 'skew', 'mad', 'std', 'var', 'sem']
AGG_FUNCTIONS_WITH_SKIPNA = ['skew', 'mad']
df_allowlist = ['quantile', 'fillna', 'mad', 'take', 'idxmax', 'idxmin', 'tshift', 'skew', 'plot', 'hist', 'dtypes', 'corrwith', 'corr', 'cov', 'diff']

@pytest.fixture(params=df_allowlist)
def df_allowlist_fixture(request):
    return request.param
s_allowlist = ['quantile', 'fillna', 'mad', 'take', 'idxmax', 'idxmin', 'tshift', 'skew', 'plot', 'hist', 'dtype', 'corr', 'cov', 'diff', 'unique', 'nlargest', 'nsmallest', 'is_monotonic_increasing', 'is_monotonic_decreasing']
...
@pytest.fixture
def df():
    return DataFrame({'A': ['foo', 'bar', 'foo', 'bar', 'foo', 'bar', 'foo', 'foo'], 'B': ['one', 'one', 'two', 'three', 'two', 'two', 'one', 'three'], 'C': np.random.randn(8), 'D': np.random.randn(8)})
...
def raw_frame():
    index = MultiIndex(levels=[['foo', 'bar', 'baz', 'qux'], ['one', 'two', 'three']], codes=[[0, 0, 0, 1, 1, 2, 2, 3, 3, 3], [0, 1, 2, 0, 1, 1, 2, 0, 1, 2]], names=['first', 'second'])
    raw_frame = DataFrame(np.random.randn(10, 3), index=index, columns=Index(['A', 'B', 'C'], name='exp'))
