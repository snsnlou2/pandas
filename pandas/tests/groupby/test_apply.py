...
@pytest.mark.parametrize('func', [(lambda x: x), pytest.param((lambda x: x[:]), marks=pytest.mark.xfail(reason='GH-34998')), (lambda x: x.copy(deep=False)), pytest.param((lambda x: x.copy(deep=True)), marks=pytest.mark.xfail(reason='GH-34998'))])
...
def test_apply_with_mixed_dtype():
    df = DataFrame({'foo1': np.random.randn(6), 'foo2': ['one', 'two', 'two', 'three', 'one', 'two']})
...
def test_apply_frame_concat_series():
    ...
    df = DataFrame({'A': np.random.randint(0, 5, 1000), 'B': np.random.randint(0, 5, 1000), 'C': np.random.randn(1000)})
...
def test_apply_typecast_fail():
    df = DataFrame({'d': [1.0, 1.0, 1.0, 2.0, 2.0, 2.0], 'c': np.tile(['a', 'b', 'c'], 2), 'v': np.arange(1.0, 7.0)})
...
def test_apply_corner_cases():
    ...
    df = DataFrame({'key': labels, 'value1': np.random.randn(N), 'value2': (['foo', 'bar', 'baz', 'qux'] * (N // 4))})
...
def test_func_returns_object():
    ...
    expected = Series([pd.Int64Index([1]), pd.Int64Index([2])], index=pd.Int64Index([1, 2], name='a'))
    tm.assert_series_equal(result, expected)
...
def test_groupby_apply_datetime_result_dtypes():
    ...
    expected = Series([np.dtype('datetime64[ns]'), object, object, np.int64, object], index=['observation', 'color', 'mood', 'intensity', 'score'])
    tm.assert_series_equal(result, expected)

@pytest.mark.parametrize('index', [pd.CategoricalIndex(list('abc')), pd.interval_range(0, 3), pd.period_range('2020', periods=3, freq='D'), pd.MultiIndex.from_tuples([('a', 0), ('a', 1), ('b', 0)])])
def test_apply_index_has_complex_internals(index):
    df = DataFrame({'group': [1, 1, 2], 'value': [0, 1, 0]}, index=index)
    result = df.groupby('group').apply((lambda x: x))
    tm.assert_frame_equal(result, df)

@pytest.mark.parametrize('function, expected_values', [((lambda x: x.index.to_list()), [[0, 1], [2, 3]]), ((lambda x: set(x.index.to_list())), [{0, 1}, {2, 3}]), ((lambda x: tuple(x.index.to_list())), [(0, 1), (2, 3)]), ((lambda x: {n: i for (n, i) in enumerate(x.index.to_list())}), [{0: 0, 1: 1}, {0: 2, 1: 3}]), ((lambda x: [{n: i} for (n, i) in enumerate(x.index.to_list())]), [[{0: 0}, {1: 1}], [{0: 2}, {1: 3}]])])
...
def test_apply_is_unchanged_when_other_methods_are_called_first(reduction_func):
    ...
    expected = DataFrame({'a': [264, 297], 'b': [15, 6], 'c': [150, 60]}, index=Index([88, 99], name='a'))
    grp = df.groupby(by='a')
...
def test_apply_by_cols_equals_apply_by_rows_transposed():
    df = DataFrame(np.random.random([6, 4]), columns=pd.MultiIndex.from_product([['A', 'B'], [1, 2]]))
...
def test_apply_dropna_with_indexed_same():
    ...
    result = df.groupby('group').apply((lambda x: x))
    expected = DataFrame({'col': [1, 4, 5], 'group': ['a', 'b', 'b']}, index=list('xxz'))
    tm.assert_frame_equal(result, expected)
