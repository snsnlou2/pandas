...
def test_series_groupby_value_counts(df, keys, bins, n, m, isort, normalize, sort, ascending, dropna):
...
def test_series_groupby_value_counts_on_categorical():
    ...
    expected = Series(data=[1, 0], index=MultiIndex.from_arrays([[0, 0], CategoricalIndex(['a', 'b'], categories=['a', 'b'], ordered=False, dtype='category')]), name=0)
    tm.assert_series_equal(result, expected)
