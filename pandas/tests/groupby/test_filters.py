...
def test_filter_against_workaround():
    ...
    df = DataFrame({'ints': Series(np.random.randint(0, 100, N)), 'floats': ((N / 10) * Series(np.random.random(N))), 'letters': Series(random_letters)})
...
def test_filter_multiple_timestamp():
    df = DataFrame({'A': np.arange(5, dtype='int64'), 'B': ['foo', 'bar', 'foo', 'bar', 'bar'], 'C': Timestamp('20130101')})
...
def test_filter_enforces_scalarness():
    df = DataFrame([['best', 'a', 'x'], ['worst', 'b', 'y'], ['best', 'c', 'x'], ['best', 'd', 'y'], ['worst', 'd', 'y'], ['worst', 'd', 'y'], ['best', 'd', 'z']], columns=['a', 'b', 'c'])
    with pytest.raises(TypeError, match='filter function returned a.*'):
        df.groupby('c').filter((lambda g: (g['a'] == 'best')))

def test_filter_non_bool_raises():
    df = DataFrame([['best', 'a', 1], ['worst', 'b', 1], ['best', 'c', 1], ['best', 'd', 1], ['worst', 'd', 1], ['worst', 'd', 1], ['best', 'd', 1]], columns=['a', 'b', 'c'])
    with pytest.raises(TypeError, match='filter function returned a.*'):
