...
import pytz
import pandas as pd
from pandas import DataFrame, DatetimeIndex, Index, MultiIndex, Series, Timestamp, date_range, offsets
...
class TestGroupBy():
    ...
    def test_groupby_with_timegrouper(self):
        ...
        for df in [df_original, df_reordered]:
            ...
            expected = DataFrame({'Quantity': 0}, index=date_range('20130901', '20131205', freq='5D', name='Date', closed='left'))
    ...
    def test_groupby_with_timezone_selection(self):
        np.random.seed(42)
        df = DataFrame({'factor': np.random.randint(0, 3, size=60), 'time': pd.date_range('01/01/2000 00:00', periods=60, freq='s', tz='UTC')})
    ...
    def test_first_last_max_min_on_time_data(self):
        ...
        df_test = DataFrame({'dt': [np.nan, '2015-07-24 10:10', '2015-07-25 11:11', '2015-07-23 12:12', np.nan], 'td': [np.nan, td(days=1), td(days=2), td(days=3), np.nan]})
