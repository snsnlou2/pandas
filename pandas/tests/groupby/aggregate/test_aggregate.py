...
def test_groupby_aggregation_multi_level_column():
    ...
    df = DataFrame(data=lst, columns=pd.MultiIndex.from_tuples([('A', 0), ('A', 1), ('B', 0), ('B', 1)]))
...
def test_agg_apply_corner(ts, tsframe):
    ...
    exp_df = DataFrame(columns=tsframe.columns, dtype=float, index=Index([], dtype=np.float64))
...
def test_agg_multiple_functions_same_name():
    df = DataFrame(np.random.randn(1000, 3), index=pd.date_range('1/1/2012', freq='S', periods=1000), columns=['A', 'B', 'C'])
...
def test_agg_multiple_functions_same_name_with_ohlc_present():
    ...
    expected_index = pd.date_range('1/1/2012', freq='3T', periods=6)
    expected_columns = pd.MultiIndex.from_tuples([('A', 'ohlc', 'open'), ('A', 'ohlc', 'high'), ('A', 'ohlc', 'low'), ('A', 'ohlc', 'close'), ('A', 'quantile', 'A'), ('A', 'quantile', 'A')])
...
def test_agg_split_object_part_datetime():
    ...
    expected = DataFrame({'A': [pd.Timestamp('2000')], 'B': ['a'], 'C': [1], 'D': ['b'], 'E': [pd.Timestamp('2000')], 'F': [1]})
    tm.assert_frame_equal(result, expected)
...
class TestNamedAggregationSeries():
    ...
    def test_series_named_agg(self):
        ...
        expected = DataFrame({'a': [3, 7], 'b': [1, 3]}, columns=['a', 'b'], index=[0, 1])
        tm.assert_frame_equal(result, expected)
...
class TestNamedAggregationDataFrame():
    ...
    def test_agg_relabel_with_level(self):
        df = DataFrame({'A': [0, 0, 1, 1], 'B': [1, 2, 3, 4]}, index=pd.MultiIndex.from_product([['A', 'B'], ['a', 'b']]))
...
@pytest.mark.parametrize('agg_col1, agg_col2, agg_col3, agg_result1, agg_result2, agg_result3', [((('y', 'A'), 'max'), (('y', 'A'), np.min), (('y', 'B'), 'mean'), [1, 3], [0, 2], [5.5, 7.5]), ((('y', 'A'), (lambda x: max(x))), (('y', 'A'), (lambda x: 1)), (('y', 'B'), 'mean'), [1, 3], [1, 1], [5.5, 7.5]), (pd.NamedAgg(('y', 'A'), 'max'), pd.NamedAgg(('y', 'B'), np.mean), pd.NamedAgg(('y', 'A'), (lambda x: 1)), [1, 3], [5.5, 7.5], [1, 1])])
...
def test_groupby_aggregate_empty_key(kwargs):
    ...
    expected = DataFrame([1, 4], index=Index([1, 2], dtype='int64', name='a'), columns=pd.MultiIndex.from_tuples([['c', 'min']]))
    tm.assert_frame_equal(result, expected)
