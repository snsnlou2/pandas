...
import pytest
import pandas as pd
from pandas import CategoricalIndex, DataFrame, Index, MultiIndex, Series, Timestamp, date_range
...
class TestSelection():
    ...
    def test_getitem_list_of_columns(self):
        df = DataFrame({'A': ['foo', 'bar', 'foo', 'bar', 'foo', 'bar', 'foo', 'foo'], 'B': ['one', 'one', 'two', 'three', 'two', 'two', 'one', 'three'], 'C': np.random.randn(8), 'D': np.random.randn(8), 'E': np.random.randn(8)})
...
class TestGrouping():
    ...
    def test_grouper_creation_bug(self):
        ...
        result = g.sum()
        tm.assert_frame_equal(result, expected)
        s = Series(np.arange(8, dtype='int64'), index=MultiIndex.from_product([list('ab'), range(2), date_range('20130101', periods=2)], names=['one', 'two', 'three']))
        result = s.groupby(pd.Grouper(level='three', freq='M')).sum()
        expected = Series([28], index=pd.DatetimeIndex([Timestamp('2013-01-31')], freq='M', name='three'))
        tm.assert_series_equal(result, expected)
    ...
    @pytest.mark.parametrize('func,expected', [('transform', Series(name=2, dtype=np.float64, index=Index([]))), ('agg', Series(name=2, dtype=np.float64, index=pd.Float64Index([], name=1))), ('apply', Series(name=2, dtype=np.float64, index=pd.Float64Index([], name=1)))])
    ...
    def test_groupby_multiindex_level_empty(self):
        ...
        expected = DataFrame(dtype='float64', columns=['value'], index=pd.Int64Index([], name='id'))
        tm.assert_frame_equal(result, expected)
