...
import numpy as np
import pytest
from pandas import DataFrame, Index, MultiIndex, Period, Series, Timedelta, Timestamp, date_range
...
class TestCounting():
    ...
    @pytest.mark.parametrize('datetimelike', [[Timestamp(f'2016-05-{i:02d} 20:09:25+00:00') for i in range(1, 4)], [Timestamp(f'2016-05-{i:02d} 20:09:25') for i in range(1, 4)], [Timedelta(x, unit='h') for x in range(1, 4)], [Period(freq='2W', year=2017, month=x) for x in range(1, 4)]])
...
def test_lower_int_prec_count():
    df = DataFrame({'a': np.array([0, 1, 2, 100], np.int8), 'b': np.array([1, 2, 3, 6], np.uint32), 'c': np.array([4, 5, 6, 8], np.int16), 'grp': list(('ab' * 2))})
