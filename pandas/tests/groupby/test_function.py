...
@pytest.fixture(params=[np.int32, np.int64, np.float32, np.float64], ids=['np.int32', 'np.int64', 'np.float32', 'np.float64'])
def numpy_dtypes_for_minmax(request):
    '\n    Fixture of numpy dtypes with min and max values used for testing\n    cummin and cummax\n    '
    dtype = request.param
    min_val = (np.iinfo(dtype).min if (np.dtype(dtype).kind == 'i') else np.finfo(dtype).min)
    max_val = (np.iinfo(dtype).max if (np.dtype(dtype).kind == 'i') else np.finfo(dtype).max)
...
def test_builtins_apply(keys, f):
    ...
    tm.assert_frame_equal(result, df.groupby(keys).apply(getattr(np, fname)))
    if (f != sum):
...
class TestGroupByNonCythonPaths():
    ...
    def test_describe(self, df, gb, gni):
        ...
        expected_col = pd.MultiIndex(levels=[['B'], ['count', 'mean', 'std', 'min', '25%', '50%', '75%', 'max']], codes=[([0] * 8), list(range(8))])
        ...
        result = gb.describe()
        tm.assert_frame_equal(result, expected)
        expected = pd.concat([df[(df.A == 1)].describe().unstack().to_frame().T, df[(df.A == 3)].describe().unstack().to_frame().T])
...
@pytest.mark.parametrize('method,data', [('first', {'df': [{'a': 1, 'b': 1}, {'a': 2, 'b': 3}]}), ('last', {'df': [{'a': 1, 'b': 2}, {'a': 2, 'b': 4}]}), ('min', {'df': [{'a': 1, 'b': 1}, {'a': 2, 'b': 3}]}), ('max', {'df': [{'a': 1, 'b': 2}, {'a': 2, 'b': 4}]}), ('nth', {'df': [{'a': 1, 'b': 2}, {'a': 2, 'b': 4}], 'args': [1]}), ('count', {'df': [{'a': 1, 'b': 2}, {'a': 2, 'b': 2}], 'out_type': 'int64'})])
...
def test_nlargest_mi_grouper():
    ...
    exp_idx = MultiIndex.from_tuples([(dts[0], dts[0], 'one'), (dts[1], dts[1], 'one'), (dts[2], dts[2], 'one'), (dts[3], dts[3], 'two'), (dts[4], dts[4], 'one'), (dts[5], dts[5], 'one'), (dts[6], dts[6], 'one'), (dts[7], dts[7], 'one'), (dts[8], dts[8], 'two'), (dts[9], dts[9], 'one')], names=['first', 'first', 'second'])
    exp_values = [2.2129019979039612, 1.8417114045748335, 0.858963679564603, 1.3759151378258088, 0.9430284594687134, 0.5296914208183142, 0.8318045593815487, (- 0.8476703342910327), 0.3804446884133735, (- 0.8028845810770998)]
...
def test_frame_describe_multikey(tsframe):
    ...
    for col in tsframe:
        ...
        group_col = pd.MultiIndex(levels=[[col], group.columns], codes=[([0] * len(group.columns)), range(len(group.columns))])
...
@pytest.mark.filterwarnings('ignore:indexing past lexsort depth may impact performance:pandas.errors.PerformanceWarning')
@pytest.mark.parametrize('as_index', [True, False])
def test_describe_with_duplicate_output_column_names(as_index):
    df = DataFrame({'a': [99, 99, 99, 88, 88, 88], 'b': [1, 2, 3, 4, 5, 6], 'c': [10, 20, 30, 40, 50, 60]}, columns=['a', 'b', 'b'])
    expected = DataFrame.from_records([('a', 'count', 3.0, 3.0), ('a', 'mean', 88.0, 99.0), ('a', 'std', 0.0, 0.0), ('a', 'min', 88.0, 99.0), ('a', '25%', 88.0, 99.0), ('a', '50%', 88.0, 99.0), ('a', '75%', 88.0, 99.0), ('a', 'max', 88.0, 99.0), ('b', 'count', 3.0, 3.0), ('b', 'mean', 5.0, 2.0), ('b', 'std', 1.0, 1.0), ('b', 'min', 4.0, 1.0), ('b', '25%', 4.5, 1.5), ('b', '50%', 5.0, 2.0), ('b', '75%', 5.5, 2.5), ('b', 'max', 6.0, 3.0), ('b', 'count', 3.0, 3.0), ('b', 'mean', 5.0, 2.0), ('b', 'std', 1.0, 1.0), ('b', 'min', 4.0, 1.0), ('b', '25%', 4.5, 1.5), ('b', '50%', 5.0, 2.0), ('b', '75%', 5.5, 2.5), ('b', 'max', 6.0, 3.0)]).set_index([0, 1]).T
