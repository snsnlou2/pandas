...
_results_for_groupbys_with_missing_categories = {'all': np.NaN, 'any': np.NaN, 'count': 0, 'corrwith': np.NaN, 'first': np.NaN, 'idxmax': np.NaN, 'idxmin': np.NaN, 'last': np.NaN, 'mad': np.NaN, 'max': np.NaN, 'mean': np.NaN, 'median': np.NaN, 'min': np.NaN, 'nth': np.NaN, 'nunique': 0, 'prod': np.NaN, 'quantile': np.NaN, 'sem': np.NaN, 'size': 0, 'skew': np.NaN, 'std': np.NaN, 'sum': 0, 'var': np.NaN}
...
def test_basic():
    ...
    tm.assert_frame_equal(df.groupby(c, observed=False).transform((lambda xs: np.max(xs))), df[['a']])
...
def test_observed(observed):
    ...
    tm.assert_frame_equal(result, expected)
    d = {'cat': Categorical(['a', 'b', 'a', 'b'], categories=['a', 'b', 'c'], ordered=True), 'ints': [1, 1, 2, 2], 'val': [10, 20, 30, 40]}
    df = DataFrame(d)
    ...
    expected = DataFrame({'val': [10, 30, 20, 40], 'cat': Categorical(['a', 'a', 'b', 'b'], categories=['a', 'b', 'c'], ordered=True), 'ints': [1, 2, 1, 2]}).set_index(['cat', 'ints'])
    if (not observed):
    ...
    for key in [('a', 1), ('b', 2), ('b', 1), ('a', 2)]:
        ...
        tm.assert_frame_equal(result, expected)
    d = {'foo': [10, 8, 4, 8, 4, 1, 1], 'bar': [10, 20, 30, 40, 50, 60, 70], 'baz': ['d', 'c', 'e', 'a', 'a', 'd', 'c']}
    df = DataFrame(d)
...
def test_observed_perf():
    df = DataFrame({'cat': np.random.randint(0, 255, size=30000), 'int_id': np.random.randint(0, 255, size=30000), 'other_id': np.random.randint(0, 10000, size=30000), 'foo': 0})
...
def test_bins_unequal_len():
    ...
    msg = 'Length of grouper \\(8\\) and axis \\(10\\) must be same length'
...
def test_as_index():
    ...
    expected = DataFrame({'cat': Categorical([1, 2], categories=df.cat.cat.categories), 'A': [10, 11], 'B': [101, 205]}, columns=['cat', 'A', 'B'])
    tm.assert_frame_equal(result, expected)
    f = (lambda r: df.loc[(r, 'A')])
    result = df.groupby(['cat', f], as_index=False, observed=True).sum()
    expected = DataFrame({'cat': Categorical([1, 2], categories=df.cat.cat.categories), 'A': [10, 22], 'B': [101, 205]}, columns=['cat', 'A', 'B'])
    tm.assert_frame_equal(result, expected)
...
def test_preserve_categories():
    ...
    tm.assert_index_equal(df.groupby('A', sort=False, observed=False).first().index, index)
...
def test_sort2():
    ...
    index = CategoricalIndex(['(0, 2.5]', '(2.5, 5]', '(5, 7.5]', '(7.5, 10]'], name='range', ordered=True)
    expected_sort = DataFrame([[1, 60], [5, 30], [6, 40], [10, 10]], columns=['foo', 'bar'], index=index)
    col = 'range'
    ...
    expected_sort = DataFrame([[1, 60], [5, 30], [6, 40], [10, 10]], columns=['foo', 'bar'], index=index)
    index = CategoricalIndex(['(7.5, 10]', '(2.5, 5]', '(5, 7.5]', '(0, 2.5]'], categories=['(7.5, 10]', '(2.5, 5]', '(5, 7.5]', '(0, 2.5]'], name='range')
    expected_nosort = DataFrame([[10, 10], [5, 30], [6, 40], [1, 60]], index=index, columns=['foo', 'bar'])
    col = 'range'
    result_sort = df.groupby(col, sort=True, observed=False).first()
    tm.assert_frame_equal(result_sort, expected_sort)
...
def test_sort_datetimelike():
    ...
    index = [datetime(2011, 1, 1), datetime(2011, 2, 1), datetime(2011, 5, 1), datetime(2011, 7, 1)]
    result_sort = DataFrame([[1, 60], [5, 30], [6, 40], [10, 10]], columns=['foo', 'bar'])
    result_sort.index = CategoricalIndex(index, name='dt', ordered=True)
    index = [datetime(2011, 7, 1), datetime(2011, 2, 1), datetime(2011, 5, 1), datetime(2011, 1, 1)]
    result_nosort = DataFrame([[10, 10], [5, 30], [6, 40], [1, 60]], columns=['foo', 'bar'])
    result_nosort.index = CategoricalIndex(index, categories=index, name='dt', ordered=True)
    col = 'dt'
    tm.assert_frame_equal(result_sort, df.groupby(col, sort=True, observed=False).first())
    tm.assert_frame_equal(result_sort, df.groupby(col, sort=False, observed=False).first())
    ...
    col = 'dt'
    tm.assert_frame_equal(result_sort, df.groupby(col, sort=True, observed=False).first())
    tm.assert_frame_equal(result_nosort, df.groupby(col, sort=False, observed=False).first())
...
def test_groupby_multiindex_categorical_datetime():
    df = DataFrame({'key1': Categorical(list('abcbabcba')), 'key2': Categorical((list(pd.date_range('2018-06-01 00', freq='1T', periods=3)) * 3)), 'values': np.arange(9)})
    result = df.groupby(['key1', 'key2']).mean()
    idx = MultiIndex.from_product([Categorical(['a', 'b', 'c']), Categorical(pd.date_range('2018-06-01 00', freq='1T', periods=3))], names=['key1', 'key2'])
...
@pytest.mark.parametrize('operation, kwargs', [('agg', {'dtype': 'category'}), ('apply', {})])
def test_seriesgroupby_observed_true(df_cat, operation, kwargs):
    index = MultiIndex.from_frame(DataFrame({'A': ['foo', 'foo', 'bar', 'bar'], 'B': ['one', 'two', 'one', 'three']}, **kwargs))
...
@pytest.mark.parametrize('observed', [False, None])
def test_dataframe_groupby_on_2_categoricals_when_observed_is_false(reduction_func, observed, request):
    if (reduction_func == 'ngroup'):
        pytest.skip('ngroup does not return the Categories on the index')
    df = DataFrame({'cat_1': Categorical(list('AABB'), categories=list('ABC')), 'cat_2': Categorical(list('1111'), categories=list('12')), 'value': [0.1, 0.1, 0.1, 0.1]})
    unobserved_cats = [('A', '2'), ('B', '2'), ('C', '1'), ('C', '2')]
...
def test_aggregate_categorical_lost_index(func):
...
def test_aggregate_categorical_with_isnan():
    ...
    expected = DataFrame(data={'numerical_col': [1.0, 0.0], 'object_col': [0, 0], 'categorical_col': [0, 0]}, index=index)
    tm.assert_frame_equal(result, expected)

def test_categorical_transform():
    df = DataFrame({'package_id': [1, 1, 1, 2, 2, 3], 'status': ['Waiting', 'OnTheWay', 'Delivered', 'Waiting', 'OnTheWay', 'Waiting']})
    ...
    result = df.copy()
    expected = DataFrame({'package_id': [1, 1, 1, 2, 2, 3], 'status': ['Waiting', 'OnTheWay', 'Delivered', 'Waiting', 'OnTheWay', 'Waiting'], 'last_status': ['Delivered', 'Delivered', 'Delivered', 'OnTheWay', 'OnTheWay', 'Waiting']})
    expected['status'] = expected['status'].astype(delivery_status_type)
    tm.assert_frame_equal(result, expected)

@pytest.mark.parametrize('func', ['first', 'last'])
def test_series_groupby_first_on_categorical_col_grouped_on_2_categoricals(func, observed):
...
@pytest.mark.parametrize('func', ['first', 'last'])
def test_df_groupby_first_on_categorical_col_grouped_on_2_categoricals(func, observed):
    cat = Categorical([0, 0, 1, 1])
    val = [0, 1, 1, 0]
    df = DataFrame({'a': cat, 'b': cat, 'c': val})
    idx = Categorical([0, 1])
...
def test_groupby_categorical_indices_unused_categories():
    ...
    expected = {'b': np.array([0, 1], dtype='int64'), 'a': np.array([2], dtype='int64'), 'c': np.array([], dtype='int64')}
