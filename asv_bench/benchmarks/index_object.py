...
import gc
import numpy as np
from pandas import DatetimeIndex, Float64Index, Index, IntervalIndex, MultiIndex, RangeIndex, Series, date_range
from .pandas_vb_common import tm

class SetOperations():
    params = (['datetime', 'date_string', 'int', 'strings'], ['intersection', 'union', 'symmetric_difference'])
    ...
    def setup(self, dtype, method):
        ...
        data = {'datetime': {'left': dates_left, 'right': dates_left[:(- 1)]}, 'date_string': {'left': date_str_left, 'right': date_str_left[:(- 1)]}, 'int': {'left': int_left, 'right': int_left[:(- 1)]}, 'strings': {'left': str_left, 'right': str_left[:(- 1)]}}
...
class Float64IndexMethod():
    ...
    def setup(self):
        N = 100000
        a = np.arange(N)
        self.ind = Float64Index((a * 4.800000041882413e-08))
