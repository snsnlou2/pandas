...
class Methods():
    params = (['DataFrame', 'Series'], [10, 1000], ['int', 'float'], ['median', 'mean', 'max', 'min', 'std', 'count', 'skew', 'kurt', 'sum'])
...
class Engine():
    params = (['DataFrame', 'Series'], ['int', 'float'], [np.sum, (lambda x: (np.sum(x) + 5))], ['cython', 'numba'])
...
class ExpandingMethods():
    params = (['DataFrame', 'Series'], ['int', 'float'], ['median', 'mean', 'max', 'min', 'std', 'count', 'skew', 'kurt', 'sum'])
    param_names = ['constructor', 'window', 'dtype', 'method']
    ...
    def setup(self, constructor, dtype, method):
        ...
        self.expanding_groupby = pd.DataFrame({'A': arr[:N_groupby], 'B': range(N_groupby)}).groupby('B').expanding()
...
class VariableWindowMethods(Methods):
    params = (['DataFrame', 'Series'], ['50s', '1h', '1d'], ['int', 'float'], ['median', 'mean', 'max', 'min', 'std', 'count', 'skew', 'kurt', 'sum'])
    param_names = ['constructor', 'window', 'dtype', 'method']
...
class Groupby():
    ...
    def setup(self, method):
        N = 1000
        df = pd.DataFrame({'A': ([str(i) for i in range(N)] * 10), 'B': (list(range(N)) * 10), 'C': pd.date_range(start='1900-01-01', freq='1min', periods=(N * 10))})
