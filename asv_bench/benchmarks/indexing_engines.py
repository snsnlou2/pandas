...
class NumericEngineIndexing():
    params = [_get_numeric_engines(), ['monotonic_incr', 'monotonic_decr', 'non_monotonic']]
    ...
    def setup(self, engine_and_dtype, index_type):
        ...
        arr = {'monotonic_incr': np.array(values, dtype=dtype), 'monotonic_decr': np.array(list(reversed(values)), dtype=dtype), 'non_monotonic': np.array(([1, 2, 3] * N), dtype=dtype)}[index_type]
