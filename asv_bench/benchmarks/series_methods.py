...
class IsInDatetime64():
    ...
    def setup(self):
        dti = date_range(start=datetime(2015, 10, 26), end=datetime(2016, 1, 1), freq='50s')
        self.ser = Series(dti)
...
class IsInLongSeriesLookUpDominates():
    params = [['int64', 'int32', 'float64', 'float32', 'object'], [5, 1000], ['random_hits', 'random_misses', 'monotone_hits', 'monotone_misses']]
...
class IsInLongSeriesValuesDominate():
    params = [['int64', 'int32', 'float64', 'float32', 'object'], ['random', 'monotone']]
...
class SeriesGetattr():
    ...
    def setup(self):
        self.s = Series(1, index=date_range('2012-01-01', freq='s', periods=int(1000000.0)))
