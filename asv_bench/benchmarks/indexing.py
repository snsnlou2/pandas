...
import warnings
import numpy as np
from pandas import CategoricalIndex, DataFrame, Float64Index, IndexSlice, Int64Index, IntervalIndex, MultiIndex, Series, UInt64Index, concat, date_range, option_context, period_range
from .pandas_vb_common import tm

class NumericSeriesIndexing():
    params = [(Int64Index, UInt64Index, Float64Index), ('unique_monotonic_inc', 'nonunique_monotonic_inc')]
    param_names = ['index_dtype', 'index_structure']

    def setup(self, index, index_structure):
        N = (10 ** 6)
        indices = {'unique_monotonic_inc': index(range(N)), 'nonunique_monotonic_inc': index(((list(range(55)) + [54]) + list(range(55, (N - 1)))))}
...
class NonNumericSeriesIndexing():
    params = [('string', 'datetime', 'period'), ('unique_monotonic_inc', 'nonunique_monotonic_inc', 'non_monotonic')]
    param_names = ['index_dtype', 'index_structure']
...
class Take():
    ...
    def setup(self, index):
        N = 100000
        indexes = {'int': Int64Index(np.arange(N)), 'datetime': date_range('2011-01-01', freq='S', periods=N)}
        index = indexes[index]
        self.s = Series(np.random.rand(N), index=index)
...
class MultiIndexing():
    ...
    def setup(self):
        ...
        n = 100000
        with warnings.catch_warnings(record=True):
            self.mdt = DataFrame({'A': np.random.choice(range(10000, 45000, 1000), n), 'B': np.random.choice(range(10, 400), n), 'C': np.random.choice(range(1, 150), n), 'D': np.random.choice(range(10000, 45000), n), 'x': np.random.choice(range(400), n), 'y': np.random.choice(range(25), n)})
...
class CategoricalIndexIndexing():
    ...
    def setup(self, index):
        ...
        indices = {'monotonic_incr': CategoricalIndex(values), 'monotonic_decr': CategoricalIndex(reversed(values)), 'non_monotonic': CategoricalIndex(list(('abc' * N)))}
        self.data = indices[index]
        self.data_unique = CategoricalIndex([''.join(perm) for perm in itertools.permutations(string.printable, 3)])
