...
class Count():
    ...
    def setup(self, axis):
        ...
        self.df_mixed.index = MultiIndex.from_arrays([self.df_mixed.index, self.df_mixed.index])
        self.df_mixed.columns = MultiIndex.from_arrays([self.df_mixed.columns, self.df_mixed.columns])
...
class SortIndexByColumns():
    ...
    def setup(self):
        N = 10000
        K = 10
        self.df = DataFrame({'key1': tm.makeStringIndex(N).values.repeat(K), 'key2': tm.makeStringIndex(N).values.repeat(K), 'value': np.random.randn((N * K))})
...
class Describe():
    ...
    def setup(self):
        self.df = DataFrame({'a': np.random.randint(0, 100, int(1000000.0)), 'b': np.random.randint(0, 100, int(1000000.0)), 'c': np.random.randint(0, 100, int(1000000.0))})
