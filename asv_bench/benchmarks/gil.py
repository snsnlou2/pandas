...
class ParallelReadCSV(BaseIO):
    ...
    def setup(self, dtype):
        ...
        rows = 10000
        cols = 50
        data = {'float': DataFrame(np.random.randn(rows, cols)), 'datetime': DataFrame(np.random.randn(rows, cols), index=date_range('1/1/2000', periods=rows)), 'object': DataFrame('foo', index=range(rows), columns=['object%03d' for _ in range(5)])}
