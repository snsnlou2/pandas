...
class Pivot():
    ...
    def setup(self):
        ...
        data = {'value': np.random.randn((N * 50)), 'variable': np.arange(50).repeat(N), 'date': np.tile(index.values, 50)}
...
class SparseIndex():
    ...
    def setup(self):
        NUM_ROWS = 1000
        self.df = DataFrame({'A': np.random.randint(50, size=NUM_ROWS), 'B': np.random.randint(50, size=NUM_ROWS), 'C': np.random.randint((- 10), 10, size=NUM_ROWS), 'D': np.random.randint((- 10), 10, size=NUM_ROWS), 'E': np.random.randint(10, size=NUM_ROWS), 'F': np.random.randn(NUM_ROWS)})
...
class WideToLong():
    ...
    def setup(self):
        ...
        yrvars = [(letter + str(num)) for (letter, num) in product(self.letters, range(1, (nyrs + 1)))]
...
class PivotTable():
    ...
    def setup(self):
        ...
        self.df = DataFrame({'key1': fac1.take(ind1), 'key2': fac2.take(ind2), 'key3': fac2.take(ind2), 'value1': np.random.randn(N), 'value2': np.random.randn(N), 'value3': np.random.randn(N)})
        self.df2 = DataFrame({'col1': list('abcde'), 'col2': list('fghij'), 'col3': [1, 2, 3, 4, 5]})
    ...
    def time_pivot_table_agg(self):
        self.df.pivot_table(index='key1', columns=['key2', 'key3'], aggfunc=['sum', 'mean'])
    ...
    def time_pivot_table_categorical(self):
        self.df2.pivot_table(index='col1', values='col3', columns='col2', aggfunc=np.sum, fill_value=0)

    def time_pivot_table_categorical_observed(self):
        self.df2.pivot_table(index='col1', values='col3', columns='col2', aggfunc=np.sum, fill_value=0, observed=True)
...
class GetDummies():
    ...
    def setup(self):
        ...
        s = pd.Series(np.random.choice(categories, size=1000000), dtype=pd.api.types.CategoricalDtype(categories))
        self.s = s
...
class Cut():
    ...
    def setup(self, bins):
        ...
        self.timedelta_series = pd.Series(np.random.randint(N, size=N), dtype='timedelta64[ns]')
        self.datetime_series = pd.Series(np.random.randint(N, size=N), dtype='datetime64[ns]')
