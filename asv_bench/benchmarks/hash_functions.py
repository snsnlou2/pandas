...
class IsinAlmostFullWithRandomInt():
    params = [[np.float64, np.int64, np.uint64, np.object], range(10, 21)]
...
class IsinWithRandomFloat():
    params = [[np.float64, np.object], [1300, 2000, 7000, 8000, 70000, 80000, 750000, 900000]]
    param_names = ['dtype', 'M']
...
class IsinWithArangeSorted():
    params = [[np.float64, np.int64, np.uint64, np.object], [1000, 2000, 8000, 100000, 1000000]]
    param_names = ['dtype', 'M']
...
class IsinWithArange():
    params = [[np.float64, np.int64, np.uint64, np.object], [1000, 2000, 8000], [(- 2), 0, 2]]
...
class Float64GroupIndex():
    ...
    def setup(self):
        self.df = pd.date_range(start='1/1/2018', end='1/2/2018', periods=1000000.0).to_frame()
        self.group_index = np.round((self.df.index.astype(int) / 1000000000.0))
...
class NumericSeriesIndexing():
    params = [(pd.Int64Index, pd.UInt64Index, pd.Float64Index), ((10 ** 4), (10 ** 5), (5 * (10 ** 5)), (10 ** 6), (5 * (10 ** 6)))]
