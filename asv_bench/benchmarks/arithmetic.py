...
class FrameWithFrameWide():
    params = [[operator.add, operator.floordiv, operator.gt]]
    param_names = ['op']
    ...
    def setup(self, op):
        ...
        df2 = pd.concat([pd.DataFrame(arr1), pd.DataFrame(arr2), pd.DataFrame(arr3)], axis=1, ignore_index=True)
...
class AddOverflowArray():
    ...
    def time_add_overflow_both_arg_nan(self):
        checked_add_with_arr(self.arr, self.arr_mixed, arr_mask=self.arr_nan_1, b_mask=self.arr_nan_2)
...
class BinaryOpsMultiIndex():
    ...
    def setup(self, func):
        ...
        self.df = pd.DataFrame(np.random.rand(len(index), 2), index=index, columns=column_names)
        self.arg_df = pd.DataFrame(np.random.randint(1, 10, (len(level_0_names), 2)), index=level_0_names, columns=column_names)
