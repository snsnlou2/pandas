...
from .pandas_vb_common import datetime_dtypes, extension_dtypes, lib, numeric_dtypes, string_dtypes
_numpy_dtypes = [np.dtype(dtype) for dtype in ((numeric_dtypes + datetime_dtypes) + string_dtypes)]
...
class DtypesInvalid():
    ...
    data_dict = {'scalar-string': 'foo', 'scalar-int': 1, 'list-string': (['foo'] * 1000), 'array-string': np.array((['foo'] * 1000))}
...
class SelectDtypes():
    params = [((((((tm.ALL_INT_DTYPES + tm.ALL_EA_INT_DTYPES) + tm.FLOAT_DTYPES) + tm.COMPLEX_DTYPES) + tm.DATETIME64_DTYPES) + tm.TIMEDELTA64_DTYPES) + tm.BOOL_DTYPES)]
    param_names = ['dtype']
    ...
    def setup(self, dtype):
        ...
        self.df_string = create_df(np.random.choice(list(string.ascii_letters), size=(N, K)))
