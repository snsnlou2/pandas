...
class JoinIndex():
    ...
    def setup(self):
        N = 50000
        self.left = DataFrame(np.random.randint(1, (N / 500), (N, 2)), columns=['jim', 'joe'])
        self.right = DataFrame(np.random.randint(1, (N / 500), (N, 2)), columns=['jolie', 'jolia']).set_index('jolie')
...
class MergeCategoricals():
    ...
    def setup(self):
        self.left_object = DataFrame({'X': np.random.choice(range(0, 10), size=(10000,)), 'Y': np.random.choice(['one', 'two', 'three'], size=(10000,))})
        self.right_object = DataFrame({'X': np.random.choice(range(0, 10), size=(10000,)), 'Z': np.random.choice(['jjj', 'kkk', 'sss'], size=(10000,))})
