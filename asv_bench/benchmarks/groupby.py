...
import numpy as np
from pandas import Categorical, DataFrame, MultiIndex, Series, Timestamp, date_range, period_range
from .pandas_vb_common import tm
method_blocklist = {'object': {'median', 'prod', 'sem', 'cumsum', 'sum', 'cummin', 'mean', 'max', 'skew', 'cumprod', 'cummax', 'pct_change', 'min', 'var', 'mad', 'describe', 'std', 'quantile'}, 'datetime': {'median', 'prod', 'sem', 'cumsum', 'sum', 'mean', 'skew', 'cumprod', 'cummax', 'pct_change', 'var', 'mad', 'describe', 'std'}}
...
class Groups():
    ...
    def setup_cache(self):
        size = (10 ** 6)
        data = {'int64_small': Series(np.random.randint(0, 100, size=size)), 'int64_large': Series(np.random.randint(0, 10000, size=size)), 'object_small': Series(tm.makeStringIndex(100).take(np.random.randint(0, 100, size=size))), 'object_large': Series(tm.makeStringIndex(10000).take(np.random.randint(0, 10000, size=size)))}
        return data
...
class AggFunctions():
    ...
    def time_different_numpy_functions(self, df):
        df.groupby(['key1', 'key2']).agg({'value1': np.mean, 'value2': np.var, 'value3': np.sum})
...
class GroupByMethods():
    ...
    params = [['int', 'float', 'object', 'datetime'], ['all', 'any', 'bfill', 'count', 'cumcount', 'cummax', 'cummin', 'cumprod', 'cumsum', 'describe', 'ffill', 'first', 'head', 'last', 'mad', 'max', 'min', 'median', 'mean', 'nunique', 'pct_change', 'prod', 'quantile', 'rank', 'sem', 'shift', 'size', 'skew', 'std', 'sum', 'tail', 'unique', 'value_counts', 'var'], ['direct', 'transformation']]
