...
class SQL():
    ...
    def setup(self, connection):
        N = 10000
        con = {'sqlalchemy': create_engine('sqlite:///:memory:'), 'sqlite': sqlite3.connect(':memory:')}
        self.table_name = 'test_type'
        self.query_all = f'SELECT * FROM {self.table_name}'
        self.con = con[connection]
        self.df = DataFrame({'float': np.random.randn(N), 'float_with_nan': np.random.randn(N), 'string': (['foo'] * N), 'bool': ([True] * N), 'int': np.random.randint(0, N, size=N), 'datetime': date_range('2000-01-01', periods=N, freq='s')}, index=tm.makeStringIndex(N))
...
class WriteSQLDtypes():
    params = (['sqlalchemy', 'sqlite'], ['float', 'float_with_nan', 'string', 'bool', 'int', 'datetime'])
...
class ReadSQLTable():
    ...
    def time_read_sql_table_parse_dates(self):
        read_sql_table(self.table_name, self.con, columns=['datetime_string'], parse_dates=['datetime_string'])
