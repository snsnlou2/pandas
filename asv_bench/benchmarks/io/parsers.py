...
import numpy as np
try:
    from pandas._libs.tslibs.parsing import _does_string_look_like_datetime, concat_date_cols
except ImportError:
    pass
...
class ConcatDateCols():
    ...
    def setup(self, value, dim):
        ...
        if (dim == 2):
            self.object = (np.array(([value] * count_elem)), np.array(([value] * count_elem)))
