...
class ToCSV(BaseIO):
    ...
    def setup(self, kind):
        ...
        long_frame = DataFrame({'A': np.arange(50000), 'B': (np.arange(50000) + 1.0), 'C': (np.arange(50000) + 2.0), 'D': (np.arange(50000) + 3.0)})
        mixed_frame = DataFrame({'float': np.random.randn(5000), 'int': np.random.randn(5000).astype(int), 'bool': ((np.arange(5000) % 2) == 0), 'datetime': date_range('2001', freq='s', periods=5000), 'object': (['foo'] * 5000)})
...
class ToCSVDatetimeBig(BaseIO):
    ...
    def setup(self, obs):
        ...
        self.data = DataFrame({'dt': ([np.datetime64(dt)] * obs), 'd': ([np.datetime64(d)] * obs), 'r': ([np.random.uniform()] * obs)})
...
class ReadCSVDInferDatetimeFormat(StringIORewind):
    ...
    def setup(self, infer_datetime_format, format):
        rng = date_range('1/1/2000', periods=1000)
        formats = {'custom': '%m/%d/%Y %H:%M:%S.%f', 'iso8601': '%Y-%m-%d %H:%M:%S', 'ymd': '%Y%m%d'}
        dt_format = formats[format]
        self.StringIO_input = StringIO('\n'.join(rng.strftime(dt_format).tolist()))

    def time_read_csv(self, infer_datetime_format, format):
        read_csv(self.data(self.StringIO_input), header=None, names=['foo'], parse_dates=['foo'], infer_datetime_format=infer_datetime_format)
...
class ReadCSVConcatDatetime(StringIORewind):
    ...
    def time_read_csv(self):
        read_csv(self.data(self.StringIO_input), header=None, names=['foo'], parse_dates=['foo'], infer_datetime_format=False)
...
''' * 50000))
    ...
    def time_read_csv(self, bad_date_value):
        read_csv(self.data(self.StringIO_input), header=None, names=['foo', 'bar'], parse_dates=['foo'], infer_datetime_format=False)
...
class ReadCSVSkipRows(BaseIO):
    ...
    def setup(self, skiprows, engine):
        ...
        df = DataFrame({'float1': np.random.randn(N), 'float2': np.random.randn(N), 'string1': (['foo'] * N), 'bool1': ([True] * N), 'int1': np.random.randint(0, N, size=N)}, index=index)
...
class ReadUint64Integers(StringIORewind):
    ...
    def time_read_uint64_na_values(self):
        read_csv(self.data(self.data1), header=None, names=['foo'], na_values=self.na_values)
...
class ReadCSVComment(StringIORewind):
    ...
    def time_comment(self, engine):
        read_csv(self.data(self.StringIO_input), comment='#', header=None, names=list('abc'))
...
class ReadCSVFloatPrecision(StringIORewind):
    ...
    def setup(self, sep, decimal, float_precision):
        floats = [''.join((random.choice(string.digits) for _ in range(28))) for _ in range(15)]
    ...
    def time_read_csv(self, sep, decimal, float_precision):
        read_csv(self.data(self.StringIO_input), sep=sep, header=None, names=list('abc'), float_precision=float_precision)

    def time_read_csv_python_engine(self, sep, decimal, float_precision):
        read_csv(self.data(self.StringIO_input), sep=sep, header=None, engine='python', float_precision=None, names=list('abc'))
...
class ReadCSVParseDates(StringIORewind):
    ...
    def time_multiple_date(self, engine):
        read_csv(self.data(self.StringIO_input), engine=engine, sep=',', header=None, names=list(string.digits[:9]), parse_dates=[[1, 2], [1, 3]])

    def time_baseline(self, engine):
        read_csv(self.data(self.StringIO_input), engine=engine, sep=',', header=None, parse_dates=[1], names=list(string.digits[:9]))
...
class ReadCSVCachedParseDates(StringIORewind):
    ...
    def time_read_csv_cached(self, do_cache, engine):
        try:
            read_csv(self.data(self.StringIO_input), engine=engine, header=None, parse_dates=[0], cache_dates=do_cache)
        except TypeError:
            pass
...
class ReadCSVParseSpecialDate(StringIORewind):
    ...
    def time_read_special_date(self, value, engine):
        read_csv(self.data(self.StringIO_input), engine=engine, sep=',', header=None, names=['Date'], parse_dates=['Date'])
...
class ParseDateComparison(StringIORewind):
    ...
    def time_read_csv_dayfirst(self, cache_dates):
        try:
            read_csv(self.data(self.StringIO_input), sep=',', header=None, names=['Date'], parse_dates=['Date'], cache_dates=cache_dates, dayfirst=True)
        except TypeError:
            pass

    def time_to_datetime_dayfirst(self, cache_dates):
        df = read_csv(self.data(self.StringIO_input), dtype={'date': str}, names=['date'])
