...
class ReadJSON(BaseIO):
    ...
    def setup(self, orient, index):
        N = 100000
        indexes = {'int': np.arange(N), 'datetime': date_range('20000101', periods=N, freq='H')}
        df = DataFrame(np.random.randn(N, 5), columns=[f'float_{i}' for i in range(5)], index=indexes[index])
...
class ToJSON(BaseIO):
    fname = '__test__.json'
    params = [['split', 'columns', 'index', 'values', 'records'], ['df', 'df_date_idx', 'df_td_int_ts', 'df_int_floats', 'df_int_float_str']]
    ...
    def setup(self, orient, frame):
        ...
        self.df_td_int_ts = DataFrame({'td_1': timedeltas, 'td_2': timedeltas, 'int_1': ints, 'int_2': ints, 'ts_1': datetimes, 'ts_2': datetimes}, index=index)
        self.df_int_floats = DataFrame({'int_1': ints, 'int_2': ints, 'int_3': ints, 'float_1': floats, 'float_2': floats, 'float_3': floats}, index=index)
        self.df_int_float_str = DataFrame({'int_1': ints, 'int_2': ints, 'float_1': floats, 'float_2': floats, 'str_1': strings, 'str_2': strings}, index=index)
        self.df_longint_float_str = DataFrame({'longint_1': longints, 'longint_2': longints, 'float_1': floats, 'float_2': floats, 'str_1': strings, 'str_2': strings}, index=index)
...
class ToJSONISO(BaseIO):
    ...
    def setup(self, orient):
        ...
        timedeltas = timedelta_range(start=1, periods=N, freq='s')
        datetimes = date_range(start=1, periods=N, freq='s')
        self.df = DataFrame({'td_1': timedeltas, 'td_2': timedeltas, 'ts_1': datetimes, 'ts_2': datetimes}, index=index)
