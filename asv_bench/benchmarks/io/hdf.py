...
class HDFStoreDataFrame(BaseIO):
    ...
    def setup(self):
        ...
        self.df = DataFrame({'float1': np.random.randn(N), 'float2': np.random.randn(N)}, index=index)
        self.df_mixed = DataFrame({'float1': np.random.randn(N), 'float2': np.random.randn(N), 'string1': (['foo'] * N), 'bool1': ([True] * N), 'int1': np.random.randint(0, N, size=N)}, index=index)
        ...
        self.df2 = DataFrame({'float1': np.random.randn(N), 'float2': np.random.randn(N)}, index=date_range('1/1/2000', periods=N))
        ...
        self.df_dc = DataFrame(np.random.randn(N, 10), columns=[('C%03d' % i) for i in range(10)])
    ...
    def time_query_store_table_wide(self):
        self.store.select('table_wide', where='index > self.start_wide and index < self.stop_wide')
