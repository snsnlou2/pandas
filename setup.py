...
_pxi_dep_template = {'algos': ['_libs/algos_common_helper.pxi.in', '_libs/algos_take_helper.pxi.in'], 'hashtable': ['_libs/hashtable_class_helper.pxi.in', '_libs/hashtable_func_helper.pxi.in', '_libs/khash_for_primitive_helper.pxi.in'], 'index': ['_libs/index_class_helper.pxi.in'], 'sparse': ['_libs/sparse_op_helper.pxi.in'], 'interval': ['_libs/intervaltree.pxi.in']}
_pxifiles = []
_pxi_dep = {}
...
if is_platform_mac():
    if ('MACOSX_DEPLOYMENT_TARGET' not in os.environ):
        ...
        if ((LooseVersion(str(python_target)) < '10.9') and (LooseVersion(current_system) >= '10.9')):
...
def maybe_cythonize(extensions, *args, **kwargs):
    ...
    if (('clean' in sys.argv) or ('sdist' in sys.argv)):
        ...
        if _CYTHON_VERSION:
            raise RuntimeError(f'Cannot cythonize with old Cython version ({_CYTHON_VERSION} installed, needs {min_cython_ver})')
...
for (name, data) in ext_data.items():
    ...
    obj = Extension(f'pandas.{name}', sources=sources, depends=data.get('depends', []), include_dirs=include, language=data.get('language', 'c'), define_macros=data.get('macros', macros), extra_compile_args=extra_compile_args, extra_link_args=extra_link_args)
...
def setup_package():
    setuptools_kwargs = {'install_requires': ['python-dateutil >= 2.7.3', 'pytz >= 2017.3', f'numpy >= {min_numpy_ver}'], 'setup_requires': [f'numpy >= {min_numpy_ver}'], 'zip_safe': False}
    setup(name=DISTNAME, maintainer=AUTHOR, version=versioneer.get_version(), packages=find_packages(include=['pandas', 'pandas.*']), package_data={'': ['templates/*', '_libs/**/*.dll']}, ext_modules=maybe_cythonize(extensions, compiler_directives=directives), maintainer_email=EMAIL, description=DESCRIPTION, license=LICENSE, cmdclass=cmdclass, url=URL, download_url=DOWNLOAD_URL, project_urls=PROJECT_URLS, long_description=LONG_DESCRIPTION, classifiers=CLASSIFIERS, platforms='any', python_requires='>=3.7.1', extras_require={'test': ['pytest>=5.0.1', 'pytest-xdist', 'hypothesis>=3.58']}, entry_points={'pandas_plotting_backends': ['matplotlib = pandas:plotting._matplotlib']}, **setuptools_kwargs)
