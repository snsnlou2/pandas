...
def main(conda_fname, pip_fname, compare=False):
    ...
    header = f'''# This file is auto-generated from {fname}, do not modify.
...
if (__name__ == '__main__'):
    ...
    argparser.add_argument('--azure', action='store_true', help='show the output in azure-pipelines format')
    args = argparser.parse_args()
    repo_path = os.path.dirname(os.path.abspath(os.path.dirname(__file__)))
    res = main(os.path.join(repo_path, 'environment.yml'), os.path.join(repo_path, 'requirements-dev.txt'), compare=args.compare)
    if res:
        msg = f'''`requirements-dev.txt` has to be generated with `{sys.argv[0]}` after `environment.yml` is modified.
'''
        if args.azure:
