...
ERROR_MSGS = {'GL04': 'Private classes ({mentioned_private_classes}) should not be mentioned in public docstrings', 'SA05': '{reference_name} in `See Also` section does not need `pandas` prefix, use {right_reference} instead.', 'EX02': 'Examples do not pass tests:\n{doctest_log}', 'EX03': 'flake8 error: {error_code} {error_message}{times_happening}', 'EX04': 'Do not import {imported_library}, as it is imported automatically for the examples (numpy as np, pandas as pd)'}
...
def get_api_items(api_doc_fd):
    ...
    for line in api_doc_fd:
        ...
        if (position == 'items'):
            ...
            (yield ('.'.join([current_module, item]), func, current_section, current_subsection))
        previous_line = line
...
def pandas_validate(func_name):
    ...
    if doc.see_also:
        for (rel_name, rel_desc) in doc.see_also.items():
            if rel_name.startswith('pandas.'):
                result['errors'].append(pandas_error('SA05', reference_name=rel_name, right_reference=rel_name[len('pandas.'):]))
    ...
    if doc.examples:
        ...
        for err in doc.validate_pep8():
            result['errors'].append(pandas_error('EX03', error_code=err.error_code, error_message=err.message, times_happening=(f' ({err.count} times)' if (err.count > 1) else '')))
        examples_source_code = ''.join(doc.examples_source_code)
        for wrong_import in ('numpy', 'pandas'):
            if (f'import {wrong_import}' in examples_source_code):
                result['errors'].append(pandas_error('EX04', imported_library=wrong_import))
    return result
...
def print_validate_all_results(prefix, errors, output_format, ignore_deprecated):
...
def main(func_name, prefix, errors, output_format, ignore_deprecated):
    ...
    if (func_name is None):
        return print_validate_all_results(prefix, errors, output_format, ignore_deprecated)
    else:
        print_validate_one_results(func_name)
        return 0
if (__name__ == '__main__'):
    format_opts = ('default', 'json', 'actions')
    func_help = 'function or method to validate (e.g. pandas.DataFrame.head) if not provided, all docstrings are validated and returned as JSON'
    ...
    argparser.add_argument('--format', default='default', choices=format_opts, help='format of the output when validating multiple docstrings (ignored when validating one). It can be {str(format_opts)[1:-1]}')
    argparser.add_argument('--prefix', default=None, help='pattern for the docstring names, in order to decide which ones will be validated. A prefix "pandas.Series.str."will make the script validate all the docstrings of methods starting by this pattern. It is ignored if parameter function is provided')
    argparser.add_argument('--errors', default=None, help='comma separated list of error codes to validate. By default it validates all errors (ignored when validating a single docstring)')
    argparser.add_argument('--ignore_deprecated', default=False, action='store_true', help='if this flag is set, deprecated objects are ignored when validating all docstrings')
    args = argparser.parse_args()
    sys.exit(main(args.function, args.prefix, (args.errors.split(',') if args.errors else None), args.format, args.ignore_deprecated))
